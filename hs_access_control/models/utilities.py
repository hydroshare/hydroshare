...
def coarse_permissions(u, r):
    ...
    if UserResourcePrivilege.objects.filter(user=u, resource=r).exists():
        results.append(
            "{} has user-resource privilege over '{}'".format(u.username, r.title)
        )

    # check raw user-group privilege
    if UserGroupPrivilege.objects.filter(user=u, group__g2grp__resource=r).exists():
        results.append(
            "{} has user-group-resource privilege over '{}'".format(u.username, r.title)
        )

    # check whether members of peer group can view community
    # we want to exclude cases where the peer group is self.
    if (
        UserGroupPrivilege.objects.filter(
            user=u, group__g2gcp__community__c2gcp__group__g2grp__resource=r
        )
        .exclude(
            pk__in=UserGroupPrivilege.objects.filter(user=u, group__g2grp__resource=r)
        )
        .exists()
    ):
        results.append(
            "{} has user-group-community-group-resource privilege over '{}'".format(
                u.username, r.title
            )
        )
    return results
    ...
    for q in UserGroupPrivilege.objects.filter(
        ...
        for q2 in GroupCommunityPrivilege.objects.filter(
            ...
            for q3 in GroupCommunityPrivilege.objects.filter(
                community=q2.community, community__c2gcp__group__g2grp__resource=r
            ):
                for q4 in GroupResourcePrivilege.objects.filter(
                    group=q3.group, resource=r
                ):
                    results.append(
                        (
                            q,
...
def access_provenance(u, r):
    ...
    output = "user {} {} resource '{}'\n".format(
        u.username, verbs[r.raccess.get_effective_privilege(u)], r.title
    )
    ...
    for tuple in e:
        ...
        for e in elements:
            if isinstance(e, UserResourcePrivilege):
                output += "  * user {} {} resource.\n".format(
                    e.user.username, verbs[e.privilege]
                )
            elif isinstance(e, UserGroupPrivilege):
                output += "  * user {} {} group {},\n".format(
                    e.user.username, verbs[e.privilege], e.group.name
                )
                ...
                if not found:
                    output += "    which {} community {},\n".format(
                        verbs[e.privilege], e.community.name
                    )
                    found = True
                else:
                    output += "    which {} resources of group {},\n".format(
                        verbs[e.privilege], e.group.name
                    )
