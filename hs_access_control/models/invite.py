...
class GroupCommunityRequest(models.Model):
    ...
    * The group owner uses GroupCommunityRequest.act() to respond.
    ...
    group = models.ForeignKey(
        Group, on_delete=models.CASCADE, editable=False, null=False
    )
    # source
    community = models.ForeignKey(
        Community, on_delete=models.CASCADE, editable=False, null=False
    )
    # invitee
    ...
    privilege = models.IntegerField(
        choices=PrivilegeCodes.CHOICES, editable=False, null=True
    )
    ...
    def __str__(self):
        return "GroupCommunityRequest: community='{}' owned by '{}', group='{}' owned by '{}'".format(
            str(self.community),
            str(self.community_owner),
            str(self.group),
            str(self.group_owner),
        )
    ...
    def create_or_update(cls, **kwargs):
        ...
        * GroupCommunityRequest.pending(requester={X}, group={Y}, community={Z}):
        ...
        if group in community.member_groups:
            message = "Group '{}' is already connected to community '{}'.".format(
                group.name, community.name
            )
            return message, True

        # requester owns both.
        if requester.uaccess.owns_community(community) and requester.uaccess.owns_group(
            group
        ):
            ...
            message = (
                ...
                " because you own both.".format(
                    request.group.name, request.community.name
                )
            )
            ...
            message = "Request created to connect group '{}' to community '{}'.".format(
                group.name, community.name
            )
            ...
            # refresh request: this has the side effect of disabling any prior denials
                ...
                request.save()
                message = (
                    "Request updated: connect group '{}' to community '{}'.".format(
                        group.name, community.name
                    )
                )
            ...
            with transaction.atomic():
                request, created = cls.objects.get_or_create(
                    defaults={"group_owner": group_owner},
                    community=community,
                    group=group,
                )
            ...
            # auto-approve if auto_approve is True
                ...
                message = "Request auto-approved to connect group '{}' to community '{}'.".format(
                    group.name, community.name
                )

            elif (
                not created
            ):  # refresh request: this has the side effect of disabling any denials
                request.group_owner = group_owner
    ...
    def cancel(self, requester):
        ...
        * success: whether the action was taken successfully.
        ...
        if self.redeemed:
            message = (
                "Connection request between community '{}' and group '{}': "
                "already acted upon.".format(self.community.name, self.group.name)
            )
        ...
        if (self.group_owner is not None and requester == self.group_owner) or (
            ...
            self.delete()
            message = "Connection request between community '{}' and group '{}' cancelled.".format(
                self.community.name, self.group.name
            )
            return message, True
        else:
            message = (
                "Connection request between community '{}' and group '{}': "
                "insufficient privilege to cancel request.".format(
                    self.community.name, self.group.name
                )
            )
            return message, False
    ...
    def queued(cls, requester=None):
        ...
        if requester is not None:
            ...
            requests = requests.filter(
                Q(
                    group__g2ugp__user=requester,
                    group__g2ugp__privilege=PrivilegeCodes.OWNER,
                )
                | Q(
                    community__c2ucp__user=requester,
                    community__c2ucp__privilege=PrivilegeCodes.OWNER,
                )
            )
        return requests
    ...
    def approve(self, responder, privilege=PrivilegeCodes.VIEW):
        ...
        if self.community_owner is None:
            if responder.uaccess.owns_community(self.community):
                ...
                message = (
                    "Request to connect group '{}' to community '{}' approved.".format(
                        self.group.name, self.community.name
                    )
                )
                return message, True
            else:
                message = (
                    "You do not own the community and cannot approve this request."
                )
                return message, False
    ...
    def decline(self, responder):
        ...
        if self.community_owner is None:
            if responder.uaccess.owns_community(self.community):
                ...
                self.save()
                message = (
                    "Request to connect group '{}' to community '{}' declined.".format(
                        self.group.name, self.community.name
                    )
                )
                return message, True
                ...
                message = (
                    "You do not own the community and cannot decline this request."
                )
                return message, False
        else:  # if self.group_owner is None:
            if responder.uaccess.owns_group(self.group):
    ...
    def remove(cls, requester, **kwargs):
        ...
        * success: whether the removal succeeded.
        ...
        if not requester.owns_community(community):
            message = "User {} does not own community '{}'".format(
                requester.username, community.name
            )
            return message, False

        # delete request from provenance chain
        try:
            request = GroupCommunityRequest.get_request(
        ...
        if group not in community.member_groups:
            message = "Group '{}' is not in community '{}'.".format(
                group.name, community.name
            )
            return message, True

        requester.uaccess_unshare_community_with_group(community, group)
        message = "Group '{}' removed from community '{}'.".format(
            group.name, community.name
        )
        return message, True
