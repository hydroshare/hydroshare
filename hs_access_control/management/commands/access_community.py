...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--owner", dest="owner", help="owner of community (does not affect quota)"
        )

        parser.add_argument(
            "--description", dest="description", help="description of community"
        )

        parser.add_argument("--purpose", dest="purpose", help="purpose of community")
    ...
    def handle(self, *args, **options):
        ...
        if command is None or command == "list":
            ...
            print("  owners:")
            for ucp in UserCommunityPrivilege.objects.filter(
                community=community, privilege=PrivilegeCodes.OWNER
            ):
                print(
                    "    {} (grantor {})".format(
            ...
            for gcp in GroupCommunityPrivilege.objects.filter(community=community):
                ...
                print(
                    "     '{}' (id={}) (grantor={}):".format(
                        gcp.group.name, gcp.group.id, gcp.grantor.username
                    )
                )
                print("         {}.".format(others))
                print(
                    "         '{}' (id={}) owners are:".format(
                        gcp.group.name, str(gcp.group.id)
                    )
                )
                for ugp in UserGroupPrivilege.objects.filter(
                    group=gcp.group, privilege=PrivilegeCodes.OWNER
                ):
                    print("             {}".format(ugp.user.username))
            print("  invitations and requests:")
            for gcr in GroupCommunityRequest.objects.filter(
                community=community, redeemed=False
            ):
                if gcr.group_owner is None:
                    print(
                        "     '{}' (id={}) invited (by community owner={}):".format(
                            gcr.group.name, gcr.group.id, gcr.community_owner.username
                        )
                    )
                else:
                    print(
                        "     '{}' (id={}) requested membership (by group owner={}):".format(
                            gcr.group.name, gcr.group.id, gcr.group_owner.username
                        )
                    )
            exit(0)
        ...
        if command == "update" or command == "create":
            ...
            if community is not None:
                ...
                print(
                    "creating community '{}' with owner '{}' and description '{}'".format(
                        cname, owner, description
                    )
                )
            ...
            if community is None:
                ...
                exit(1)
            print(
                "removing community '{}' (id={})".format(community.name, community.id)
            )
            community.delete()
            ...
            print(
                "renaming community '{}' (id={}) to '{}'".format(
                    community.name, community.id, nname
                )
            )
            ...
            if len(options["command"]) < 4:
                print(
                    "user {} owns community '{}' (id={})".format(
                        owner.username, community.name, str(community.id)
                    )
                )
            ...
            if action == "add":
                ...
                UserCommunityPrivilege.unshare(
                    user=owner, community=community, grantor=owner
                )
            ...
            if len(options["command"]) < 3:
                ...
                for gcp in GroupCommunityPrivilege.objects.filter(community=community):
                    ...
                    print(
                        "    '{}' (grantor {}):".format(
                            gcp.group.name, gcp.grantor.username
                        )
                    )
                    print("         {}.".format(others))
                exit(0)
            ...
            if action == "update" or action == "add":
                ...
                try:
                    print(
                        "Updating group '{}' (id={}) status in community '{}' (id={}).".format(
                            gname, str(group.id), cname, str(community.id)
                        )
                    )
                    gcp = GroupCommunityPrivilege.objects.get(
                    ...
                    if gcp.privilege != privilege or owner != gcp.grantor:
                        ...
                        print(
                            "Group '{}' is already a member of community '{}'".format(
                                gname, cname
                            )
                        )
                    ...
                    print(
                        "Adding group '{}' (id={}) to community '{}' (id={})".format(
                            gname, str(group.id), cname, str(community.id)
                        )
                    )
                ...
                try:
                    print(
                        "Inviting group '{}' (id={}) to community '{}' (id={}).".format(
                            gname, str(group.id), cname, str(community.id)
                        )
                    )
                    gcp = GroupCommunityPrivilege.objects.get(
                ...
                print(
                    "Requesting that group '{}' (id={}) join community '{}' (id={}).".format(
                        gname, str(group.id), cname, str(community.id)
                    )
                )
                try:
                ...
                try:
                    ...
                    print(
                        "GroupCommunityRequest for community '{}' and group '{}' does not exist.".format(
                            cname, gname
                        )
                    )
                ...
                if gcr.redeemed:
                    print(
                        "request connecting '{}' and '{}' is already redeemed.".format(
                    ...
                    community_owner = community.first_owner
                    print(
                        "owner '{}' of community '{}' approves request from group '{}'".format(
                            community_owner.username, cname, gname
                        )
                    )
                    ...
                    group_owner = group.gaccess.first_owner
                    print(
                        "owner '{}' of group '{}' approves invitation from community '{}'".format(
                            group_owner.username, gname, cname
                        )
                    )
                ...
                if gcr.redeemed:
                    ...
                    community_owner = community.first_owner
                    print(
                        "owner '{}' of community '{}' declines request from group '{}'".format(
                            community_owner.username, cname, gname
                        )
                    )
                    ...
                    group_owner = group.gaccess.first_owner
                    print(
                        "owner '{}' of group '{}' declines invitation from community '{}'".format(
                            group_owner.username, gname, cname
                        )
                    )
                ...
                print(
                    "removing group '{}' (id={}) from community '{}' (id={})".format(
                        group.name, str(group.id), community.name, str(community.id)
                    )
                )
                GroupCommunityPrivilege.unshare(
                    group=group, community=community, grantor=owner
                )
