...
def populate_provenance(apps, schema_editor):
    ...
    GroupResourcePrivilege = apps.get_model(
        "hs_access_control", "GroupResourcePrivilege"
    )
    UserResourceProvenance = apps.get_model(
        "hs_access_control", "UserResourceProvenance"
    )
    UserGroupProvenance = apps.get_model("hs_access_control", "UserGroupProvenance")
    GroupResourceProvenance = apps.get_model(
        "hs_access_control", "GroupResourceProvenance"
    )

    # brute force addition of provenance records
    for r in GroupResourcePrivilege.objects.all():
        GroupResourceProvenance.objects.create(
            group=r.group,
            resource=r.resource,
            start=r.start,
    ...
    for r in UserResourcePrivilege.objects.all():
        UserResourceProvenance.objects.create(
            user=r.user,
            resource=r.resource,
            start=r.start,
            privilege=r.privilege,
    ...
    for r in UserGroupPrivilege.objects.all():
        UserGroupProvenance.objects.create(
            user=r.user,
            group=r.group,
            start=r.start,
            privilege=r.privilege,
