...
class T11ExplicitGet(MockIRODSTestCaseMixin, TestCase):
    def setUp(self):
        ...
        self.A_group = self.A_user.uaccess.create_group(
            title="Test Group A", description="This group is all about testing"
        )

        self.B_group = self.B_user.uaccess.create_group(
            title="Test Group B", description="This group is all about testing"
        )

        self.C_group = self.C_user.uaccess.create_group(
            title="Test Group C", description="This group is all about testing"
        )
    ...
    def test_01_user_level_access(self):
        ...
        self.assertTrue(
            is_equal_to_as_set(foo, [r1_resource, r2_resource, r3_resource])
        )
    ...
    def test_02_group_level_access(self):
        ...
        # A owns everything
        g = A_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.OWNER, via_user=True, via_group=False
        )
        self.assertTrue(is_equal_to_as_set(g, [r1_resource, r2_resource, r3_resource]))

        g = B_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.CHANGE, via_user=False, via_group=True
        )
        self.assertTrue(is_equal_to_as_set(g, []))

        A_user.uaccess.share_resource_with_group(
            r1_resource, A_group, PrivilegeCodes.CHANGE
        )
        ...
        g = B_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.CHANGE, via_user=True, via_group=True
        )
        self.assertTrue(is_equal_to_as_set(g, [r1_resource]))
        ...
        B_user.uaccess.share_resource_with_group(
            r1_resource, B_group, PrivilegeCodes.CHANGE
        )
        A_user.uaccess.share_resource_with_user(
            r1_resource, B_user, PrivilegeCodes.OWNER
        )
        g = B_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.OWNER, via_user=True, via_group=True
        )
        # should be OWNER now
        self.assertTrue(is_equal_to_as_set(g, [r1_resource]))
        ...
        # OWNER squashes VIEW
        g = B_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.VIEW, via_user=True, via_group=True
        )
        self.assertTrue(is_equal_to_as_set(g, []))
    ...
    def test_03_immutability(self):
        ...
        self.assertTrue(is_equal_to_as_set(g, []))
        g = B_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.CHANGE, via_user=True, via_group=False
        )
        self.assertTrue(is_equal_to_as_set(g, [r1_resource]))
        g = B_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.VIEW, via_user=True, via_group=False
        )
        self.assertTrue(is_equal_to_as_set(g, []))

        # C_user's CHANGE should not be present for user
        g = C_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.OWNER, via_user=True, via_group=False
        )
        self.assertTrue(is_equal_to_as_set(g, []))
        g = C_user.uaccess.get_resources_with_explicit_access(
        ...
        g = C_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.OWNER, via_user=False, via_group=True
        )
        self.assertTrue(is_equal_to_as_set(g, []))
        g = C_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.CHANGE, via_user=False, via_group=True
        ...
        self.assertTrue(is_equal_to_as_set(g, [r1_resource]))
        g = C_user.uaccess.get_resources_with_explicit_access(
            PrivilegeCodes.VIEW, via_user=False, via_group=True
        )
        self.assertTrue(is_equal_to_as_set(g, []))
