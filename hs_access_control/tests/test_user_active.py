...
class T12UserActive(MockIRODSTestCaseMixin, TestCase):
    ...
    def test_01_reporting(self):
        ...
        self.assertTrue(
            is_equal_to_as_set(cat.uaccess.get_group_unshare_users(felines), [cat, dog])
        )
        self.assertTrue(
        ...
        self.assertTrue(
            is_equal_to_as_set(cat.uaccess.get_group_unshare_users(felines), [])
        )
        self.assertTrue(
            is_equal_to_as_set(cat.uaccess.get_resource_unshare_users(scratching), [])
        )
