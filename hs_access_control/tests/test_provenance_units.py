...
class UnitTests(MockIRODSTestCaseMixin, TestCase):
    ...
    def test_usergroupprivilege_get_undo_users(self):
        ...
        self.assertTrue(
            is_equal_to_as_set(
                UserGroupProvenance.get_undo_users(group=bikers, grantor=george), [alva]
            )
        )
    ...
    def test_usergroupprivilege_undo_share(self):
        ...
        UserGroupProvenance.update(
            group=bikers, user=alva, privilege=PrivilegeCodes.NONE, grantor=george
        )
        self.assertEqual(
            UserGroupProvenance.get_privilege(group=bikers, user=alva),
            PrivilegeCodes.NONE,
    ...
    def test_userresourceprivilege_get_undo_users(self):
        ...
        self.assertTrue(
            is_equal_to_as_set(
                UserResourceProvenance.get_undo_users(resource=bikes, grantor=george),
                [alva],
            )
        )
    ...
    def test_userresourceprivilege_undo_share(self):
        ...
        UserResourceProvenance.update(
            resource=bikes, user=alva, privilege=PrivilegeCodes.NONE, grantor=george
        )
        self.assertEqual(
            UserResourceProvenance.get_privilege(resource=bikes, user=alva),
            PrivilegeCodes.NONE,
    ...
    def test_groupresourceprivilege_get_undo_groups(self):
        ...
        self.assertTrue(
            is_equal_to_as_set(
                GroupResourceProvenance.get_undo_groups(resource=bikes, grantor=george),
                [bikers],
            )
        )
    ...
    def test_groupresourceprivilege_undo_share(self):
        ...
        GroupResourceProvenance.update(
            resource=bikes, group=bikers, privilege=PrivilegeCodes.NONE, grantor=george
        )
        self.assertEqual(
            GroupResourceProvenance.get_privilege(resource=bikes, group=bikers),
            PrivilegeCodes.NONE,
        ...
        with self.assertRaises(PermissionDenied):
            GroupResourceProvenance.undo_share(
                resource=bikes, group=bikers, grantor=george
            )
        with self.assertRaises(PermissionDenied):
            GroupResourceProvenance.undo_share(
                resource=bikes, group=bikers, grantor=george
    ...
    def test_can_undo_share_resource_with_user(self):
        ...
        self.assertFalse(
            george.uaccess.can_undo_share_resource_with_user(bikes, george)
        )
    ...
    def test_can_undo_share_resource_with_group(self):
        ...
        self.assertTrue(
            GroupResourceProvenance.get_current_record(
                resource=bikes, group=bikers
            ).undone
        )
