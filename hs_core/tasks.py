...
from hs_core.hydroshare.hs_bagit import (
    create_bag_metadata_files,
    create_bag,
...
from hs_core.hydroshare.resource import (
    get_activated_doi,
    get_crossref_url,
    deposit_res_metadata_with_crossref,
    get_resource_doi,
)
...
FILE_TYPE_MAP = {
    "GenericLogicalFile": GenericLogicalFile,
...
def setup_periodic_tasks(sender, **kwargs):
    if hasattr(settings, "DISABLE_PERIODIC_TASKS") and settings.DISABLE_PERIODIC_TASKS:
        ...
        sender.add_periodic_task(
            crontab(minute=30, hour=22), nightly_metadata_review_reminder.s()
        )
        sender.add_periodic_task(crontab(minute=45), manage_task_hourly.s())
        sender.add_periodic_task(
            crontab(minute=15, hour=0, day_of_week=1, day_of_month="1-7"),
            send_over_quota_emails.s(),
        )
        sender.add_periodic_task(crontab(minute=00, hour=12), daily_odm2_sync.s())
        sender.add_periodic_task(
            crontab(day_of_month=1), monthly_group_membership_requests_cleanup.s()
        )
        sender.add_periodic_task(
            crontab(minute=30, hour=0), daily_innactive_group_requests_cleanup.s()
        )
...
def nightly_zips_cleanup():
    ...
    federated_prefixes = (
        BaseResource.objects.all().values_list("resource_federation_path").distinct()
    )

    for p in federated_prefixes:
        prefix = p[0]  # strip tuple
        if prefix != "":
            zips_daily_date = "{prefix}/zips/{daily_date}".format(
                prefix=prefix, daily_date=date_folder
            )
            if __debug__:
                logger.debug("cleaning up {}".format(zips_daily_date))
...
def manage_task_hourly():
    ...
    failed_resources = BaseResource.objects.filter(
        raccess__published=True, doi__contains="failure"
    )
    for res in failed_resources:
        if res.metadata.dates.all().filter(type="published"):
            ...
            if response.status_code == status.HTTP_200_OK:
                ...
                msg_lst.append(
                    "Metadata deposition with CrossRef for the published resource "
                    "DOI {res_doi} failed again after retry with first metadata "
                    "deposition requested since {pub_date}.".format(
                        res_doi=act_doi, pub_date=pub_date
                    )
                )
                logger.debug(response.content)
        else:
            msg_lst.append(
                "{res_id} does not have published date in its metadata.".format(
                    res_id=res.short_id
                )
            )

    pending_resources = BaseResource.objects.filter(
        raccess__published=True, doi__contains="pending"
    )
    for res in pending_resources:
        if res.metadata.dates.all().filter(type="published"):
            ...
            req_str = (
                "{MAIN_URL}servlet/submissionDownload?usr={USERNAME}&pwd="
            ...
            if not success:
                msg_lst.append(
                    "Published resource DOI {res_doi} is not yet activated with request "
                    "data deposited since {pub_date}.".format(
                        res_doi=act_doi, pub_date=pub_date
                    )
                )
                logger.debug(response.content)
    ...
    if msg_lst and not settings.DISABLE_TASK_EMAILS:
        email_msg = "\n".join(msg_lst)
        subject = (
            "Notification of pending DOI deposition/activation of published resources"
        )
    ...
    for res in pending_resources:
        ...
        if review_date:
            ...
            if review_date < cutoff_date:
                ...
                subject = (
                    f"Metadata review pending since "
...
def notify_owners_of_publication_success(resource):
    ...
    if not settings.DISABLE_TASK_EMAILS:
        send_mail(
            subject="HydroShare resource metadata review completed",
            message=email_msg,
...
def send_over_quota_emails():
    ...
    for u in users:
        uq = UserQuota.objects.filter(
            user__username=u.username, zone=hs_internal_zone
        ).first()
        if uq:
            ...
            if used_percent >= qmsg.soft_limit_percent:
                ...
                replace_substr = (
                    "<a href='mailto:{0}?subject=Request more quota'>{0}</a>".format(
                        settings.DEFAULT_SUPPORT_EMAIL
                    )
                )
                ...
                if settings.DEBUG or settings.DISABLE_TASK_EMAILS:
                    logger.info(
                        "quota warning email not sent out on debug server but logged instead: "
                        "{}".format(msg_str)
                    )
                else:
                    try:
                        ...
                        logger.debug(
                            "Failed to send quota warning email: " + ex.message
                        )
            else:
                if uq.remaining_grace_period >= 0:
            ...
            logger.debug(
                "user " + u.username + " does not have UserQuota foreign key relation"
            )
...
def add_zip_file_contents_to_resource(pk, zip_file_path):
    ...
    try:
        ...
        for i, f in enumerate(files):
            ...
            resource.file_unpack_message = (
                "Imported {0} of about {1} file(s) ...".format(i, num_files)
            )
            resource.save()
...
def create_temp_zip(
    ...
    if aggregation or sf_zip:
        ...
        temp_folder_name, ext = os.path.splitext(
            output_path
        )  # strip zip to get scratch dir
        head, tail = os.path.split(
            temp_folder_name
        )  # tail is unqualified folder name "foo"
        out_with_folder = os.path.join(
            temp_folder_name, tail
        )  # foo/foo is subdir to zip
        ...
        if not out_with_folder.endswith(aggr_filename):
            out_with_folder = os.path.join(
                os.path.dirname(out_with_folder), aggr_filename
            )
        ...
        if aggregation:
            ...
            if aggregation.is_model_program or aggregation.is_model_instance:
                ...
                if aggregation.is_model_instance:
                    try:
                        istorage.copyFiles(
                            aggregation.schema_values_file_path, temp_folder_name
                        )
                    except SessionException:
                        logger.error(
                            "cannot copy {}".format(aggregation.schema_values_file_path)
                        )
...
def copy_resource_task(ori_res_id, new_res_id=None, request_username=None):
    try:
        ...
        if (
            new_res.metadata.relations.all()
            .filter(type=RelationTypes.isVersionOf)
            .exists()
        ):
            ...
            new_res.metadata.relations.all().filter(
                type=RelationTypes.isVersionOf
            ).first().delete()
        ...
        if (
            not new_res.metadata.relations.all()
            .filter(type=RelationTypes.source, value=derived_from)
            .exists()
        ):
            new_res.metadata.create_element(
                "relation", type=RelationTypes.source, value=derived_from
            )
...
def create_new_version_resource_task(ori_res_id, username, new_res_id=None):
    ...
    try:
        ...
        ori_res.metadata.create_element(
            "relation", type=RelationTypes.isReplacedBy, value=new_res.get_citation()
        )
        ...
        new_res.metadata.create_element(
            "relation", type=RelationTypes.isVersionOf, value=ori_res.get_citation()
        )
...
def replicate_resource_bag_to_user_zone_task(res_id, request_username):
    ...
    if istorage.exists(res_coll):
        ...
        raise ValidationError(
            "Resource {} does not exist in iRODS".format(res.short_id)
        )
...
def delete_resource_task(resource_id, request_username=None):
    ...
    if res.metadata.relations.all().filter(type=RelationTypes.isVersionOf).exists():
        ...
        if (
            obsolete_res.metadata.relations.all()
    ...
    for res_in_col in res.resources.all():
        ...
        # collection
        if res_in_col.metadata.relations.filter(
            type="isPartOf", value__endswith=res.short_id
        ).exists():
            res_in_col.metadata.relations.filter(
                type="isPartOf", value__endswith=res.short_id
            ).delete()
    ...
    for collection_res in resource_related_collections:
        ...
        collection_res.metadata.relations.filter(
            type="hasPart", value__endswith=res.short_id
        ).delete()
...
def update_web_services(services_url, api_token, timeout, publish_urls, res_id):
    ...
    session.headers.update({"Authorization": " ".join(("Token", str(api_token)))})
        ...
        if publish_urls and response.status_code == status.HTTP_201_CREATED:
            try:
                ...
                for url in response_content["content"]:
                    ...
                    lf = logical_files[
                        [i.aggregation_name for i in logical_files].index(
...
def move_aggregation_task(res_id, file_type_id, file_type, tgt_path):
    ...
    for file in res_files:
        tgt_full_path = os.path.join(
            res.file_path, tgt_path, os.path.basename(file.storage_path)
        )
    ...
    new_aggregation_name = os.path.join(
        tgt_path, os.path.basename(orig_aggregation_name)
    )
    res.set_flag_to_recreate_aggregation_meta_files(
        orig_path=orig_aggregation_name, new_path=new_aggregation_name
    )
...
def daily_innactive_group_requests_cleanup():
    ...
    """
    GroupMembershipRequest.objects.filter(request_from__is_active=False).update(
        redeemed=True
    )
    GroupMembershipRequest.objects.filter(invitation_to__is_active=False).update(
        redeemed=True
    )
