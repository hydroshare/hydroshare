...
class GenericResourceMeta(object):
    ...
    def read_metadata_from_resource_bag(cls, bag_content_path, hydroshare_host='www.hydroshare.org'):
        ...
        To work with this factory, each xDCIShare resource type must implement
        ...
        content.
        :param hydroshare_host: Host name of the xDCIShare server.
        ...
        (root_uri, res_meta_path, res_meta) = cls._read_resource_map(bag_content_path,
                                                                     hydroshare_host)
        # Iterate over xDCIShare resource types
    ...
    def write_metadata_to_resource(self, resource,
        ...
        :param resource: xDCIShare resource instance
        ...
        if update_creators:
            for c in self.get_creators():
                if isinstance(c, GenericResourceMeta.ResourceCreator):
                    ...
                    if c.rel_uri:
                        # xDCIShare user URIs are stored as relative not absolute URIs
                        kwargs['description'] = c.rel_uri
                    else:
                        kwargs['description'] = None

                    if self.owner_is_hs_user and c.order == 1:
                        # Use metadata from bag for owner if the owner is a xDCIShare user
    ...
    class ResourceContributor(object):
        ...
        def __init__(self):
            # xDCIShare user ID of user specified by self.url (set by self.set_uri)
            self.id = None
            # Relative version of self.uri (applies only to xDCIShare user URIs; set by self.set_uri)
            self.rel_uri = None
        ...
        def set_uri(self, uri):
            ...
            """
            # Make sure this is a xDCIShare user URI
            ...
            if is_hs_user_uri:
                ...
                # Separate out the user ID for xDCIShare users
    ...
    class ResourceCreator(ResourceContributor):
        ...
        Fields beyond URI are ignored for creators that are xDCIShare users (i.e. have valid URI).
        """
