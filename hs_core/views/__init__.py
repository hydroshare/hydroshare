...
from .utils import (authorize, upload_from_irods, ACTION_TO_AUTHORIZE,
                    run_script_to_update_hyrax_input_files, get_my_resources_list,
...
from . import resource_rest_api  # noqa
from . import user_rest_api  # noqa
...
def _get_resource_sender(element_name, resource):
    core_metadata_element_names = [el_name.lower()
                                   for el_name in CoreMetaData.get_supported_element_names()]
...
def update_metadata_element(request, shortkey, element_name, element_id, *args, **kwargs):
    ...
    handler_response = pre_metadata_element_update.send(
        sender=sender_resource, element_name=element_name,
...
def rep_res_bag_to_irods_user_zone(request, shortkey, *args, **kwargs):
    """
    This function needs to be called via AJAX. The function replicates resource bag to iRODS user
    zone on users.hydroshare.org which is federated with hydroshare zone under the iRODS user
    account corresponding to a HydroShare user. This function should only be called or exposed to
    be called from web interface when a corresponding iRODS user account on hydroshare user Zone
    exists. The purpose of this function is to allow HydroShare resource bag that a HydroShare
    user has access to be copied to HydroShare user's iRODS space in HydroShare user zone so that
    users can do analysis or computations on the resource
    ...
    try:
        ...
        return HttpResponse(
            json.dumps({"success": ("This resource bag zip file has been successfully"
                                    " replicated to your iRODS user zone.")}),
            content_type="application/json"
        )
...
def create_new_version_resource(request, shortkey, *args, **kwargs):
    ...
    if res.locked_time:
        ...
        if elapsed_time.days >= 0 or elapsed_time.seconds > settings.RESOURCE_LOCK_TIMEOUT_SECONDS:
            ...
            # cannot create new version for this resource since the resource is locked by
            # another user
            request.session['new_version_resource_creation_error'] = (
                "Failed to create a new version for this resource since another user is"
                " creating a new version for this resource synchronously.")
            return HttpResponseRedirect(res.get_absolute_url())

    new_resource = None
    try:
        # lock the resource to prevent concurrent new version creation since only one new version
        # for an obsoleted resource is allowed
...
def _share_resource(request, shortkey, privilege, user_or_group_id, user_or_group):
    ...
    :param user_or_group: indicates if the resource to be shared with a user or group. A
                          value of 'user' will share the resource with a user whose id is
                          provided with the parameter 'user_or_group_id'. Any other value
                          for this parameter assumes resource to be shared with a group.
    :return:
    """
...
def unshare_resource_with_user(request, shortkey, user_id, *args, **kwargs):
    ...
    try:
        if user.uaccess.can_unshare_resource_with_user(res, user_to_unshare_with):
            # requesting user is the resource owner or user is self unsharing (user is
...
def resend_verification_email(request):
    ...
    try:
        ...
        u.email_user(
            ...
            "This is an automated email from Hydroshare.org. If you requested a Hydroshare "
            "account, please go to http://{domain}/verify/{token}/ and verify your account.".format(
...
def add_generic_context(request, page):
    ...
    class AddGroupForm(forms.Form):
        group = forms.ModelChoiceField(
            Group.objects.filter(gaccess__active=True).exclude(name='Hydroshare Author').all(),
...
def create_resource(request, *args, **kwargs):
    ...
    try:
        page_url_dict, res_title, metadata, fed_res_path = (
            hydroshare.utils.resource_pre_create_actions(resource_type=resource_type,
                                                         ...
                                                         requesting_user=request.user, **kwargs))
    except utils.ResourceFileSizeException as ex:
        context = {'file_size_error': ex.message}
        return render_to_response('pages/create-resource.html', context,
                                  context_instance=RequestContext(request))
    ...
    try:
        utils.resource_post_create_actions(
            resource=resource, user=request.user, metadata=metadata, **kwargs)
...
def make_group_membership_request(request, group_id, user_id=None, *args, **kwargs):
    ...
    :param user_id: needed only when an owner is inviting a user to join a group. This is the id
                    of the user the owner is inviting
    :return:
    """
    requesting_user = request.user
...
def group_membership(request, uidb36, token, membership_request_id, **kwargs):
    """
    View for the link in the verification email that was sent to a user when they request/invite
    to join a group.  User is logged in and the request to join a group is accepted. Then the user
    is redirected to the group profile page of the group for which the membership got accepted.
...
def act_on_group_membership_request(request, membership_request_id, action, *args, **kwargs):
    ...
    :param request: requesting user is either owner of the group taking action on a request from a
                    user or a user taking action on a invitation to join a group from a group owner
    :param membership_request_id: id of the membership request object (an instance of
                                  GroupMembershipRequest) to act on
...
def get_user_data(request, user_id, *args, **kwargs):
    ...
    if user.userprofile.state:
        address = user.userprofile.get_state_display()
    if user.userprofile.country:
        if len(address) > 0:
            address = '{}, {}'.format(address, user.userprofile.country.name)
        else:
        ...
        if user not in users_to_keep:
            try:
                if requesting_user.uaccess.can_unshare_resource_with_user(resource, user):
                    ...
                    # TODO from @alvacouch: This can raise a PermissionDenied exception without a
                    # guard such as user.uaccess.can_undo_share_resource_with_user(res,
                    # user_to_unshare_with)
...
def _set_resource_sharing_status(request, user, resource, flag_to_set, flag_value):
    ...
    if is_public and not can_resource_be_public_or_discoverable:
        messages.error(
            request,
            _get_message_for_setting_resource_flag(has_files, has_metadata,
                                                   resource_flag='public'))
    else:
        if is_discoverable:
        ...
        if (
            flag_to_set == 'public' and flag_value and settings.RUN_HYRAX_UPDATE and
...
def _get_message_for_setting_resource_flag(has_files, has_metadata, resource_flag):
    ...
    if not has_metadata and not has_files:
        msg = ("Resource does not have sufficient required metadata and content files to be"
               " {flag}".format(flag=resource_flag))
    elif not has_metadata:
        msg = "Resource does not have sufficient required metadata to be {flag}".format(
            flag=resource_flag)
    elif not has_files:
        msg = "Resource does not have required content files to be {flag}".format(
            flag=resource_flag)
