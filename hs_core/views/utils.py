...
def get_my_resources_list(user, annotate=False, filter=None, **kwargs):
    ...
    if annotate:
        ...
        resource_collection = resource_collection.only(
            "short_id", "resource_type", "created"
        )
...
def send_action_to_take_email(request, user, action_type, **kwargs):
    ...
    if action_type == "group_membership":
        ...
        reject_body = parse.quote(
            "Your Publication Request for the following resource was rejected: "
        )
...
def show_relations_section(res_obj):
    ...
    has_inspecific_count = res_obj.metadata.relations.filter(
        type=RelationTypes.relation
    ).count()
...
def zip_folder(user, res_id, input_coll_path, output_zip_fname, bool_remove_original):
    ...
    if not istorage.exists(res_coll_input):
        raise ValidationError(
            f"Specified folder path ({input_coll_path}) doesn't exist."
        )
    ...
    if not ResourceFile.is_filename_valid(output_zip_fname):
        ...
        err_msg = (
            f"Invalid zip filename ({output_zip_fname}). Filename can't have any of these "
...
def zip_by_aggregation_file(user, res_id, aggregation_name, output_zip_fname):
    ...
    if not istorage.exists(aggr_storage_path):
        raise ValidationError(
            f"Specified aggregation path ({aggregation_name}) was not found"
        )

    if not ResourceFile.is_filename_valid(output_zip_fname):
        filename_banned_chars = " ".join(ResourceFile.banned_symbols())
        err_msg = (
            f"Invalid zip filename ({output_zip_fname}). Filename can't have any of "
...
def unzip_file(
    ...
    try:
        ...
        unzip_path_temp = istorage.unzip(
            zip_with_full_path, unzipped_folder=unzip_folder_name
        )
        ...
        for file_in_zip in unzipped_files:
            ...
            if not ResourceFile.is_filename_valid(file_in_zip):
                log_msg = (
                    f"Failed to unzip. Zip file ({zip_with_full_path}) has file with name that contains "
        ...
        for folder_in_zip in unzipped_folder_paths:
            ...
            if not ResourceFile.is_folder_name_valid(folder_in_zip):
                log_msg = (
                    f"Failed to unzip. Zip file ({zip_with_full_path}) has folder with name that contains "
                    f"one or more prohibited characters."
                )
                logger.error(log_msg)
        ...
        if unzip_to_folder:
            ...
            unzip_folder = os.path.splitext(os.path.basename(zip_with_full_path))[
                0
            ].strip()
            unzip_to_folder_path = istorage.unzip(
                zip_with_full_path, unzipped_folder=unzip_folder
            )
            res_files = link_irods_folder_to_django(
                resource, istorage, unzip_to_folder_path, auto_aggregate
            )
            if resource.resource_type == "CompositeResource":
            ...
            for sub_dir_name in unzip_subdir_list:
                dest_sub_path = os.path.join(
                    os.path.dirname(zip_with_full_path), sub_dir_name
                )
            ...
            if not overwrite and override_tgt_paths:
                override_simplified_paths = [
                    tgt_path.split("data/contents/")[1]
                    for tgt_path in override_tgt_paths
                ]
                raise FileOverrideException(
                    "move would overwrite {}".format(
                        ", ".join(override_simplified_paths)
                    )
                )
                ...
                if istorage.exists(override_tgt_path):
                    if resource.resource_type == "CompositeResource":
                        ...
                        if aggregation_object:
                            ...
                            try:
                                f = ResourceFile.get(
                                    resource=resource, file=override_tgt_path
                                )
                                f.delete()
                            except ObjectDoesNotExist:
                                ...
                                files = ResourceFile.objects.filter(
                                    ...
                                    file_folder=override_tgt_path.rsplit(
                                        "data/contents/"
                                    )[1],
                                )
                                if files:
...
def ingest_bag(resource, bag_file, user):
    ...
    for unzipped_folder in unzipped_folders:
        ...
        if not ResourceFile.is_folder_name_valid(base_folder):
            ...
            log_msg = (
                f"Failed to ingest bag. Bag file ({zip_with_full_path}) has folder with name that contains "
                f"one or more prohibited characters."
            )
            logger.error(log_msg)
    ...
    for unzipped_file in unzipped_files:
        ...
        if not ResourceFile.is_filename_valid(base_file):
            istorage.delete(unzip_path)
            log_msg = (
                f"Failed to ingest bag. Bag file ({zip_with_full_path}) has file with name that contains "
                f"one or more prohibited characters."
            )
            logger.error(log_msg)
...
def create_folder(res_id, folder_path, migrating_resource=False):
    ...
    for folder in folders:
        if not ResourceFile.is_folder_name_valid(folder):
            ...
            err_msg = (
                f"Folder name ({folder}) contains one more prohibited characters. "
            )
    ...
    for src_path in src_paths:
        ...
        if resource.resource_type == "CompositeResource":
            resource.set_flag_to_recreate_aggregation_meta_files(
                orig_path=src_full_path, new_path=tgt_qual_path
            )
...
def _path_move_rename(user, res_id, src_path, tgt_path, validate_move_rename=True):
    ...
    if src_base_name != tgt_base_name:
        if istorage.isFile(src_full_path):
            ...
            if not ResourceFile.is_folder_name_valid(tgt_folder_name_new):
                ...
                err_msg = (
                    f"{err_msg}Prohibited characters are: {foldername_banned_chars}"
                )
                raise SuspiciousFileOperation(err_msg)
    ...
    if resource.resource_type == "CompositeResource":
        resource.set_flag_to_recreate_aggregation_meta_files(
            orig_path=src_full_path, new_path=tgt_full_path
        )
