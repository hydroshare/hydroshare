...
def data_store_structure(request):
    ...
    if res_id is None:
        ...
        return HttpResponse(
            "Bad request - resource id is not included",
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
    res_id = str(res_id).strip()
    try:
        resource, _, _ = authorize(
            request, res_id, needed_permission=ACTION_TO_AUTHORIZE.VIEW_RESOURCE
        )
    except NotFound:
        logger.error("resource {} not found".format(res_id))
        return HttpResponse(
            "Bad request - resource not found", status=status.HTTP_400_BAD_REQUEST
        )
    ...
    try:
        ...
        logger.error(
            "session exception querying store_path {} for {}".format(store_path, res_id)
        )
    ...
    for dname in store[0]:  # directories
        ...
        if resource.resource_type == "CompositeResource":
            ...
            aggregation_object = resource.get_folder_aggregation_object(
                dir_path, aggregations=res_aggregations
            )
            ...
            if aggregation_object is not None:
                folder_aggregation_type = (
                    aggregation_object.get_aggregation_class_name()
                )
                folder_aggregation_name = (
                    aggregation_object.get_aggregation_display_name()
                )
                folder_aggregation_id = aggregation_object.id
                folder_aggregation_appkey = (
                    aggregation_object.metadata.extra_metadata.get(_APPKEY, "")
                )
                ...
                can_set_model_instance = (
                    ModelInstanceLogicalFile.can_set_folder_to_aggregation(
                        resource=resource,
                        dir_path=dir_path,
                        aggregations=res_aggregations,
                    )
                )
                can_set_model_program = (
                    ModelProgramLogicalFile.can_set_folder_to_aggregation(
                        resource=resource,
                        dir_path=dir_path,
                        aggregations=res_aggregations,
                    )
                ...
                elif FileSetLogicalFile.can_set_folder_to_aggregation(
    ...
    for index, fname in enumerate(store[1]):  # files
        ...
        if is_federated:
            f = ResourceFile.objects.filter(
                object_id=resource.id, fed_resource_file=file_in_irods
            ).first()
        else:
            f = ResourceFile.objects.filter(
                object_id=resource.id, resource_file=file_in_irods
            ).first()
        ...
        if f.has_logical_file:
            ...
            if f.extension and main_extension.endswith(f.extension):
                if (
                    not hasattr(f.logical_file, "folder")
                    or f.logical_file.folder is None
                ):
                    aggregation_appkey = f.logical_file.metadata.extra_metadata.get(
                        _APPKEY, ""
                    )
                ...
                aggregations.append(
                    {
                        ...
                        "preview_data_url": f.logical_file.metadata.get_preview_data_url(
                            resource=resource, folder_path=f_store_path
                        ),
            ...
            if not hasattr(logical_file, "folder") or logical_file.folder is None:
                aggregation_appkey = logical_file.metadata.extra_metadata.get(
                    _APPKEY, ""
                )
...
def data_store_folder_zip(request, res_id=None):
    ...
    if res_id is None:
        return JsonResponse(
            {"error": "Resource id was not provided"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    res_id = str(res_id).strip()
    try:
        resource, _, user = authorize(
            request, res_id, needed_permission=ACTION_TO_AUTHORIZE.EDIT_RESOURCE
        )
    except NotFound:
        return JsonResponse(
            {"error": "Resource was not found"}, status=status.HTTP_400_BAD_REQUEST
        )
    except PermissionDenied:
        return JsonResponse(
    ...
    if output_zip_fname is None:
        return JsonResponse(
            {"error": "Output zip file name was not provided"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    output_zip_fname = str(output_zip_fname).strip()
    if not output_zip_fname:
        return JsonResponse(
            {"error": "Output zip file name can't be be empty string"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    try:
        output_zip_fname, size = zip_folder(
            user, res_id, input_coll_path, output_zip_fname, bool_remove_original
        )
    except SessionException as ex:
        return JsonResponse(
...
def zip_aggregation_file(request, res_id=None):
    ...
    if resource.resource_type != "CompositeResource":
        err_msg = (
            f"{resource.display_name} type doesn't support zipping of aggregation."
        )
    ...
    if output_zip_fname is None:
        return JsonResponse(
            {"error": "Output zip filename was not provided"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    output_zip_fname = str(output_zip_fname).strip()
    if not output_zip_fname:
        return JsonResponse(
            {"error": "Output zip filename can't be empty"},
            status=status.HTTP_400_BAD_REQUEST,
        )

    try:
        output_zip_fname, size = zip_by_aggregation_file(
            user, res_id, aggregation_path, output_zip_fname
        )
    except SessionException as ex:
        return JsonResponse(
            {"error": ex.stderr}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
...
rid = openapi.Parameter(
    "id", openapi.IN_PATH, description="id of the resource", type=openapi.TYPE_STRING
)
body = openapi.Schema(
    type=openapi.TYPE_OBJECT,
    properties={
        "input_coll_path": openapi.Schema(
            type=openapi.TYPE_STRING,
            description="the relative path under res_id/data/ \
            contents to be zipped",
        ),
        "output_zip_file_name": openapi.Schema(
            type=openapi.TYPE_STRING,
            description="the file name only with no path of \
            the generated zip file name",
        ),
        "remove_original_after_zip": openapi.Schema(
            type=openapi.TYPE_BOOLEAN,
            description="whether original files will \
...
body = openapi.Schema(
    type=openapi.TYPE_OBJECT,
    properties={
        "aggregation_path": openapi.Schema(
            type=openapi.TYPE_STRING,
            description="the relative path under res_id/data/ \
            contents to be zipped",
        ),
...
@swagger_auto_schema(
    method="post",
    operation_description="Zip requested aggregation into a zip file in hydroshareZone \
...
def data_store_folder_unzip(request, **kwargs):
    ...
    zip_with_rel_path = request.POST.get(
        "zip_with_rel_path", kwargs.get("zip_with_rel_path")
    )
    ...
    overwrite = (
        request.POST.get("overwrite", "false").lower() == "true"
    )  # False by default
    auto_aggregate = (
        request.POST.get("auto_aggregate", "true").lower() == "true"
    )  # True by default
    ingest_metadata = (
        request.POST.get("ingest_metadata", "false").lower() == "true"
    )  # False by default
    remove_original_zip = (
        request.POST.get("remove_original_zip", "true").lower() == "true"
    )
    ...
    if request.is_ajax():
        ...
        try:
            ...
            specific_msg = (
                "iRODS error resulted in unzip being cancelled. This may be due to "
            ...
            return JsonResponse(
                {"error": err_msg}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
@swagger_auto_schema(
    method="post",
    operation_description="Ingests metadata files",
...
@swagger_auto_schema(
    method="post",
    operation_description="Create reference URL file",
    responses={200: "JsonResponse with status code and message"},
    request_body=body,
)
@api_view(["POST"])
...
def data_store_add_reference(request):
    ...
    ret_status, msg, file_id = add_reference_url_to_resource(
        request.user,
        res_id,
        ref_url,
        ref_name,
...
body = openapi.Schema(
    ...
    properties={
        "res_id": openapi.Schema(type=openapi.TYPE_STRING, description="res_id"),
        "curr_path": openapi.Schema(type=openapi.TYPE_STRING, description="curr_path"),
        "url_filename": openapi.Schema(
            type=openapi.TYPE_STRING, description="url_filename"
        ),
        "new_ref_url": openapi.Schema(
            type=openapi.TYPE_STRING, description="new_ref_url"
        ),
    },
)
...
@swagger_auto_schema(
    method="post",
    operation_description="Edit the referenced url in a url file",
    responses={
        200: "JsonResponse on success",
        400: "HttpResponse with error status code on error",
    },
    request_body=body,
)
@api_view(["POST"])
...
def data_store_edit_reference_url(request):
    ...
    if not url_filename:
        return HttpResponseBadRequest(
            "Must have url_filename included in the POST data"
        )
    if not new_ref_url:
...
body = openapi.Schema(
    type=openapi.TYPE_OBJECT,
    properties={
        "source_path": openapi.Schema(
            type=openapi.TYPE_STRING,
            description="path (relative to path \
            res_id/data/contents) for source file or folder under id collection",
        ),
        "target_path": openapi.Schema(
            type=openapi.TYPE_STRING,
            description="path (relative to path \
...
def data_store_move_to_folder(request, pk=None):
    ...
    if not irods_path_is_directory(istorage, tgt_storage_path):
        return HttpResponse(
            "Target of move is not an existing folder",
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    for src_path in src_paths:
        ...
        if not irods_path_is_directory(
            istorage, src_storage_path
        ):  # there is django record
            try:
    ...
    if override_tgt_paths:
        ...
        for override_tgt_path in override_tgt_paths:
            override_storage_tgt_path = os.path.join(
                resource.root_path, "data", "contents", override_tgt_path
            )
            if irods_path_is_directory(istorage, override_storage_tgt_path):
                # folder rather than a data object, just delete the folder
                remove_folder(
                    user, pk, os.path.join("data", "contents", override_tgt_path)
                )
            else:
                # data object or file
...
def data_store_rename_file_or_folder(request, pk=None):
    ...
    if src_folder != tgt_folder:
        return JsonResponse(
            {"error": "Source and target names must be in same folder"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    istorage = resource.get_irods_storage()
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Object to be renamed does not exist"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if not irods_path_is_directory(istorage, src_storage_path):
        try:  # Django record should exist for each file
            ...
            return JsonResponse(
                {"error": "Path to be renamed does not exist"},
                status=status.HTTP_400_BAD_REQUEST,
            )
