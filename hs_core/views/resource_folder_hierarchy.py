...
def data_store_structure(request):
    ...
    in hydroshareZone or any federated zone used for xDCIShare resource backend store.
...
def data_store_folder_zip(request, res_id=None):
    ...
    used for xDCIShare resource backend store. It is invoked by an AJAX call and returns
...
def data_store_folder_unzip(request, **kwargs):
    ...
    any federated zone used for xDCIShare resource backend store. It is invoked by an AJAX call,
