...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        count_disc_solr = 0
        discoverable = BaseResource.objects.filter(
            raccess__discoverable=True, raccess__public=False
        )
        ...
        disc_solr = (
            SearchQuerySet()
            .filter(availability="discoverable")
            .exclude(availability="public")
        )
        count_disc_solr = disc_solr.count()

        print(
            "discoverable shown={}, hidden={} solr={}".format(
                count_disc_include, count_disc_exclude, count_disc_solr
            )
        )
        ...
        count_public_solr = 0
        public = BaseResource.objects.filter(
            raccess__public=True, raccess__published=False
        )
        for d in public:
            if d.show_in_discover:
        ...
        disc_solr = (
            ...
            .exclude(availability="published")
        )
        count_public_solr = disc_solr.count()

        print(
            "public shown={}, hidden={} solr={}".format(
                count_public_include, count_public_exclude, count_public_solr
            )
        )
        ...
        print(
            "published shown={}, hidden={} solr={}".format(
                count_published_include, count_published_exclude, count_published_solr
            )
        )
