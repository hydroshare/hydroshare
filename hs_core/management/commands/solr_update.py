        ...
        if len(options["resource_ids"]) > 0:  # an array of resource short_id to check.
            ...
            for r in dqs:
                if not options["debug"]:
                    try:
                        ...
                        print(
                            "resource {} no longer found in Django.".format(r.short_id)
                        )
                        continue
                    except Exception as e:
                        print(
                            "resource {} generated exception {}".format(
                                r.short_id, str(e)
                            )
                        )
                else:
                ...
                if resource.show_in_discover and r.short_id not in found_in_solr:
                    print(
                        "{} {} NOT FOUND in SOLR: adding to index".format(
                ...
                # update everything to be safe.
                    ...
                    if r.show_in_discover:
                        print(
                            "{} {}: refreshing index (forced)".format(
                                r.short_id, resource.discovery_content_type
                            )
                        )
                        if not options["debug"]:
            ...
            for r in sqs:
                try:
                    ...
                    print(
                        "SOLR resource {} NOT FOUND in Django; cannot remove from SOLR".format(
                            r.short_id
                        )
                    )
                    solr_deleted += 1
            ...
            print(
                "Django contains {} discoverable resources and {} replaced resources".format(
                    django_indexed, django_replaced
                )
            )
            print("{} resources in Django refreshed in SOLR".format(django_refreshed))
            print(
                "SOLR contains {} discoverable resources and {} replaced resources".format(
