...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--owned_by", dest="owned_by", help="prior owner of the resources"
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["owned_by"] is not None:
            ...
            for res in BaseResource.objects.filter(
                r2urp__user=prior, r2urp__privilege=PrivilegeCodes.OWNER
            ):
                with transaction.atomic():
                    ...
                    print(
                        "added owner {} to {}".format(
                            options["new_owner"], resource.short_id
                        )
                    )
                    if options["set_quota_holder"]:
                        set_quota_holder(resource, user)
                        print(
                            "set quota holder to {} for {}".format(
                                options["new_owner"], resource.short_id
                            )
                        )
