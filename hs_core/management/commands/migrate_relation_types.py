        ...
        for res in BaseResource.objects.all().iterator():
            ...
            for rel in res.metadata.relations.all():
                if rel.type in ("cites", "isHostedBy", "isDataFor", "isCopiedFrom"):
                    ...
                    log_msg = log_msg.format(
                        old_type, rel.type, res.short_id, res.resource_type
                    )
                    print(log_msg)
                    log.info(log_msg)
                    type_migrated = True
            ...
            if hasattr(res.metadata, "sources"):
                for source in res.metadata.sources.all():
                    if not res.metadata.relations.filter(
                        type="source", value=source.derived_from
                    ).exists():
                        res.metadata.create_element(
                            "relation", type="source", value=source.derived_from
                        )
            ...
            for rel in res.metadata.relations.filter(
                ...
                if len(res_url_parts) == 2:
                    ...
                    try:
                        res_to_link = utils.get_resource_by_shortkey(
                            shortkey=res_id, or_404=False
                        )
                        ...
                        log_msg = (
                            "Resource doesn't exist for ID:{}. Failed to update relation type '{}' "
                    ...
                    log_msg = (
                        "Invalid value ({}) for relation type '{}' for resource with ID:{}. "
                        "Failed to update relation type '{}' value with citation for this resource"
                    )
                    log_msg = log_msg.format(
                        rel.value, rel.type, res.short_id, rel.type
                    )
                    print(log_msg)
                    log.warning(log_msg)
            ...
            if any(
                [
                    source_migrated,
                    type_migrated,
                    res.metadata.relations.filter(type="isVersionOf").exists(),
            ...
            if source_migrated:
                log_msg = (
                    "Source metadata element was migrated to relation metadata element for resource "
                    "with ID:{} and type:{}."
                )
            ...
            if res.resource_type == "CollectionResource":
                if res.resources.count() > 0:
                    ...
                    for res_in_collection in res.resources.all():
                        ...
                        res_in_collection.metadata.relations.filter(
                            type="isPartOf"
                        ).all().delete()

                        # add relation type 'isPartOf' to the resource that is part of the collection resource
                        res_in_collection.metadata.create_element(
                            "relation", type="isPartOf", value=res.get_citation()
                        )
                        log_msg = (
                            "Added 'isPartOf relation to resource with ID:{} and type:{} that is part of the"
                            "collection resource ID:{}"
                        )
                        log_msg = log_msg.format(
                            res_in_collection.short_id,
                            res_in_collection.resource_type,
                            res.short_id,
                        )
                        print(log_msg)
                        log.info(log_msg)
                    ...
                    set_dirty_bag_flag(res)
                    log_msg = (
                        "Updated relation metadata of type 'hasPart' to citation of the contained resource "
                        "for collection resource with ID:{} and type:{}."
                    )
                    log_msg = log_msg.format(res.short_id, res.resource_type)
                    print(log_msg)
                    log.info(log_msg)
        log_msg = "Total resources processed for migrating relation types:{}".format(
            res_count
        )
        print(log_msg)
        log.info(log_msg)
