...
class Command(BaseCommand):
    help = (
        "Output last modified date for resources in the input list with three columns, version_of_resource, "
        ...
        with open(input_file) as f:
            ...
            for res_id in res_list:
                ...
                writer.writerow(
                    [
                        res_id,
                        res.metadata.dates.all().filter(type="modified")[0].start_date,
                    ]
                )
            wf.close()
