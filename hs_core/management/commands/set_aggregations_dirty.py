...
class Command(BaseCommand):
    help = (
        "Sets metadata dirty for all aggregations of a specific aggregation type, or all aggregations of a "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            help=(
                "The existing id (short_id) of the resource. Required when aggr_type "
                "is not provided."
            ),
        )
        ...
        parser.add_argument(
            ...
            type=str,
            help=(
                "Name of the aggregation type for which metadata "
    ...
    def handle(self, *args, **options):
        valid_aggr_types = {
            GenericLogicalFile.type_name(): GenericLogicalFile,
        ...
        if options["resource_id"]:
            ...
            if res.resource_type != "CompositeResource":
                raise CommandError(
                    f"Specified resource (ID:{res_id}) is not a Composite Resource"
                )

        if not options["aggr_type"] and res is None:
            raise CommandError(
                "aggr_type argument is required when resource_id is not provided"
            )
        if res is not None and options["aggr_type"]:
            raise CommandError(
                "aggr_type can't be used when resource_id is specified. Use only one of these "
                "two arguments"
            )
        ...
        if res is None:
            ...
            for aggr in aggr_class.objects.all().iterator():
                ...
                log_msg = (
                    f"{aggr_count}. Metadata was set dirty for aggregation:{aggr_path}"
                )
                print(log_msg)
                aggr_count += 1
        else:
