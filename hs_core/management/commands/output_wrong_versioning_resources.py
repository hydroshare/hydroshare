...
class Command(BaseCommand):
    help = (
        "Output versioning chain violation resources (e.g., #4028) for fixing and delete those relations "
    ...
    def handle(self, *args, **options):
        ...
        for obj in replace_by_qs:
            ...
            if not BaseResource.objects.filter(short_id=replace_by_id).exists():
                obj.delete()
                msg = (
                    f"{replace_by_id} does not exist, so delete the corresponding isReplacedBy relation "
                    "metadata element"
                )
                print(msg, flush=True)
                continue
            ...
            if not res_obj:
                obj.delete()
                msg = (
                    f"resource with {obj.object_id} object_id does not exist, so delete the corresponding "
            ...
            if res_obj.short_id in replaced_by_dict:
                if replace_by_id in replaced_by_dict[res_obj.short_id]:
                    print(
                        f"{res_obj.short_id} is already replaced by {replace_by_id} - clean up duplicate entry",
                        flush=True,
                    )
                    obj.delete()
                else:
        ...
        for obj in version_of_qs:
            ...
            if not BaseResource.objects.filter(short_id=version_of_id).exists():
                obj.delete()
                msg = (
                    f"{version_of_id} does not exist, so delete the corresponding isVersionOf relation "
                    "metadata element"
                )
                print(msg, flush=True)
                continue
            ...
            if res_obj.short_id in version_of_dict:
                if version_of_id in version_of_dict[res_obj.short_id]:
                    print(
                        f"{res_obj.short_id} is already version of {version_of_id} - clean up duplicate entry"
                    )
                    obj.delete()
                else:
