...
class Command(BaseCommand):
    help = (
        "Fix versioning chain violation resources captured in an csv input file with three columns: "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "input_file",
            help="input csv file name with full path to be processed and "
    ...
    def handle(self, *args, **options):
        ...
        with open(input_file) as f:
            ...
            if (
                len(headers) < 3
                or headers[version_of_col].lower() != "isversionof"
                or headers[res_col].lower() != "resource"
            ...
            for row in reader:
                if not row[res_col]:
                    print(
                        "Resource column in the csv file cannot be empty, exit",
                        flush=True,
                    )
                    return
                fixed = False
                ...
                if not row[version_of_col] and not row[replace_by_col]:
                    ...
                    if res.metadata.relations.filter(
                        type=RelationTypes.isVersionOf
                    ).exists():
                        res.metadata.relations.filter(
                            type=RelationTypes.isVersionOf
                        ).delete()
                        res.save()
                    ...
                    if res_id == "4111306529e74503b090494ef1e808e2":
                        ...
                        try:
                            ...
                            print(
                                f"Resource {copy_of_res_id} does not exist, so isCopyOf relation cannot be added",
                                flush=True,
                            )
                            continue
                        hs_identifier = copy_of_res.metadata.identifiers.filter(
                            name="hydroShareIdentifier"
                        ).first()
                        if hs_identifier:
                            res.metadata.create_element(
                                "source", derived_from=hs_identifier.url
                            )
                            print(
                                f"{res_id} is made a copy of {copy_of_res_id}",
                                flush=True,
                            )
                            fixed = True
                    ...
                    try:
                        ...
                        print(
                            f"Resource {replace_by_res_id} does not exist", flush=True
                        )
                        continue
                    res.metadata.relations.filter(
                        type=RelationTypes.isReplacedBy
                ...
                if fixed:
                    count += 1
        print(
            f"versioning relationships for {count} resources have been fixed",
            flush=True,
        )
