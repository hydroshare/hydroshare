...
def check_displayed(resource, isreplacedby, replaces):
    ...
    """
    print(
        "resource {} (show={}, disc={}, created={}) '{}':".format(
            resource.short_id,
    ...
    if resource.short_id in replaces:
        ...
        for rid in repl:
            ...
            if r is not None:
                print(
                    "    replaces {} (show={}, disc={}, created={}) '{}'".format(
                        r.short_id,
                        r.show_in_discover,
    ...
    if resource.short_id in isreplacedby:
        repl = isreplacedby[
            resource.short_id
        ]  # things that are replaced by resource.short_id
        for rid in repl:
            try:
                r = get_resource_by_shortkey(rid, or_404=False)
            except BaseResource.DoesNotExist:
            ...
            if r is not None:
                print(
                    "    is replaced by {} (show={}, disc={}, created={}) '{}'".format(
                        r.short_id,
                        r.show_in_discover,
                        r.raccess.discoverable,
                        r.created.strftime("%Y-%m-%d %H:%M:%S"),
