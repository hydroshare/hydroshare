...
urlpatterns = [
    ...
    url(r'^_internal/metadata_review/(?P<shortkey>[0-9a-f-]+)/\
        (?P<action>[a-z]+)/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[-\w]+)/',
    ...
    url(r'^_internal/spam_allowlist/(?P<shortkey>[0-9a-f-]+)/(?P<action>[a-z]+)/',
