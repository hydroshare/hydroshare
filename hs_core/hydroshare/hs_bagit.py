...
class ResourceMeta(object):
    ...
    _res_meta_path = None
    _bag_content_path = None

    def __init__(self):
        pass

    def read_metadata_from_bag(self, bag_content_path):
        self._bag_content_path = bag_content_path
    ...
    def _read_resource_map(self):
        ...
        :raises: ResourceMetaException if metadata cannot be found or do not appear
         as expected.
        """
        rmap_path = os.path.join(self._bag_content_path, 'data', 'resourcemap.xml')
        ...
        for s,p,o in g.triples((None, None, None)):
            if s.endswith("resourcemap.xml") and p == rdflib.namespace.DC.identifier:
                self.id = o
        if self.id is None:
            raise ResourceMeta.ResourceMetaException("Unable to determine resource ID from resource map {0}".format(rmap_path))
    ...
    def _read_resource_metadata(self):
        ...
        :return: None
        """
        rmeta_path = os.path.join(self._bag_content_path, self._res_meta_path)
