...
def create_bagit_files_by_irods(res, istorage):
    ...
    if irods_bagit_input_path.startswith(resource_id):
        ...
        irods_dest_prefix = (
            "/" + settings.IRODS_ZONE + "/home/" + settings.IRODS_USERNAME
        )
    ...
    if is_exist:
        ...
        if not is_bagit_readme_exist:
            from_file_name = getattr(
                settings, "HS_BAGIT_README_FILE_WITH_PATH", "docs/bagit/readme.txt"
            )
            istorage.saveFile(from_file_name, bagit_readme_file, True)

        # call iRODS bagit rule here
        bagit_rule_file = getattr(
            settings, "IRODS_BAGIT_RULE", "hydroshare/irods/ruleGenerateBagIt_HS.r"
        )

        try:
            # call iRODS run command to create the files for the bag
            istorage.runBagitRule(
                bagit_rule_file, bagit_input_path, bagit_input_resource
            )
...
def create_bag_metadata_files(resource):
    ...
    resMetaFile._dc.title = (
        "Dublin Core science metadata document describing the HydroShare " "resource"
    )
    ...
    if resource.resource_type == "CollectionResource" and resource.resources:
        ...
        for logical_file in resource.logical_files:
            if logical_file.has_parent:
                ...
                continue
            aggr_uri = (
                "{hs_url}/resource/{res_id}/data/contents/{map_file_path}#aggregation"
            )
            ...
            agg_type_url = agg_type_url.format(
    ...
    try:
        ...
        if os.path.isdir(bag_path):
            ...
            if mtype[0] != "application/zip":
                msg = (
                    "Expected bag to have MIME type application/zip, "
