...
def hs_date_to_datetime(datestr):
    """
    Parse xDCIShare (HS) formatted date from a String to a datetime.datetime.
...
def hs_date_to_datetime_iso(datestr):
    """
    Parse the ISO 8601-formatted xDCIShare (HS) date from a String to a datetime.datetime.
    :param datestr: String representing the date in HS format
    :return: datetime.datetime with timezone set to UTC
    """
...
def hs_date_to_datetime_notz(datestr):
    """
    Parse xDCIShare (HS) formatted datetime (without timezone information) from a String
