...
class TestCaseCommonUtilities(object):
    ...
    def resource_file_oprs(self):
        ...
        self.assertIn(
            "sub_test_dir", store[0], msg="resource does not contain created sub-folder"
        )

        # create a temporary zips folder to make sure no duplicate folders are returned from listdir()
        zip_res_coll_path = os.path.join(
            "zips", "2020-02-03", res.short_id, "data", "contents", "sub_test_dir"
        )
        istorage.session.run("imkdir", None, "-p", zip_res_coll_path)
        store = istorage.listdir(res_path)
        self.assertEqual(
            store[0].count("sub_test_dir"),
            1,
            msg="duplicate folder: sub_test_dir occurred more " "than once",
        )
        ...
        self.assertIn(
            "sub_test_dir/" + file_name_list[1],
            updated_res_file_names,
            msg="resource does not contain "
            + file_name_list[1]
        ...
        output_zip_fname, size = zip_folder(
            user, res.short_id, "data/contents/sub_test_dir", "sub_test_dir.zip", True
        )
        ...
        self.assertEqual(
            res.files.all().count(), 2, msg="resource file count didn't match-"
        )
        ...
        res_file_txt = _add_file_to_resource(
            resource=res, file_to_add=self.test_data_file_path
        )
        ...
        with self.assertRaises(FileOverrideException):
            unzip_file(
                user,
                res.short_id,
                f"data/contents/{self.test_data_zip_file_name}",
                bool_remove_original=False,
            )
        ...
        zip_storage_file_path = os.path.join(
            res.file_path, self.test_data_zip_file_name
        )
        ...
        file_storage_path_old = os.path.join(
            res.file_path, "sub_test_dir", file_name_list[0]
        )
        file_storage_path_new = os.path.join(
            res.file_path, "sub_test_dir", f"new_{file_name_list[0]}"
        )
        ...
        move_or_rename_file_or_folder(
            user, res.short_id, "data/contents/sub_test_dir", "data/contents/sub_dir"
        )
    ...
    def raster_metadata_extraction(self):
        ...
        self.assertEqual(
            self.resRaster.metadata.coverages.all().filter(type="box").count(), 1
        )

        box_coverage = (
            self.resRaster.metadata.coverages.all().filter(type="box").first()
        )
        ...
        self.assertEqual(
            self.resRaster.metadata.formats.all()
            .filter(value="application/vrt")
            .count(),
            1,
        )
        self.assertEqual(
            self.resRaster.metadata.formats.all().filter(value="image/tiff").count(), 1
        )
        ...
        projection_string = (
            'PROJCS["NAD83 / UTM zone 12N",GEOGCS["NAD83",'
    ...
    def netcdf_metadata_extraction(self, expected_creators_count=1):
        ...
        # test core metadata after metadata extraction
        extracted_title = (
            "Snow water equivalent estimation at TWDEF site from "
        ...
        self.assertEqual(
            self.resNetcdf.metadata.relations.filter(type="source").count(), 1
        )
        ...
        self.assertEqual(
            self.resNetcdf.metadata.relations.all().filter(type="references").count(), 1
        )

        # there should be creators equal to expected_creators_count
        self.assertEqual(
            self.resNetcdf.metadata.creators.all().count(), expected_creators_count
        )
        ...
        self.assertEqual(
            self.resNetcdf.metadata.coverages.all().filter(type="box").count(), 1
        )
        self.assertEqual(
            self.resNetcdf.metadata.coverages.all().filter(type="period").count(), 1
        )

        box_coverage = (
            self.resNetcdf.metadata.coverages.all().filter(type="box").first()
        )
        self.assertEqual(box_coverage.value["projection"], "WGS 84 EPSG:4326")
        self.assertEqual(box_coverage.value["units"], "Decimal degrees")
        ...
        temporal_coverage = (
            self.resNetcdf.metadata.coverages.all().filter(type="period").first()
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.resNetcdf.metadata.formats.all().filter(value="text/plain").count(), 1
        )
        self.assertEqual(
            self.resNetcdf.metadata.formats.all()
        ...
        var_grid = (
            self.resNetcdf.metadata.variables.all()
            .filter(name="transverse_mercator")
            .first()
        )
    ...
    def timeseries_metadata_extraction(self):
        ...
        self.assertEqual(
            self.resTimeSeries.metadata.description.abstract.strip(), extracted_abstract
        )

        # there should be 2 coverage element -  box type and period type
        self.assertEqual(self.resTimeSeries.metadata.coverages.all().count(), 2)
        self.assertEqual(
            self.resTimeSeries.metadata.coverages.all().filter(type="box").count(), 1
        )
        self.assertEqual(
            self.resTimeSeries.metadata.coverages.all().filter(type="period").count(), 1
        )

        box_coverage = (
            self.resTimeSeries.metadata.coverages.all().filter(type="box").first()
        )
        self.assertEqual(box_coverage.value["projection"], "WGS 84 EPSG:4326")
        self.assertEqual(box_coverage.value["units"], "Decimal degrees")
        ...
        temporal_coverage = (
            self.resTimeSeries.metadata.coverages.all().filter(type="period").first()
        )
        self.assertEqual(
            parser.parse(temporal_coverage.value["start"]).date(),
        ...
        self.assertEqual(
            self.resTimeSeries.metadata.cv_aggregation_statistics.all().count(), 17
        )
