...
pre_delete_file_from_resource = django.dispatch.Signal(
    providing_args=["file", "resource"]
)
post_delete_file_from_resource = django.dispatch.Signal(providing_args=["resource"])
post_add_files_to_resource = django.dispatch.Signal(
    providing_args=["files", "resource"]
)

pre_metadata_element_create = django.dispatch.Signal(
    providing_args=["element_name", "request"]
)
pre_metadata_element_update = django.dispatch.Signal(
    providing_args=["element_name", "element_id", "request"]
)
post_metadata_element_update = django.dispatch.Signal(
    providing_args=["element_name", "element_id"]
)

pre_download_file = django.dispatch.Signal(
    providing_args=["sender", "request", "resource", "download_file_name"]
)
...
pre_move_or_rename_file_or_folder = django.dispatch.Signal(
    providing_args=["resource", "src_full_path", "tgt_full_path"]
)

post_remove_file_aggregation = django.dispatch.Signal(
    providing_args=["resource", "files"]
)
post_add_generic_aggregation = django.dispatch.Signal(
    providing_args=["resource", "file"]
)
