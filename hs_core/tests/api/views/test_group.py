...
class TestGroup(MockIRODSTestCaseMixin, ViewTestCase):
    ...
    def test_group_create_failures(self):
        ...
        new_group = Group.objects.filter(
            gaccess__description="This is a cool group"
        ).first()
    ...
    def test_unshare_group_with_user(self):
        ...
        response = unshare_group_with_user(
            request, group_id=new_group.id, user_id=self.mike.id
        )
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
    ...
    def test_unshare_resource_with_group(self):
        ...
        self.john.uaccess.share_resource_with_group(
            self.resource, new_group, PrivilegeCodes.VIEW
        )
        self.assertIn(self.resource, new_group.gaccess.view_resources)
        ...
        request = self.factory.post(url)
        request.user = self.john
        response = unshare_resource_with_group(
            request, shortkey=self.resource.short_id, group_id=new_group.id
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response_content = json.loads(response.content.decode())
        self.assertEqual(response_content["status"], "success")
        ...
        self.john.uaccess.share_group_with_user(
            new_group, self.mike, PrivilegeCodes.VIEW
        )
    ...
    def _share_resource_with_group(self, group, privilege):
        ...
        self.set_request_message_attributes(request)
        request.user = self.john
        response = share_resource_with_group(
            request,
            shortkey=self.resource.short_id,
            privilege=privilege,
            group_id=group.id,
    ...
    def _share_group_with_user(self, group, privilege):
        ...
        request.user = self.john
        response = share_group_with_user(
            request, group_id=group.id, user_id=self.mike.id, privilege=privilege
        )
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
        self.assertEqual(response["Location"], request.META["HTTP_REFERER"])
        ...
        if privilege == "view":
            self.assertIn(
                self.mike,
                group.gaccess.get_users_with_explicit_access(PrivilegeCodes.VIEW),
            )
        elif privilege == "edit":
            self.assertIn(
                self.mike,
                group.gaccess.get_users_with_explicit_access(PrivilegeCodes.CHANGE),
            )
        else:
            self.assertIn(
                self.mike,
                group.gaccess.get_users_with_explicit_access(PrivilegeCodes.OWNER),
            )
