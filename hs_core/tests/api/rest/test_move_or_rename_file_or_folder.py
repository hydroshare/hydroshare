...
class TestPublicRenameEndpoint(HSRESTTestCase):
    def setUp(self):
        ...
        params = {
            "file": (self.txt_file_name, open(self.txt_file_path, "rb"), "text/plain")
        }
    ...
    def test_bad_requests(self):
        ...
        response = self.client.post(
            unzip_url,
            {"source_path": " ", "target_path": "data/contents/foo"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response = self.client.post(
            unzip_url,
            {"source_path": "data/contents/foo", "target_path": " "},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response = self.client.post(
            unzip_url, {"target_path": "data/contents/foo"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response = self.client.post(
            unzip_url, {"source_path": "data/contents/foo"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_good_request(self):
        ...
        response = self.client.post(
            unzip_url,
            {"source_path": "data/contents/foo", "target_path": "data/contents/bar"},
            format="json",
        )
