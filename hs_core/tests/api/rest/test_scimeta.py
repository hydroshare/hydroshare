...
class TestScienceMetadata(SciMetaTestCase):
    ...
    def test_put_scimeta_generic(self):
        ...
        try:
            ...
            self.getAbstract(
                scimeta, should_exist=False, rdf_type="hsterms:GenericResource"
            )

            # Modify science metadata
            desc = scimeta.xpath(
                "/rdf:RDF/hsterms:GenericResource[1]", namespaces=self.NS
            )[0]
            ...
            self.updateScimetaResourceID(
                scimeta, "THISISNOTARESOURCEID", rdf_type="hsterms:GenericResource"
            )
            #    Write out to a file
            out = etree.tostring(scimeta, encoding="UTF-8", pretty_print=True).decode()
            with open(sci_meta_new, "w") as f:
