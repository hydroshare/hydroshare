...
class TestResourceFile(HSRESTTestCase):
    ...
    def test_DEPRECATED_resource_file_list(self):
        response = self.client.get(
            "/hsapi/resource/{pid}/file_list/".format(pid=self.pid), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 2)
        content_list = [
            os.path.basename(content["results"][0]["url"]),
    ...
    def test_resource_file_list(self):
        response = self.client.get(
            "/hsapi/resource/{pid}/files/".format(pid=self.pid), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 2)
    ...
    def test_create_resource_file(self):
        ...
        params = {"file": (txt_file_name, open(txt_file_path, "rb"), "text/plain")}
