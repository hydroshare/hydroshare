...
class TestResourceList(HSRESTTestCase):
    ...
    def test_resource_list_by_type(self):
        ...
        comp_res = resource.create_resource(
            "CompositeResource", self.user, "My composite resource", files=(raster,)
        )
        comp_pid = comp_res.short_id
        self.resources_to_delete.append(comp_pid)

        app_res = resource.create_resource(
            "ToolResource", self.user, "My Test App Resource"
        )
        ...
        self.assertTrue(
            content["results"][0]["resource_url"].endswith(
                res_tail.format(res_id=gen_pid)
            )
        )
        ...
        self.assertTrue(
            content["results"][2]["resource_url"].endswith(
                res_tail.format(res_id=app_pid)
            )
        )

        # Filter by type (single)
        response = self.client.get(
            "/hsapi/resource/", {"type": "CompositeResource"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        ...
        self.assertTrue(
            content["results"][0]["resource_url"].endswith(
                res_tail.format(res_id=comp_pid)
            )
        )
        ...
        response = self.client.get(
            "/hsapi/resource/",
            {"type": ["CompositeResource", "ToolResource"]},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        ...
        self.assertTrue(
            content["results"][1]["resource_url"].endswith(
                res_tail.format(res_id=app_pid)
            )
        )

    def test_resource_list_by_keyword(self):
        gen_res_one = resource.create_resource(
            "GenericResource", self.user, "Resource 1"
        )
        gen_res_two = resource.create_resource(
            "GenericResource", self.user, "Resource 2"
        )
        gen_res_three = resource.create_resource(
            "GenericResource", self.user, "Resource 3"
        )
        gen_res_four = resource.create_resource(
            "GenericResource", self.user, "Resource 2"
        )
        ...
        response = self.client.get(
            "/hsapi/resource/", {"subject": "one"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 2)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"subject": "other"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 2)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"subject": "one,other"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        ...
        response = self.client.get(
            "/hsapi/resource/", {"subject": "oth"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 2)
    ...
    def test_resource_list_obsolete(self):
        ...
        new_ver_gen_res_one = resource.create_empty_resource(
            gen_res_one.short_id, self.user
        )

        new_ver_gen_res_one = resource.create_new_version_resource(
            gen_res_one, new_ver_gen_res_one, self.user
        )
        ...
        self.assertEqual(content["count"], 1)
        self.assertEqual(
            content["results"][0]["resource_id"], new_ver_gen_res_one.short_id
        )

        # set include_obsolete to True, which should return all resources including obsoleted ones
        response = self.client.get(
            "/hsapi/resource/", {"include_obsolete": True}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 2)
    ...
    def test_resource_list_by_bounding_box(self):
        metadata_dict_one = [
            {
                "coverage": {
                    "type": "point",
                    "value": {"north": "70", "east": "70", "units": "decimal deg"},
                }
            }
        ]
        comp_res_one = resource.create_resource(
            "CompositeResource", self.user, "Resource 1", metadata=metadata_dict_one
        )
        ...
        comp_res_with_aggregation = resource.create_resource(
            "CompositeResource", self.user, "Resource with Aggregation", files=files
        )

        res_file = comp_res_with_aggregation.files.first()
        GenericLogicalFile.set_file_type(
            resource=comp_res_with_aggregation, user=self.user, file_id=res_file.id
        )
        ...
        comp_res_two = resource.create_resource(
            "CompositeResource", self.user, "Resource 2"
        )
        ...
        comp_res_three = resource.create_resource(
            "CompositeResource", self.user, "Resource 3", metadata=metadata_dict_three
        )
        ...
        comp_res_four = resource.create_resource(
            "CompositeResource", self.user, "Resource 4", metadata=metadata_dict_four
        )
        ...
        expected_matched_resources = [
            comp_res_one.short_id,
            comp_res_three.short_id,
            comp_res_four.short_id,
        ]
        ...
        response = self.client.get(
            "/hsapi/resource/",
            {"coverage_type": "bad", "nonsensical": "90", "params": "140"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_resource_list_by_group(self):
        group_one = self.user.uaccess.create_group(
            title="Group One",
            description="This is a great group",
            purpose="To have fun",
            auto_approve=True,
        )
        group_two = self.user.uaccess.create_group(
            title="Group Two",
        ...
        gen_res_one = resource.create_resource(
            ...
            self.user,
            "Resource 1",
            edit_groups=[group_one, group_two],
            view_groups=None,
        )
        gen_res_two = resource.create_resource(
            "GenericResource",
            ...
            "Resource 2",
            edit_groups=None,
            view_groups=[group_one, group_two],
        )
        gen_res_three = resource.create_resource(
            "GenericResource",
            self.user,
            "Resource 3",
            edit_groups=[group_one],
            view_groups=None,
        )
        gen_res_four = resource.create_resource(
            "GenericResource",
        ...
        response = self.client.get(
            "/hsapi/resource/", {"group": str(group_one.pk)}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 3)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"group": str(group_two.pk)}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 2)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"group": str(group_one.name)}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 3)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"group": str(group_two.name)}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 2)
    ...
    def test_resource_list_by_user(self):
        ...
        response = self.client.get(
            "/hsapi/resource/", {"owner": self.user.username}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 3)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"owner": self.user.email}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 3)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"creator": self.user.username}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 3)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"creator": self.user.email}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 3)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"user": self.user.username}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 3)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"user": self.user.email}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 3)
        ...
        response = self.client.get(
            "/hsapi/resource/", {"author": self.user.email}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 3)
        ...
        response = self.client.get(
            "/hsapi/resource/",
            {"author": ",".join(self.user.email + "bad")},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
    ...
    def test_resource_list_paging(self):
        ...
        response = self.client.get(
            "/hsapi/resource/", {"page": 2, "count": 1}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        content = json.loads(response.content.decode())
        self.assertEqual(content["count"], 4)
