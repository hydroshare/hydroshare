...
class TestPublicUnzipEndpoint(HSRESTTestCase):
    def setUp(self):
        ...
        res = resource.create_resource(
            self.rtype, self.user, self.title, files=(payload,), unpack_file=False
        )
        ...
        params = {"file": ("test.zip", open(zip_path, "rb"), "application/zip")}
    ...
    def test_unzip(self):
        ...
        response = self.client.post(
            unzip_url, data={"remove_original_zip": "false", "unzip_to_folder": "true"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
