...
class TestPublicZipEndpoint(HSRESTTestCase):
    def setUp(self):
        ...
        self.res = resource.create_resource(
            self.rtype, self.user, self.title, unpack_file=False
        )
        ...
        params = {"file": ("text.txt", open(self.txt_file_path, "rb"), "text/plain")}
    ...
    def test_zip_folder_bad_requests(self):
        ...
        response_no_path = self.client.post(
            zip_url, {"output_zip_file_name": "test.zip"}, format="json"
        )
        response_empty_path = self.client.post(
            zip_url, {"output_zip_file_name": "test.zip"}, format="json"
        )
        response_no_fname = self.client.post(
            zip_url,
            {"output_zip_file_name": " ", "input_coll_path": "/files/foo"},
            format="json",
        )
        response_empty_fname = self.client.post(
            zip_url,
            {"output_zip_file_name": "test.zip", "input_coll_path": " "},
            format="json",
        )
