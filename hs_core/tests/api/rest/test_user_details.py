...
class TestUserDetails(APITestCase):
    def setUp(self):
        ...
        self.user.userprofile.identifiers = {
            "ORCID": "1234566",
            "ResearchGate": "someresearchgateid",
        }
        self.user.userprofile.user_type = "Computer Programming"
        self.user.userprofile.subject_areas = (
            "Hydrology, Water quality, Hydroinformatics"
        )
    ...
    def _verify_user(self, response):
        ...
        self.assertEqual(
            content["url"],
            "{domain}/user/{uid}/".format(domain=utils.current_site_url(), uid=user.pk),
        )
        ...
        self.assertEqual(
            content["subject_areas"], "Hydrology, Water quality, Hydroinformatics"
        )
    ...
    def test_user_details_username(self):
        response = self.client.get(
            f"/hsapi/userDetails/{self.username}/", format="json"
        )
    ...
    def test_user_details_user_email(self):
        response = self.client.get(
            f"/hsapi/userDetails/{self.user.email}/", format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self._verify_user(response)
    ...
    def test_user_details_username_case_insensitive(self):
        response = self.client.get(
            f"/hsapi/userDetails/{self.username.upper()}/", format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self._verify_user(response)
    ...
    def test_user_details_user_email_case_insensitive(self):
        response = self.client.get(
            f"/hsapi/userDetails/{self.user.email.upper()}/", format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self._verify_user(response)
