...
class TestPublicZipEndpoint(HSRESTTestCase):
    ...
    def test_folder_download_rest(self):
        zip_download_url = "/django_irods/rest_download/{pid}/data/contents/foo".format(
            pid=self.pid
        )
        ...
        self.assertTrue(
            len(task_id) > 0, msg="ensure a task_id is returned for async zipping"
        )
        ...
        response_json = json.loads(response.content.decode())
        self.assertNotEqual(
            response_json["download_path"].split("/")[5], download_split[5]
        )
