...
class TestPublicResourceFlagsEndpoint(HSRESTTestCase):
    def setUp(self):
        ...
        self.rtype = "GenericResource"
        self.title = "My Test resource"
        res_two = resource.create_resource(
            self.rtype,
            self.user,
            self.title,
            files=(self.file_one,),
    ...
    def test_set_resource_flag_make_public(self):
        ...
        response = self.client.post(flag_url, {"flag": "make_public"}, format="json")
    ...
    def test_set_resource_flag_make_private(self):
        flag_url = "/hsapi/resource/%s/flag/" % self.pid
        response = self.client.post(flag_url, {"flag": "make_private"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)

    def test_set_resource_flag_make_discoverable(self):
        flag_url = "/hsapi/resource/%s/flag/" % self.pid_two
        response = self.client.post(
            flag_url, {"flag": "make_discoverable"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)
    ...
    def test_set_resource_flag_make_not_discoverable(self):
        flag_url = "/hsapi/resource/%s/flag/" % self.pid
        response = self.client.post(
            flag_url, {"flag": "make_not_discoverable"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)
    ...
    def test_set_resource_flag_make_not_shareable(self):
        flag_url = "/hsapi/resource/%s/flag/" % self.pid
        response = self.client.post(
            flag_url, {"flag": "make_not_shareable"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)

    def test_set_resource_flag_make_shareable(self):
        flag_url = "/hsapi/resource/%s/flag/" % self.pid
        response = self.client.post(flag_url, {"flag": "make_shareable"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)
