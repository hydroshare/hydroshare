...
class TestModelProgramTemplateSchema(HSRESTTestCase):
    ...
    def test_assign_template_meta_schema_to_model_program(self):
        ...
        mp_aggr = self._create_model_program_aggregation(
            expected_file_count=1, upload_folder="mp-folder"
        )
        ...
        for template_schema_filename in self.schema_templates:
            ...
            url = (
                f"/hsapi/resource/{self.res.short_id}/modelprogram/template/meta/schema/{aggr_path}/"
    ...
    def test_update_metadata_model_program(self):
        ...
        self.assertEqual(
            mp_aggr.metadata.extra_metadata, metadata_json["additional_metadata"]
        )
        self.assertEqual(mp_aggr.metadata.version, metadata_json["version"])
        self.assertEqual(
            mp_aggr.metadata.programming_languages,
            metadata_json["programming_languages"],
        )
        self.assertEqual(
            mp_aggr.metadata.operating_systems, metadata_json["operating_systems"]
        )
        self.assertEqual(
            mp_aggr.metadata.release_date.strftime("%Y-%m-%d"),
            metadata_json["release_date"],
        )
        self.assertEqual(mp_aggr.metadata.website, metadata_json["website"])
        self.assertEqual(
            mp_aggr.metadata.code_repository, metadata_json["code_repository"]
        )
        self.assertEqual(
            mp_aggr.metadata_schema_json, metadata_json["program_schema_json"]
        )
        mp_program_file_types = []
        for mp_file_type in mp_aggr.metadata.mp_file_types.all():
            file_type = ModelProgramResourceFileType.type_name_from_type(
                type_number=mp_file_type.file_type
            )
            file_path = mp_file_type.res_file.short_path
            mp_program_file_types.append(
    ...
    def test_get_metadata_model_program(self):
        ...
        self.assertEqual(
            response_json["additional_metadata"], metadata_json["additional_metadata"]
        )
        self.assertEqual(response_json["version"], metadata_json["version"])
        self.assertEqual(
            response_json["programming_languages"],
            metadata_json["programming_languages"],
        )
        self.assertEqual(
            response_json["operating_systems"], metadata_json["operating_systems"]
        )
        ...
        self.assertEqual(
            response_json["code_repository"], metadata_json["code_repository"]
        )
        self.assertEqual(
            response_json["program_schema_json"], metadata_json["program_schema_json"]
        )
        self.assertEqual(
            response_json["program_file_types"], metadata_json["program_file_types"]
        )
    ...
    def test_update_model_instance_meta_schema(self):
        ...
        url = (
            f"/hsapi/resource/{self.res.short_id}/modelprogram/template/meta/schema/{mp_aggr_path}/"
            f"{template_schema_filename}"
        )
    ...
    def test_get_metadata_model_instance(self):
        ...
        response = self.client.put(
            url, data={"executed_by": mp_aggr.aggregation_name}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mi_aggr = ModelInstanceLogicalFile.objects.first()
        ...
        self.assertEqual(
            response_json["has_model_output"], metadata_json["has_model_output"]
        )
        ...
        self.assertEqual(
            response_json["spatial_coverage"], metadata_json["spatial_coverage"]
        )
    ...
    def test_update_metadata_model_instance(self):
        ...
        self.assertEqual(
            mi_aggr.metadata.extra_metadata, metadata_json["additional_metadata"]
        )
        self.assertEqual(
            mi_aggr.metadata.has_model_output, metadata_json["has_model_output"]
        )
        self.assertEqual(
            mi_aggr.metadata.executed_by.aggregation_name, metadata_json["executed_by"]
        )
    ...
    def _create_model_program_aggregation(self, expected_file_count, upload_folder=""):
        ...
        res_file = add_file_to_resource(
            self.res, file_to_upload, folder=upload_folder, check_target_folder=True
        )
        ...
        if upload_folder:
            ...
            ModelProgramLogicalFile.set_file_type(
                self.res, self.user, folder_path=upload_folder
            )
        else:
            # set the file as the model program aggregation type
            ModelProgramLogicalFile.set_file_type(
                self.res, self.user, file_id=res_file.id
            )
