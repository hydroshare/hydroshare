...
class TestCreateResource(MockIRODSTestCaseMixin, TestCase):
    ...
    def test_create_resource_without_content_files(self):
        res = resource.create_resource("GenericResource", self.user, "My Test Resource")
    ...
    def test_create_resource_with_content_files(self):
        new_res = resource.create_resource(
            "GenericResource", self.user, "My Test Resource", files=(self.file_one,)
        )
    ...
    def test_create_resource_with_metadata(self):
        ...
        metadata_dict = [
            ...
            {"language": {"code": "fre"}},
            {
                "date": {
                    "type": "valid",
                    "start_date": parser.parse("01/20/2016"),
        ...
        self.assertEqual(
            res.metadata.dates.all().count(),
            3,
            msg="Number of date elements not equal to 3.",
        )
        self.assertIn(
            "created",
            [dt.type for dt in res.metadata.dates.all()],
        ...
        self.assertEqual(
            res.metadata.creators.all().count(),
            2,
            msg="Number of creators not equal to 2",
        )
        self.assertIn(
            "John Smith",
            [cr.name for cr in res.metadata.creators.all()],
        ...
        self.assertEqual(
            res.metadata.contributors.all().count(),
            1,
        ...
        self.assertEqual(
            res.metadata.coverages.all().count(),
            2,
        ...
        self.assertEqual(
            res.metadata.identifiers.all().count(),
            2,
        ...
        self.assertEqual(
            res.metadata.subjects.all().count(),
            2,
            msg="Number of subject elements found not be 1.",
        )
        self.assertIn(
            "sub-1",
            [sub.value for sub in res.metadata.subjects.all()],
        ...
        valid_start_date = timezone.make_aware(
            dtime.datetime.strptime("01/20/2016", "%m/%d/%Y"),
            timezone.get_default_timezone(),
        )
        valid_end_date = timezone.make_aware(
            dtime.datetime.strptime("02/20/2016", "%m/%d/%Y"),
            timezone.get_default_timezone(),
        )
    ...
    def test_create_resource_with_metadata_for_publisher(self):
        ...
        metadata_dict = [
            {"publisher": {"name": "HydroShare", "url": "https://hydroshare.org"}},
        ]
    ...
    def test_create_resource_with_metadata_for_type(self):
        ...
        metadata_dict = [
            {"type": {"url": "https://hydroshare.org/GenericResource"}},
        ]
        res = resource.create_resource(
            resource_type="GenericResource",
            owner=self.user,
        ...
        self.assertEqual(
            res.metadata.type.url, type_url, msg="type element url is wrong"
        )
        if res:
            res.delete()
    ...
    def test_create_resource_with_metadata_for_format(self):
        ...
        metadata_dict = [
            {"format": {"value": "plain/text"}},
            {"format": {"value": "image/tiff"}},
        ]
        res = resource.create_resource(
            resource_type="GenericResource",
            owner=self.user,
    ...
    def test_create_resource_with_metadata_for_date(self):
        ...
        # of the type 'valid'
        metadata_dict = [
            {"date": {"type": "created", "start_date": parser.parse("01/16/2016")}},
        ...
        skipped_date = timezone.make_aware(
            dtime.datetime.strptime("01/16/2016", "%m/%d/%Y"),
            timezone.get_default_timezone(),
        )

        self.assertNotIn(
            skipped_date,
            [dat.start_date for dat in res.metadata.dates.all()],
            msg="Matching date value was found",
        )

        self.assertEqual(
            res.metadata.dates.filter(type="publisher").count(),
            0,
            msg="Publisher date was found.",
        )
        self.assertEqual(
            res.metadata.dates.filter(type="available").count(),
            0,
            msg="Available date was found.",
        )
        ...
        self.assertIn(
            valid_start_date,
            [dt.start_date for dt in res.metadata.dates.all()],
            msg="Matching date value was not found",
        )

        self.assertIn(
            valid_end_date,
            [dt.end_date for dt in res.metadata.dates.all()],
            msg="Matching date value was not found",
        )
        if res:
            res.delete()
    ...
    def test_create_resource_with_file(self):
        ...
        res = resource.create_resource(
            "GenericResource", self.user, "My Test resource", files=(raster,)
        )
        pid = res.short_id
    ...
    def test_create_resource_with_two_files(self):
        ...
        res = resource.create_resource(
            "GenericResource", self.user, "My Test resource", files=(raster, text)
        )
        pid = res.short_id
    ...
    def test_create_resource_with_zipfile(self):
        ...
        payload = MyTemporaryUploadedFile(
            open(zip_path, "rb"),
            name=zip_path,
        ...
        res = resource.create_resource(
            "GenericResource", self.user, "My Test resource", files=(payload,)
        )
        pid = res.short_id
        ...
        payload2 = MyTemporaryUploadedFile(
            open(zip_path, "rb"),
            name=zip_path,
            content_type="application/zip",
            size=os.stat(zip_path).st_size,
