...
class TestNewVersionResource(TestCase):
    ...
    def test_new_version_generic_resource(self):
        ...
        with self.assertRaises(PermissionDenied):
            hydroshare.create_empty_resource(self.res.short_id, self.nonowner)
        ...
        new_res = hydroshare.create_empty_resource(self.res.short_id, self.owner)
        # test to make sure the new versioned empty resource has no content files
        self.assertEqual(new_res.files.all().count(), 0)

        new_res = hydroshare.create_new_version_resource(self.res, new_res, self.owner)
        ...
        self.assertEqual(new_res.creator, self.owner,
        ...
        self.assertGreater(new_res.metadata.relations.all().count(), 0,
                           msg="New versioned resource does has relation element.")

        relation_is_version_of = new_res.metadata.relations.filter(type='isVersionOf').first()
        self.assertNotEqual(relation_is_version_of, None)
        self.assertEqual(relation_is_version_of.value, self.res.get_citation())

        relation_is_replaced_by = self.res.metadata.relations.filter(type='isReplacedBy').first()
        self.assertNotEqual(relation_is_replaced_by, None)
        self.assertEqual(relation_is_replaced_by.value, new_res.get_citation())

        # test isReplacedBy is removed after the new versioned resource is deleted
        hydroshare.delete_resource(new_res.short_id)
