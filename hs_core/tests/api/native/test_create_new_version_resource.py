...
class TestNewVersionResource(TestCase):
    ...
    def test_new_version_generic_resource(self):
        ...
        self.owner.uaccess.share_resource_with_user(
            self.res_generic, self.nonowner, PrivilegeCodes.CHANGE
        )
        with self.assertRaises(PermissionDenied):
            hydroshare.create_empty_resource(self.res_generic.short_id, self.nonowner)
        ...
        new_res_generic = hydroshare.create_empty_resource(
            self.res_generic.short_id, self.owner
        )
        ...
        new_res_generic = hydroshare.create_new_version_resource(
            self.res_generic, new_res_generic, self.owner
        )
        ...
        self.assertGreater(
            new_res_generic.metadata.relations.all().count(),
            0,
            msg="New versioned resource does has relation element.",
        )

        relation_is_version_of = new_res_generic.metadata.relations.filter(
            type="isVersionOf"
        ).first()
        ...
        relation_is_replaced_by = self.res_generic.metadata.relations.filter(
            type="isReplacedBy"
        ).first()
