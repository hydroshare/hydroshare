...
class TestUpdateGroupAPI(TestCase):
    def setUp(self):
        ...
        self.group = self.john_group_owner.uaccess.create_group(
            title="Modeling Group",
            description="This group is interested in hydrological " "modelling",
        )
    ...
    def test_set_group_active_status(self):
        ...
        with self.assertRaises(PermissionDenied):
            hydroshare.set_group_active_status(
                self.lisa_not_group_owner, self.group.id, False
            )

        with self.assertRaises(PermissionDenied):
            hydroshare.set_group_active_status(
                self.lisa_not_group_owner, self.group.id, True
            )
