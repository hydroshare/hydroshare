...
class TestResourceFileAPI(
    ...
    def test_unfederated_root_path_checks(self):
        ...
        errors, ecount = check_irods_files(
            self.res, return_errors=True, log_errors=False
        )

        self.assertTrue(
            errors[0].endswith("data/contents/fuzz.txt does not exist in iRODS")
        )
        self.assertTrue(
            errors[1].endswith(
        ...
        self.assertTrue(
            errors[2].endswith("type is GenericResource, title is 'My Test Resource'")
        )
        ...
        errors, ecount = check_irods_files(
            self.res, stop_on_error=True, log_errors=False
        )
