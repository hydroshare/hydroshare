...
class TestResourceFileFolderOprsAPI(
    ...
    def setUp(self):
        ...
        self.file_name_list = [
            ...
            self.test_file_name2,
            self.test_file_name3,
        ]
        ...
        self.test_data_zip_file_path = (
            f"hs_core/tests/data/{self.test_data_zip_file_name}"
        )
    ...
    def test_resource_file_folder_oprs(self):
        ...
        hydroshare.add_resource_files(
            self.res.short_id, self.test_file_1, self.test_file_2, self.test_file_3
        )

        # resource should has only three files at this point
        self.assertEqual(
            self.res.files.all().count(), 3, msg="resource file count didn't match"
        )
