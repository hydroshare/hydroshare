...
class TestGetCitation(MockIRODSTestCaseMixin, TestCase):
    ...
    def test_one_author(self):
        ...
        hs_identifier = self.res.metadata.identifiers.all().filter(
            name="hydroShareIdentifier"
        )[0]
        hs_url = hs_identifier.url
        hs_date = str(date.today().year)
        correct_citation = (
            "Creator_LastName, C. ({}). Generic resource, HydroShare, {}".format(
                hs_date, hs_url
            )
        )
    ...
    def test_two_authors_comma(self):
        # add a creator element
        resource.create_metadata_element(
            self.res.short_id, "creator", name="Smith, John"
        )
    ...
    def test_two_authors_multiple_first_and_last_names_comma(self):
        # add a creator element
        resource.create_metadata_element(
            self.res.short_id, "creator", name="Smith William, John Mason Jingle"
        )
    ...
    def test_two_authors_multiple_first_and_last_names_no_comma(self):
        # add a creator element
        resource.create_metadata_element(
            self.res.short_id, "creator", name="John Mason Jingle Smith William"
        )
    ...
    def test_two_authors_and_organization(self):
        ...
        resource.create_metadata_element(
            self.res.short_id, "creator", organization="U.S. Geological Survey"
        )
