...
class TestRelationTypeUpdateCommand(MockIRODSTestCaseMixin, TestCase):
    ...
    def test_update_is_part_of_relation_1(self):
        ...
        self.assertFalse(
            metadata.relations.filter(type=RelationTypes.isPartOf).exists()
        )
        self.assertFalse(
            metadata.relations.filter(type=RelationTypes.isReferencedBy).exists()
        )
        # add 'isPartOf' relation that is not related to collection
        metadata.create_element(
            "relation", type=RelationTypes.isPartOf, value="testing"
        )
        ...
        self.assertEqual(
            metadata.relations.filter(type=RelationTypes.isReferencedBy).count(), 1
        )
    ...
    def test_update_is_part_of_relation_2(self):
        ...
        metadata.create_element(
            "relation", type=RelationTypes.isPartOf, value="testing-1"
        )
        metadata.create_element(
            "relation", type=RelationTypes.isPartOf, value=comp_res_2.get_citation()
        )
        self.assertEqual(
            metadata.relations.filter(type=RelationTypes.isPartOf).count(), 2
        )
        ...
        self.assertEqual(
            metadata.relations.filter(type=RelationTypes.isReferencedBy).count(), 2
        )
    ...
    def test_update_is_part_of_relation_3(self):
        ...
        metadata.create_element(
            "relation", type=RelationTypes.isPartOf, value=col_res.get_citation()
        )
        ...
        metadata.create_element(
            "relation", type=RelationTypes.isPartOf, value="testing-2"
        )
        self.assertEqual(
            metadata.relations.filter(type=RelationTypes.isPartOf).count(), 3
        )

        # run  update command to update the type 'isPartOf' to 'isReferencedBy'
        call_command(self.update_command)
        self.assertEqual(
            metadata.relations.filter(type=RelationTypes.isPartOf).count(), 1
        )
        self.assertEqual(
            metadata.relations.filter(type=RelationTypes.isReferencedBy).count(), 2
        )
    ...
    def _create_resource(self, res_type="CompositeResource", add_keywords=False):
        res = hydroshare.create_resource(
            res_type, self.user, "Testing updating relation type isPartOf"
        )
        if add_keywords:
