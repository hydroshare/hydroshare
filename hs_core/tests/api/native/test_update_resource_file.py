...
class TestUpdateResourceFileAPI(MockIRODSTestCaseMixin, unittest.TestCase):
    ...
    def test_update_resource_file(self):
        ...
        new_res = hydroshare.create_resource(
            "GenericResource", self.user_creator, "My Test Resource"
        )

        # resource should not have any files at this point
        self.assertEqual(
            new_res.files.all().count(), 0, msg="resource file count didn't match"
        )
        ...
        self.assertEqual(
            new_res.files.all().count(), 1, msg="resource file count didn't match"
        )
        ...
        rf = hydroshare.update_resource_file(
            new_res.short_id, original_file_name, new_file
        )

        # test if the file name matches
        self.assertEqual(
            os.path.basename(rf.resource_file.name),
            new_file_name,
            msg="resource file name didn't match",
        )
        ...
        resource_file = hydroshare.get_resource_file(
            new_res.short_id, new_file_name
        ).resource_file
        self.assertEqual(
            resource_file.read().decode("utf-8"),
            new_file_data,
            msg="resource file content didn't match",
        )
        ...
        new_file_name = (
            "update.txt"  # file has the same name as the file that we will be updating
        )
        ...
        self.assertEqual(
            ...
            new_file_name,
            msg="{0} != {1}".format(
                os.path.basename(rf.resource_file.name), new_file_name
            ),
        )
        ...
        with self.assertRaises(ObjectDoesNotExist):
            hydroshare.update_resource_file(
                new_res.short_id, "file_not_in_resource.txt", new_file
            )
