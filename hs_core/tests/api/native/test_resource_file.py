...
class TestResourceFileAPI(
    ...
    def test_unfederated_folder_path_setting(self):
        ...
        # determine where that file should live
        shortpath = os.path.join(
            self.res.short_id, "data", "contents", "foo", "file1.txt"
        )
        ...
        # non-existent files should raise error
        otherpath = os.path.join(
            self.res.short_id, "data", "contents", "foo", "file2.txt"
        )
        with self.assertRaises(ValidationError):
            resfile.path_is_acceptable(otherpath)
    ...
    def test_federated_root_path_logic(self):
        ...
        shortpath = os.path.join(
            fedpath, self.res.short_id, "data", "contents", "file1.txt"
        )
        ...
        otherpath = os.path.join(
            fedpath, self.res.short_id, "data", "contents", "file2.txt"
        )
    ...
    def test_federated_folder_path_logic(self):
        ...
        self.assertEqual(
            resfile.resource_file.name,
            os.path.join(self.res.short_id, "data", "contents", "foo", "file1.txt"),
        )
        self.assertEqual(resfile.file_folder, "foo")
        ...
        # determine where that file should live
        shortpath = os.path.join(
            fedpath, self.res.short_id, "data", "contents", "foo", "file1.txt"
        )
        ...
        # non-existent files should raise error
        otherpath = os.path.join(
            fedpath, self.res.short_id, "data", "contents", "foo", "file2.txt"
        )
        resfile.path_is_acceptable(otherpath, test_exists=False)
        # This won't work because federation path is fake.
        # with self.assertRaises(ValidationError):
