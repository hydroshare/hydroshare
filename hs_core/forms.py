...
class Helper(object):
    ...
    def get_element_add_modal_form(cls, element_name, modal_form_context_name):
        ...
        layout = Layout(
            HTML(
                '<div class="modal fade" id="add-element-dialog" tabindex="-1" '
        ...
        layout[0] = HTML(
            '<div class="modal fade" id="add-%s-dialog" tabindex="-1" role="dialog" '
        ...
        html_str = (
            "{% load crispy_forms_tags %} {% crispy"
            + " add_{element}_modal_form".format(element=element_name.lower())
            + " %}"
        )
...
def _get_modal_confirm_delete_matadata_element():
    layout = Layout(
        HTML(
            '<div class="modal fade" id="delete-metadata-element-dialog" '
...
class MetaDataElementDeleteForm(forms.Form):
    ...
    def __init__(self, res_short_id, element_name, element_id, *args, **kwargs):
        ...
        self.helper.layout[0] = HTML(
            '<div class="modal fade" id="delete-%s-element-dialog_%s" '
            'tabindex="-1" role="dialog" aria-labelledby="myModalLabel" '
...
class ExtendedMetadataForm(forms.Form):
    ...
    def __init__(
        self, resource_mode="edit", extended_metadata_layout=None, *args, **kwargs
    ):
        """Render an extensible metadata form via the extended_metadata_layout kwarg."""
...
class PartyValidationForm(forms.Form):
    ...
    hydroshare_user_id = forms.IntegerField(
        required=False, validators=[validate_hydroshare_user_id]
    )
    ...
    def clean(self):
        ...
        if data["name"].lower() == "hydroshareidentifier":
            raise forms.ValidationError(
                "Identifier name attribute can't have a value "
                "of '{}'.".format(data["name"])
            )
        return data
...
class FundingAgencyForm(ModelForm):
    ...
    def __init__(self, allow_edit=True, res_short_id=None, *args, **kwargs):
        ...
        if res_short_id:
            self.action = (
                "/hsapi/_internal/%s/fundingagency/add-metadata/" % res_short_id
            )
        else:
            self.action = ""
...
class SubjectsFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(SubjectsFormHelper, self).__init__(
...
class SubjectsForm(forms.Form):
    ...
    def __init__(
        ...
        self.helper = SubjectsFormHelper(
            allow_edit, res_short_id, element_id, element_name="subject"
        )
        self.number = 0
        self.delete_modal_form = None
        if res_short_id:
...
class AbstractFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(AbstractFormHelper, self).__init__(
            allow_edit, res_short_id, element_id, element_name, layout, *args, **kwargs
        )
...
class AbstractForm(ModelForm):
    ...
    def __init__(
        ...
        self.helper = AbstractFormHelper(
            allow_edit, res_short_id, element_id, element_name="description"
        )
        if not allow_edit:
...
class CoverageTemporalFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(CoverageTemporalFormHelper, self).__init__(
            allow_edit, res_short_id, element_id, element_name, layout, *args, **kwargs
        )
...
class CoverageTemporalForm(forms.Form):
    ...
    def __init__(
        ...
        self.helper = CoverageTemporalFormHelper(
            allow_edit,
            res_short_id,
            element_id,
...
class CoverageSpatialFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(CoverageSpatialFormHelper, self).__init__(
            allow_edit, res_short_id, element_id, element_name, layout, *args, **kwargs
        )
...
class CoverageSpatialForm(forms.Form):
    ...
    projection = forms.CharField(
        max_length=100, required=False, label="Coordinate System/Geographic Projection"
    )
    ...
    def clean(self):
        ...
        if spatial_coverage_type == "point":
            ...
            box_fields_map = {
                "northlimit": "north latitude",
                "southlimit": "south latitude",
                "eastlimit": "east longitude",
                "westlimit": "west longitude",
            }
            for limit in box_fields_map.keys():
                ...
                if not limit_data and limit_data != 0:
                    self.add_error(
                        limit, "Data for %s is missing" % box_fields_map[limit]
                    )
                    is_form_errors = True
