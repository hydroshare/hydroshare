...
class S3Storage(S3Storage):
    ...
    def create_bucket(self, bucket_name):
        if not self.bucket_exists(bucket_name) and bucket_name not in ["tmp", "bags", "zips"]:
            ...
            subprocess.run(["mc", "event", "add", f"hydroshare/{bucket_name}", "arn:minio:sqs::RESOURCEFILE:kafka", "--event", "put,delete"], check=True)
