...
print(
    (
        "[{0}] Find {1} Collection Resources\n".format(
            str(datetime.now()), collection_count
        )
    )
)
counter = 0
...
for collection_res_obj in collection_res_list:
    try:
        ...
        print(
            (
                "[{0}] Processing on collection {1}: ({2}/{3})\n".format(
                    str(datetime.now()),
        ...
        print(
            (
                "[{0}] Done with Collection {1}\n".format(
                    str(datetime.now()), collection_res_obj.short_id
                )
            )
        )
        success_counter += 1
        ...
        print(
            (
                "[{0}] Error on Collection {1}: {2}\n".format(
                    str(datetime.now()), collection_res_obj.short_id, str(ex)
                )
            )
        )
        error_counter += 1
...
print(
    (
        "TOTAL: {0}; SUCCESS: {1};  ERROR: {2}\n".format(
            collection_count, success_counter, error_counter
        )
    )
)
