...
class TestCollection(MockIRODSTestCaseMixin, TransactionTestCase):
    def setUp(self):
        ...
        self.resGen1 = create_resource(
            resource_type="GenericResource", owner=self.user1, title="Gen 1"
        )

        self.resGen2 = create_resource(
            resource_type="GenericResource", owner=self.user1, title="Gen 2"
        )

        self.resGen3 = create_resource(
            resource_type="GenericResource", owner=self.user1, title="Gen 3"
        )

        self.resGen4 = create_resource(
            resource_type="GenericResource", owner=self.user1, title="Gen 4"
        )
        ...
        self.resGen5 = create_resource(
            resource_type="GenericResource", owner=self.user2, title="Gen 4"
        )

        base_url = "/hsapi/_internal/{0}"
        self.url_to_update_collection = base_url + "/update-collection/"
        self.url_to_collection_member_permission = (
            base_url + "/collection-member-permission/{1}/"
        )
    ...
    def test_collection_basic_functions(self):
        ...
        self.assertIn(self.resGen1, self.resCollection.resources.all())
        self.assertIn(
            self.resGen1, self.resCollection_with_missing_metadata.resources.all()
        )
    ...
    def test_collection_deleted_resource(self):
        ...
        CollectionDeletedResource.objects.create(
            resource_title=self.resGen1.metadata.title,
            deleted_by=self.user1,
            collection=self.resCollection,
        )
        CollectionDeletedResource.objects.create(
            resource_title=self.resGen2.metadata.title,
            deleted_by=self.user1,
            collection=self.resCollection,
        )
        ...
        self.assertEqual(
            self.resCollection.deleted_resources.filter(
                resource_title=self.resGen1.metadata.title
            ).count(),
            1,
        )
    ...
    def test_update_collection_set_add_remove_logic(self):
        ...
        response = self.api_client.post(
            url_to_update_collection,
            {"update_type": "set", "resource_id_list": []},
        )
        self.assertEqual(resp_json["status"], "success")
        self.assertEqual(self.resCollection.resources.count(), 0)
        ...
        response = self.api_client.post(
            url_to_update_collection,
            {"update_type": "add", "resource_id_list": [self.resGen4.short_id]},
        )
        resp_json = json.loads(response.content.decode())
        self.assertEqual(resp_json["status"], "success")
        ...
        response = self.api_client.post(
            url_to_update_collection,
            {"update_type": "add", "resource_id_list": []},
        )
        resp_json = json.loads(response.content.decode())
        self.assertEqual(resp_json["status"], "success")
        self.assertEqual(self.resCollection.resources.count(), 4)
        ...
        response = self.api_client.post(
            url_to_update_collection,
            {"update_type": "add", "resource_id_list": [self.resGen3.short_id]},
        )
        resp_json = json.loads(response.content.decode())
        ...
        response = self.api_client.post(
            url_to_update_collection,
            {"update_type": "remove", "resource_id_list": []},
        )
        resp_json = json.loads(response.content.decode())
        self.assertEqual(resp_json["status"], "success")
        self.assertEqual(self.resCollection.resources.count(), 2)
        ...
        response = self.api_client.post(
            url_to_update_collection,
            {"update_type": "remove", "resource_id_list": [self.resGen1.short_id]},
        )
        resp_json = json.loads(response.content.decode())
        self.assertEqual(resp_json["status"], "error")
        self.assertEqual(self.resCollection.resources.count(), 2)
    ...
    def test_update_collection_own_permission(self):
        ...
        response = self.api_client.post(
            url_to_update_collection,
            {"resource_id_list": [self.resGen2.short_id, self.resGen5.short_id]},
        )
        resp_json = json.loads(response.content.decode())
        self.assertEqual(resp_json["status"], "error")
        self.assertEqual(self.resCollection.resources.count(), 2)
        ...
        self.user2.uaccess.share_resource_with_user(
            self.resGen5, self.user1, PrivilegeCodes.VIEW
        )
        ...
        self.assertFalse(
            self.resCollection_with_missing_metadata.can_be_public_or_discoverable
        )
        response = self.api_client.post(
            url_to_update_collection,
    ...
    def test_update_collection_edit_permission(self):
        ...
        self.user1.uaccess.share_resource_with_user(
            self.resCollection, self.user2, PrivilegeCodes.VIEW
        )
        ...
        self.user1.uaccess.share_resource_with_user(
            self.resCollection, self.user2, PrivilegeCodes.CHANGE
        )
        ...
        self.user1.uaccess.share_resource_with_user(
            self.resGen3, self.user2, PrivilegeCodes.VIEW
        )
    ...
    def test_collection_holds_collection(self):
        ...
        self.assertEqual(self.resCollection.resources.count(), 1)
        self.assertEqual(
            self.resCollection.resources.all()[0].resource_type.lower(),
            "collectionresource",
        )
    ...
    def test_update_collection_for_deleted_resources(self):
        ...
        url_to_delete_resource_for_resGen1 = self.url_to_delete_resource.format(
            self.resGen1.short_id
        )
        self.api_client.post(
            url_to_delete_resource_for_resGen1, HTTP_REFERER="http://foo/bar"
        )

        # delete resGen2
        res_id_resGen2 = self.resGen2.short_id
        url_to_delete_resource_for_resGen2 = self.url_to_delete_resource.format(
            self.resGen2.short_id
        )
        self.api_client.post(
            url_to_delete_resource_for_resGen2, HTTP_REFERER="http://foo/bar"
        )
        ...
        self.assertEqual(self.resCollection.deleted_resources.count(), 2)
        self.assertIn(
            CollectionDeletedResource.objects.get(resource_id=res_id_resGen1),
            self.resCollection.deleted_resources.all(),
        )
        self.assertIn(
            CollectionDeletedResource.objects.get(resource_id=res_id_resGen2),
            self.resCollection.deleted_resources.all(),
        )
    ...
    def test_are_all_contained_resources_published(self):
        ...
        # should return False
        self.assertEqual(
            self.resCollection.are_all_contained_resources_published, False
        )
    ...
    def test_versioning(self):
        ...
        new_collection = create_empty_resource(
            self.resCollection.short_id, self.user1.username
        )

        new_collection = create_new_version_resource(
            self.resCollection, new_collection, self.user1
        )
        ...
        self.assertEqual(
            new_collection.resources.count(), self.resCollection.resources.count()
        )
    ...
    def test_copy(self):
        ...
        new_collection = create_empty_resource(
            self.resCollection.short_id, self.user1.username, action="copy"
        )
    ...
    def test_update_collection_coverages(self):
        ...
        update_science_metadata(
            pk=self.resGen1.short_id, metadata=metadata_dict, user=self.user1
        )
        ...
        self.assertEqual(
            parser.parse(period_coverage_obj.value["start"].lower()),
            parser.parse("1/1/2016"),
        )
        self.assertEqual(
            parser.parse(period_coverage_obj.value["end"].lower()),
        ...
        update_science_metadata(
            pk=self.resGen2.short_id, metadata=metadata_dict, user=self.user1
        )
        ...
        self.assertEqual(
            parser.parse(period_qs[0].value["start"].lower()), parser.parse("1/1/2016")
        )
        self.assertEqual(
            parser.parse(period_qs[0].value["end"].lower()), parser.parse("12/31/2016")
        )
        # test point
        ...
        update_science_metadata(
            pk=self.resGen3.short_id, metadata=metadata_dict, user=self.user1
        )
        ...
        self.assertEqual(period_qs.count(), 1)
        self.assertEqual(
            parser.parse(period_qs[0].value["start"].lower()), parser.parse("1/1/2010")
        )
        self.assertEqual(
            parser.parse(period_qs[0].value["end"].lower()), parser.parse("12/31/2016")
        )
        ...
        url_to_calculate_collection_coverages = (
            self.url_to_calculate_collection_coverages.format(
                self.resCollection.short_id
            )
        )
        ...
        for cv in resp_json["new_coverage_list"]:
            if cv["type"] == "period":
                found_period = True
                self.assertEqual(
                    parser.parse(cv["value"]["start"].lower()), parser.parse("1/1/2010")
                )
                self.assertEqual(
                    parser.parse(cv["value"]["end"].lower()), parser.parse("12/31/2016")
                )
    ...
    def test_hasPart_metadata(self):
        ...
        self.assertEqual(
            self.resCollection.metadata.relations.filter(
                type=hasPart, value=value
            ).count(),
            1,
        )
    ...
    def test_save_resource_list_csv_to_bag(self):
        ...
        self.assertEqual(self.resCollection.resources.count(), 3)
        self.assertEqual(
            ResourceFile.objects.filter(object_id=self.resCollection.id).count(), 0
        )
        csv_list = update_collection_list_csv(self.resCollection)
        self.assertEqual(
            ResourceFile.objects.filter(object_id=self.resCollection.id).count(), 1
        )
    ...
    def test_collection_resource_delete(self):
        ...
        self.assertEqual(
            self.resGen1.metadata.relations.filter(type="isPartOf").count(), 1
        )
        self.assertEqual(
            self.resGen2.metadata.relations.filter(type="isPartOf").count(), 1
        )
        ...
        self.assertEqual(
            self.resGen1.metadata.relations.filter(type="isPartOf").count(), 0
        )
        self.assertEqual(
            self.resGen2.metadata.relations.filter(type="isPartOf").count(), 0
        )
    ...
    def test_delete_resource_in_collection(self):
        ...
        self.assertEqual(
            self.resCollection.metadata.relations.filter(type="hasPart").count(), 2
        )
        # check of the resources that are part of the collection resource has 'isPartOf' relation metadata
        self.assertEqual(
            self.resGen1.metadata.relations.filter(type="isPartOf").count(), 1
        ...
        self.assertEqual(
            self.resCollection.metadata.relations.filter(type="hasPart").count(), 1
        )
