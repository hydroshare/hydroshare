...
class TestCollection(MockIRODSTestCaseMixin, TransactionTestCase):
    def setUp(self):
        ...
        self.res_1_user_1 = create_resource(
            resource_type="CompositeResource", owner=self.user1, title="Gen 1"
        )

        self.res_2_user_1 = create_resource(
            resource_type="CompositeResource", owner=self.user1, title="Gen 2"
        )
        ...
        self.res_2_user_2 = create_resource(
            resource_type="CompositeResource", owner=self.user2, title="Gen 3"
        )
    ...
    def test_collectable_shareable_resources(self):
        ...
        self.assertEqual(
            get_collectable_resources(self.user1, self.resCollection).all().count(), 2
        )
        ...
        self.assertEqual(
            get_collectable_resources(self.user1, self.resCollection).all().count(), 3
        )
