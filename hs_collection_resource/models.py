...
class SupportedResTypes(AbstractMetaDataElement):
    ...
    supported_res_types = models.ManyToManyField(BaseResource, null=True, blank=True)

    def get_supported_res_types_str(self):
        return ','.join([res.short_id for res in self.supported_res_types.all()])
    ...
    def create(cls, **kwargs):
        if 'supported_res_types' in kwargs:
            ...
            for res_id in kwargs['supported_res_types']:
                res = BaseResource.objects.filter(short_id__iexact=res_id)
                if res.exists():
                    new_meta_instance.supported_res_types.add(res[0])
