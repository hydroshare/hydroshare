...
class SchemaBaseModel(BaseModel):
    model_config = ConfigDict(
        json_schema_extra=modify_json_schema, extra="allow")


class DefinedTerm(SchemaBaseModel):
    type:
        str = Field(alias="@type", default="DefinedTerm")
    name:
        str = Field(description="The name of the term or item being defined.")
    description:
        str = Field(description="The description of the item being defined.")


class Published(DefinedTerm):
    name:
        str = Field(default="Published")
    description:
        str = Field(
...
class Discoverable(DefinedTerm):
    name:
        str = Field(default="Discoverable")
    description:
        str = Field(
...
class CreativeWork(SchemaBaseModel):
    ...
    name:
        str = Field(description="Submission's name or title",
                    title="Name or title")
    description:
        Optional[str] = Field(
...
class FunderOrganization(Organization):
    ...
    name:
        str = Field(description="Name of the organization.")


class PublisherOrganization(Organization):
    name:
        str = Field(description="Name of the publishing organization.")
    url:
        Optional[HttpUrl] = Field(
            title="URL",
...
class DefinedTerm(SchemaBaseModel):
    type:
        Literal["DefinedTerm"] = Field(
            alias="@type", default="DefinedTerm")  # type: ignore
    name:
        str = Field(description="The name of the term or item being defined.")
    description:
        str = Field(description="The description of the item being defined.")
...
class IdentifierStr(str):
    ...
    def validate(
        ...
        # Join the list into a single string for storage
...
class SpatialReference(SchemaBaseModel):
    type:
        Literal["SpatialReference"] = Field(
            alias="@type",  # type: ignore
    ...
    def validate_content_size(cls, v):
        ...
        if v not in ["geographic", "projected"]:
            raise ValueError(
                "SRS Type must be either 'geographic' or 'projected'")
        return v
...
class Grant(SchemaBaseModel):
    ...
    funder:
        Optional[FunderOrganization] = Field(
    ...
    def validate_box(cls, v, info):
        ...
        for index, item in enumerate(v_parts, start=1):
            try:
                ...
                raise ValueError(
                    "Bounding box coordinate value is not a number")
            item = abs(item)
            if index % 2 == 0:
                if item > 180:
                    raise ValueError(
...
class PropertyValue(SchemaBaseModel):
    type:
        Literal["PropertyValue"] = Field(
            alias="@type",  # type: ignore
            default="PropertyValue",
            description="A property-value pair.",
        )
    name:
        str = Field(description="The name of the property.")
...
class Place(SchemaBaseModel):
    ...
    name:
        Optional[str] = Field(description="Name of the place.", default=None)
    geo:
        Optional[Union[GeoCoordinates, GeoShape]] = Field(
    ...
    additionalProperty:
        Optional[List[PropertyValue]] = Field(
    ...
    srs:
        Optional[SpatialReference] = Field(
            description="The spatial reference system associated with the Place's geographic representation",
            default=None,
        )
...
class MediaObject(SchemaBaseModel):
    type:
        Literal["MediaObject"] = Field(
            alias="@type",  # type: ignore
            default="MediaObject",
            description="An item that encodes the record.",
        )
    contentUrl:
        Union[str, AnyUrl] = Field(
            title="Content URL",
    ...
    name:
        str = Field(description="The name of the media object (file).")
    sha256:
        Optional[str] = Field(
            title="SHA-256",
    ...
    isPartOf:
        Optional[List[MediaObjectPartOf]] = Field(
            title="Is part of",
...
class Dataset(CreativeWork):
    ...
    catalog:
        Optional[Union["DataCatalog", List["DataCatalog"]]] = None
    variablesMeasured:
        Optional[
    ...
    includedInDataCatalog:
        Optional[Union["DataCatalog", List["DataCatalog"]]] = None
    datasetTimeInterval:
        Optional[Union[datetime, List[datetime]]] = None
    distribution:
        Optional[Union["DataDownload", List["DataDownload"]]] = None
...
class DataCatalog(CreativeWork):
    ...
    dataset:
        Optional[Union[Dataset, List[Dataset]]] = None
    measurementTechnique:
        Optional[
            Union[
                str,
