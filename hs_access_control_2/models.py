...
Privilege models determine what objects are accessible to which users. These models:
    ...
    a) determine three kinds of privilege
        ...
        iii) group privilege over resources.

    b) track the provenance of privilege granting to:

        i) allow accounting for what happened.

        ii) allow "undo" operations for users who granted privilege.

        iii) limit each grantor to granting one privilege to one user and resource.
"""
...
class PrivilegeCodes(object):
    ...
    CHANGE = 2
    VIEW = 3
    NONE = 4  # This is a return from subroutines but not a choice in the database
    PRIVILEGE_CHOICES = (
        (OWNER, 'Owner'),
        (CHANGE, 'Change'),
        (VIEW, 'View')
        # (NONE, 'None') : disallow "no privilege" records
    )
######################################
...
# TODO: get_or_create plus transaction control for access records.
...
class CommandCodes(object):
    ...
    This is only used in this module.
    """
    CHECK = 1
    DO = 2
...
class UserGroupPrivilege(models.Model):
    ...
    However, this is currently disallowed and PrivilegeCodes.NONE is only a return value.
    """
...
class GroupResourcePrivilege(models.Model):
    ...
    The group privilege over a resource is never meaningful on its own;
    it is resolved instead into user privilege for each member of
    the group, as listed in UserGroupPrivilege above. A user with any level
    of UserGroupPrivilege is accorded the privileges in GroupResourcePrivilege,
    with the following caveats:

        1) a group cannot own a resource.
    """
