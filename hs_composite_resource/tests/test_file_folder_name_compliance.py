...
class TestAddResourceFiles(MockIRODSTestCaseMixin, unittest.TestCase):
    def setUp(self):
        ...
        self.zip_file_valid = "hs_composite_resource/tests/data/{}".format(
            self.zip_file_name
        )
        self.zip_file_bad_folder_name = "bad-folder.zip"
        self.zip_file_bad_folder = "hs_composite_resource/tests/data/{}".format(
            self.zip_file_bad_folder_name
        )
        self.zip_file_bad_file_name = "bad-file.zip"
        self.zip_file_bad_file = "hs_composite_resource/tests/data/{}".format(
            self.zip_file_bad_file_name
        )
        ...
        self.compliant_non_english_file = open(
            self.compliant_non_english_file_name, "rb"
        )
    ...
    def test_add_compliant_files(self):
        ...
        add_resource_files(
            self.res.short_id,
            self.compliant_file_1,
            self.compliant_file_2,
            self.compliant_file_no_ext,
        ...
        self.assertTrue(
            sanitized_file_name in file_list,
            f"{self.compliant_file_name_1} has not been added",
        )
        self.assertTrue(
            self.compliant_file_name_2 in file_list,
            f"{self.compliant_file_name_2} has not been added",
        )
        self.assertTrue(
            self.compliant_file_name_no_ext in file_list,
            f"{self.compliant_file_name_no_ext} has not been added",
        )
        self.assertTrue(
            self.compliant_non_english_file_name in file_list,
            f"{self.compliant_non_english_file_name} has not been added",
        )

        # check the filename is in english
        self.assertTrue(
            len(self.compliant_file_name_1) == len(self.compliant_file_name_1.encode())
        )
        # check the filename ois not in english
        self.assertFalse(
            len(self.compliant_non_english_file_name)
    ...
    def test_add_non_compliant_files(self):
        ...
        with self.assertRaises(SuspiciousFileOperation):
            add_resource_files(
                self.res.short_id, self.non_compliant_file_1, self.compliant_file_2
            )
        ...
        with self.assertRaises(SuspiciousFileOperation):
            add_resource_files(
                self.res.short_id, self.non_compliant_file_1, self.non_compliant_file_2
            )
        ...
        for bad_test_file in self.bad_files:
            ...
            with self.assertRaises(SuspiciousFileOperation):
                move_or_rename_file_or_folder(
                    self.user, self.res.short_id, src_path, tgt_path
                )
    ...
    def test_meets_preferred_naming_for_files_and_folders(self):
        ...
        non_preferred_symbols = [
            symbol for symbol in string.punctuation if symbol not in skip_symbols
        ]
    ...
    def test_unzip_zipfile_validation(self):
        ...
        file_to_upload = UploadedFile(
            file=open(self.zip_file_valid, "rb"),
            name=os.path.basename(self.zip_file_valid),
        )

        res_zip_file = add_file_to_resource(
            self.res, file_to_upload, check_target_folder=True
        )
        self.assertEqual(self.res.files.count(), 1)
        zip_file_rel_path = os.path.join("data", "contents", res_zip_file.short_path)
        unzip_file(
            self.user, self.res.short_id, zip_file_rel_path, bool_remove_original=False
        )
        ...
        file_to_upload = UploadedFile(
            file=open(self.zip_file_bad_folder, "rb"),
        ...
        file_to_upload = UploadedFile(
            file=open(self.zip_file_bad_file, "rb"),
