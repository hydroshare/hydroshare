...
def landing_page(request, page):
    ...
    if not edit_resource:
        ...
        context['rsmetadata'] = content_model.metadata.program

        # get the helptext for each mp field
        attributes = content_model.metadata.rsmetadata.model._meta.get_fields_with_model()
        attribute_dict = {}
        for att in attributes:
            attribute_dict[att[0].attname] = att[0].help_text
        context["rshelptext"] = attribute_dict

    else:
        output_form = RSForm(instance=content_model.metadata.program,
