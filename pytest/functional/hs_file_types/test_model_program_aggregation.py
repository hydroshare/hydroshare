...
@pytest.mark.parametrize(
    "mp_type", ["software", "computational engine", "documentation", "release notes"]
)
def test_mark_res_file_as_mp_file_type(composite_resource, mp_type, mock_irods):
    ...
    ModelProgramResourceFileType.create(
        file_type=mp_type, res_file=res_file, mp_metadata=mp_aggregation.metadata
    )
    ...
    assert mp_res_file_type.file_type == ModelProgramResourceFileType.type_from_string(
        mp_type
    )
...
def test_mark_multiple_res_files_as_mp_file_type(composite_resource, mock_irods):
    ...
    file_to_upload = UploadedFile(
        file=open(txt_file_path, "rb"), name=os.path.basename(txt_file_path)
    )
    ...
    file_to_upload = UploadedFile(
        file=open(vrt_file_path, "rb"), name=os.path.basename(vrt_file_path)
    )
    ...
    ModelProgramResourceFileType.create(
        file_type=mp_type, res_file=res_file_txt, mp_metadata=mp_aggregation.metadata
    )
    assert ModelProgramResourceFileType.objects.count() == 1
    # set the vrt file as software for this aggregation
    ModelProgramResourceFileType.create(
        file_type=mp_type, res_file=res_file_vrt, mp_metadata=mp_aggregation.metadata
    )
...
def test_mark_res_file_as_mp_file_type_failure_1(composite_resource, mock_irods):
    ...
    # set the res_file as software for this aggregation
    ModelProgramResourceFileType.create(
        file_type="software", res_file=res_file, mp_metadata=mp_aggregation.metadata
    )
    ...
    with pytest.raises(ValidationError):
        ModelProgramResourceFileType.create(
            file_type="engine", res_file=res_file, mp_metadata=mp_aggregation.metadata
        )
...
@pytest.mark.django_db(transaction=True)
def test_delete_res_file_deletes_mp_file_object(
    composite_resource_with_mp_aggregation, mock_irods
):
...
def test_metadata_schema_json_templates(mock_irods):
    ...
    for schema_template in glob.glob(template_path):
        ...
        form_data = {
            "mp_program_type": "Test Model Program",
            "mi_json_schema_template": schema_template,
        }
        metadata_validation_form = ModelProgramMetadataValidationForm(data=form_data)
        assert metadata_validation_form.is_valid()
...
def test_metadata_schema_json_valid_file_upload(mock_irods):
    ...
    assert file_size > 0
    file_to_upload = UploadedFile(
        file=open(schema_file_path, "rb"),
        name=os.path.basename(schema_file_path),
        size=file_size,
    )

    form_data = {"mp_program_type": "Test Model Program"}
    files = {"mi_json_schema_file": file_to_upload}
    metadata_validation_form = ModelProgramMetadataValidationForm(
...
def test_set_metadata(composite_resource_with_mp_aggregation, mock_irods):
    ...
    temp_cov = mp_aggr.metadata.create_element(
    ...
    spatial_cov = mp_aggr.metadata.create_element(
...
def test_move_single_file_aggr_into_model_prog_aggr_failure(
