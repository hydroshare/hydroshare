...
@pytest.mark.parametrize(
    "aggr_cls", [ModelProgramLogicalFile, ModelInstanceLogicalFile]
)
...
def test_create_aggregation_from_file_3(composite_resource, aggr_cls, mock_irods):
    ...
    add_file_to_resource(
        res, file_to_upload, folder=fs_folder, check_target_folder=True
    )
    assert res.files.count() == 1
...
def test_create_aggregation_from_folder_inside_fileset(
    composite_resource, aggr_cls, mock_irods
):
    """test that we can create a model program/instance aggregation from a folder that contains a resource file
    ...
    add_file_to_resource(
        res, file_to_upload, folder=mp_folder_path, check_target_folder=True
    )
...
def test_move_aggregation_to_fileset(
    composite_resource, aggr_folder, aggr_cls, mock_irods
):
    ...
    res_file = add_file_to_resource(
        res, file_to_upload, folder=aggr_folder, check_target_folder=True
    )
    if not aggr_folder:
...
def test_upload_file_to_aggregation_sub_folder(
    composite_resource, aggr_cls, mock_irods
):
    ...
    add_file_to_resource(
        res, file_to_upload, folder=new_sub_folder, check_target_folder=True
    )
    assert res.files.count() == 2
    # both files should be part of the aggregation
    for res_file in res.files.all():
...
def test_create_aggregation_from_folder_failure_1(
    composite_resource, aggr_cls, mock_irods
):
...
def test_create_aggregation_from_folder_failure_2(
    composite_resource, aggr_cls, mock_irods
):
    ...
    add_file_to_resource(
        res, file_to_upload, folder=parent_mp_mi_folder, check_target_folder=True
    )
    file_path = "pytest/assets/logan.vrt"
    file_to_upload = UploadedFile(
        file=open(file_path, "rb"), name=os.path.basename(file_path)
    ...
    add_file_to_resource(
        res, file_to_upload, folder=child_fs_folder, check_target_folder=True
    )
    assert res.files.count() == 2
    assert FileSetLogicalFile.objects.count() == 0
...
def test_create_aggregation_from_folder_failure_3(
    composite_resource, aggr_cls, mock_irods
):
    ...
    add_file_to_resource(
        res, file_to_upload, folder=child_mp_mi_folder, check_target_folder=True
    )
    assert res.files.count() == 2
...
def test_create_fileset_from_folder_failure(composite_resource, aggr_cls, mock_irods):
    ...
    with pytest.raises(ValidationError):
        FileSetLogicalFile.set_file_type(
            resource=res, user=user, folder_path=child_mp_mi_folder
        )
    assert FileSetLogicalFile.objects.count() == 0
    assert not res.dangling_aggregations_exist()
...
def test_move_model_aggr_into_model_aggr_folder_failure(
    composite_resource, aggr_cls, move_type, mock_irods
):
    ...
    add_file_to_resource(
        res, file_to_upload, folder=mp_mi_folder, check_target_folder=True
    )
    assert res.files.count() == 1
    # at this point there should not be any model program/instance aggregation
    assert aggr_cls.objects.count() == 0
    ...
    res_file = add_file_to_resource(
        res, file_to_upload, folder=folder, check_target_folder=True
    )
    if move_type == "file":
...
def test_move_fileset_into_model_aggr_folder_failure(
    composite_resource, aggr_cls, mock_irods
):
...
def test_resource_copy(composite_resource, mock_irods):
    ...
    mi_res_file = add_file_to_resource(
        res, file_to_upload, folder=mp_mi_folder, check_target_folder=True
    )
    file_path = "pytest/assets/logan.vrt"
    file_to_upload = UploadedFile(
    ...
    mp_res_file = add_file_to_resource(
        res, file_to_upload, folder=mp_mi_folder, check_target_folder=True
    )

    assert res.files.count() == 2
    ModelInstanceLogicalFile.set_file_type(
        resource=res, user=user, file_id=mi_res_file.id
    )
    ModelProgramLogicalFile.set_file_type(
        resource=res, user=user, file_id=mp_res_file.id
    )
...
def test_resource_copy_with_aggregations_with_no_files(composite_resource, mock_irods):
    ...
    ModelProgramLogicalFile.set_file_type(
        resource=res, user=user, folder_path=mp_folder
    )
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.parametrize(
    "aggr_to_delete", [ModelProgramLogicalFile, ModelInstanceLogicalFile]
)
def test_resource_copy_after_aggr_delete(
    composite_resource, aggr_to_delete, mock_irods
):
    ...
    if aggr_to_delete == ModelProgramLogicalFile:
        assert ModelInstanceLogicalFile.objects.filter(resource=copy_res).count() == 1
        copy_mi_aggr = ModelInstanceLogicalFile.objects.filter(
            resource=copy_res
        ).first()
        assert copy_mi_aggr.metadata_schema_json
        assert copy_mi_aggr.metadata.metadata_json
