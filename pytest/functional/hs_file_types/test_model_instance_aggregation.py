...
def test_link_model_aggregations_same_resource(
    composite_resource_with_mi_aggregation, mock_irods
):
    ...
    file_to_upload = UploadedFile(
        file=open(file_path, "rb"), name=os.path.basename(file_path)
    )
    ...
    mi_validation_form = ModelInstanceMetadataValidationForm(
        data={"executed_by": mp_aggr.id}, user=user, resource=res
    )
...
@pytest.mark.django_db(transaction=True)
def test_model_instance_on_model_program_delete(
    composite_resource_with_mi_aggregation, mock_irods
):
...
@pytest.mark.django_db(transaction=True)
def test_model_instance_on_model_program_rename_1(
    composite_resource_with_mi_aggregation, mock_irods
):
...
@pytest.mark.django_db(transaction=True)
def test_model_instance_on_model_program_rename_2(
    composite_resource_with_mi_aggregation, mock_irods
):
...
def test_set_metadata(composite_resource_with_mi_aggregation, mock_irods):
    ...
    temp_cov = mi_aggr.metadata.create_element(
    ...
    spatial_cov = mi_aggr.metadata.create_element(
...
@pytest.mark.django_db(transaction=True)
def test_auto_netcdf_aggregation_creation(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
    ...
    _add_files_to_resource(
        resource=resource, files_to_add=[netcdf_file_path], upload_folder=mi_aggr_path
    )
    ...
    nc_res_file = ResourceFile.get(
        resource=resource, file=nc_file_name, folder=mi_aggr_path
    )
...
@pytest.mark.django_db(transaction=True)
def test_auto_raster_aggregation_creation(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
    ...
    _add_files_to_resource(
        resource=resource, files_to_add=[raster_file_path], upload_folder=mi_aggr_path
    )
    ...
    raster_res_file = ResourceFile.get(
        resource=resource, file=raster_file_name, folder=mi_aggr_path
    )
...
@pytest.mark.django_db(transaction=True)
def test_auto_geofeature_aggregation_creation(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
    ...
    _add_files_to_resource(
        resource=resource, files_to_add=geo_feature_files, upload_folder=mi_aggr_path
    )
    ...
    shp_res_file = ResourceFile.get(
        resource=resource, file=shp_file_name, folder=mi_aggr_path
    )
...
@pytest.mark.django_db(transaction=True)
def test_auto_timeseries_aggregation_creation(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
    ...
    _add_files_to_resource(
        resource=resource, files_to_add=[sqlite_file_path], upload_folder=mi_aggr_path
    )
    ...
    sqlite_res_file = ResourceFile.get(
        resource=resource, file=sqlite_file_name, folder=mi_aggr_path
    )
...
@pytest.mark.django_db(transaction=True)
def test_auto_ref_timeseries_aggregation_creation(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
    ...
    _add_files_to_resource(
        resource=resource,
        files_to_add=[ref_timeseries_file_path],
        upload_folder=mi_aggr_path,
    )

    # there should be 2 resource files
    assert resource.files.all().count() == 2
    # the json file added to the model instance folder should be part of a new ref timeseries aggregation
    ref_ts_res_file = ResourceFile.get(
        resource=resource, file=ref_timeseries_file_name, folder=mi_aggr_path
    )
...
@pytest.mark.django_db(transaction=True)
def test_canot_create_fileset_within_mi_aggregation(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
    ...
    _add_files_to_resource(
        resource=resource, files_to_add=[file_path], upload_folder=fs_folder_path
    )
...
@pytest.mark.django_db(transaction=True)
def test_canot_create_mi_aggregation_within_mi_aggregation(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
    ...
    _add_files_to_resource(
        resource=resource, files_to_add=[file_path], upload_folder=mi_sub_folder_path
    )
    # trying to set folder to model instance should fail
    assert ModelInstanceLogicalFile.objects.count() == 1
    with pytest.raises(ValidationError):
        ModelInstanceLogicalFile.set_file_type(
            resource, user, folder_path=mi_sub_folder_path
        )
...
@pytest.mark.django_db(transaction=True)
def test_move_single_file_aggr_into_model_instance_aggregation(
    composite_resource, mock_irods
):
    ...
    ModelInstanceLogicalFile.set_file_type(
        resource=res, user=user, folder_path=mi_folder
    )
...
@pytest.mark.django_db(transaction=True)
def test_update_spatial_coverage_from_children(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
...
@pytest.mark.django_db(transaction=True)
def test_no_auto_update_spatial_coverage_from_children(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
...
@pytest.mark.django_db(transaction=True)
def test_auto_update_temporal_coverage_from_children(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
    ...
    for temp_date in ("start", "end"):
        assert (
            mi_aggr.metadata.temporal_coverage.value[temp_date]
...
@pytest.mark.django_db(transaction=True)
def test_no_auto_update_temporal_coverage_from_children(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
...
@pytest.mark.django_db(transaction=True)
def test_update_temporal_coverage_from_children(
    composite_resource_with_mi_aggregation_folder, mock_irods
):
...
def _add_files_to_resource(resource, files_to_add, upload_folder=None):
    ...
    added_resource_files = add_resource_files(
        resource.short_id, *files_to_upload, folder=upload_folder
    )
