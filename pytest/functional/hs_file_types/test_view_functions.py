...
def test_create_model_aggregation_from_file(composite_resource, aggr_type, mock_s3):
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.parametrize('aggr_type', ['ModelProgram', 'ModelInstance'])
def test_create_model_aggregation_from_folder(composite_resource, aggr_type, mock_s3):
    res, user = composite_resource
    file_path = 'pytest/assets/generic_file.txt'
...
@pytest.mark.django_db(transaction=True)
def test_update_csv_aggregation_table_schema(composite_resource, mock_s3):
    res, user = composite_resource
...
@pytest.mark.django_db(transaction=True)
def test_update_model_program_metadata(composite_resource_with_mp_aggregation, mock_s3):
...
@pytest.mark.django_db(transaction=True)
def test_update_model_instance_metadata(composite_resource_with_mi_aggregation, mock_s3):
...
@pytest.mark.django_db(transaction=True)
def test_update_model_instance_metadata_json(composite_resource_with_mi_mp_aggregation, mock_s3):
...
def test_move_aggr_into_model_instance_aggr(composite_resource_with_mi_aggregation_folder, move_aggr_cls, mock_s3):
