...
@pytest.mark.django_db(transaction=True)
@pytest.mark.parametrize("aggr_type", ["ModelProgram", "ModelInstance"])
def test_create_model_aggregation_from_folder(
    composite_resource, aggr_type, mock_irods
):
    res, user = composite_resource
    file_path = "pytest/assets/generic_file.txt"
    ...
    assert aggr_class.objects.count() == 0
    hs_file_type = aggr_type
    url_params = {"resource_id": res.short_id, "hs_file_type": hs_file_type}
    ...
    # this is the view function we are testing
    response = set_file_type(
        request, resource_id=res.short_id, hs_file_type=hs_file_type
    )
    assert response.status_code == status.HTTP_201_CREATED
    assert aggr_class.objects.count() == 1
    assert not res.dangling_aggregations_exist()
...
@pytest.mark.django_db(transaction=True)
def test_update_model_program_metadata(
...
@pytest.mark.django_db(transaction=True)
def test_update_model_instance_metadata(
...
@pytest.mark.django_db(transaction=True)
def test_update_model_instance_metadata_json(
    composite_resource_with_mi_mp_aggregation, mock_irods
):
...
def test_move_aggr_into_model_instance_aggr(
    composite_resource_with_mi_aggregation_folder, move_aggr_cls, mock_irods
):
