...
def test_search_term_found_in_title(admin_client, public_resource_with_metadata):
    ...
    djangoresponse = admin_client.get(
        "/discoverapi/?q={}".format(search_term), follow=True
    )
...
def test_bad_filter_json_format(admin_client, public_resource_with_metadata):
    ...
    djangoresponse = admin_client.get(
        "/discoverapi/?filter={}".format(query_filter), follow=True
    )
    response = json.loads(djangoresponse.content.decode("utf-8"))
...
@pytest.mark.skip(reason="skipping for now until a fix for the view function is made")
@pytest.mark.django_db
def test_filter_by_atribute(
    admin_client, public_resource_with_metadata, private_resource_with_metadata
):
    """
    ...
    djangoresponse = admin_client.get(
        "/discoverapi/?filter={}".format(json.dumps(query_filter)), follow=True
    )
    response = json.loads(djangoresponse.content.decode("utf-8"))
...
def test_filter_returns_empty_results(admin_client, public_resource_with_metadata):
    ...
    query_filter = {
        "author": ["{}".format(str(uuid.uuid4()))],
        "owner": ["{}".format(str(uuid.uuid4()))],
    }
    djangoresponse = admin_client.get(
        "/discoverapi/?filter={}".format(json.dumps(query_filter)), follow=True
    )
