  ...
  methods: {
      ...
      try {
        let leafletLayer = L.geoJSON(geojson, {
          ...
          pointToLayer: function (feature, latlng) {
            return marker ? L.marker(latlng, style) : L.circleMarker(latlng, style);
          },
    ...
    updateAppWithResSpatialExtent() {
      ...
      if (geoconnexApp.resSpatialType == "point") {
        ...
        geoconnexApp.pointLat = parseFloat($("#id_north").val()) || parseFloat($('#cov_north').text().replace(/[^\d.-]/g, ''));
        geoconnexApp.pointLong = parseFloat($("#id_east").val()) || parseFloat($('#cov_east').text().replace(/[^\d.-]/g, ''));
