...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "output_file_name_with_path", help="output file name with path"
        )

    def handle(self, *args, **options):
        quota_report_list = []
        for uq in UserQuota.objects.filter(user__is_active=True).filter(
            ...
            if not math.isclose(used_value, uq.used_value, abs_tol=0.1):
                ...
                print(
                    "quota incosistency: {} reported in django vs {} reported in iRODS for user {}".format(
                        uq.used_value, used_value, uq.user.username
                    ),
                    flush=True,
                )
        ...
        if quota_report_list:
            with open(options["output_file_name_with_path"], "w") as csvfile:
                ...
                fields = ["User" "Quota reported in Django", "Quota reported in iRODS"]
                w.writerow(fields)

                for q in quota_report_list:
                    values = [q["user"], q["django"], q["irods"]]
