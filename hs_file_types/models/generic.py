...
class GenericFileMetaDataMixin(AbstractFileMetaData):
    ...
    def create_element(self, element_model_name, **kwargs):
        element = super(GenericFileMetaDataMixin, self).create_element(
            element_model_name, **kwargs
        )
        self.is_dirty = True
        return element
    ...
    def has_modified_metadata(self):
        ...
        if (
            self.logical_file.dataset_name
            and file_name != self.logical_file.dataset_name
        ):
            return True
        return False
    ...
    def get_html(self, **kwargs):
        ...
        if not self.has_metadata:
            no_metadata_message = div(
                id="#fb-metadata-default", cls="text-center text-muted", role="alert"
            )
    ...
    def get_html_forms(
        self, dataset_name_form=True, temporal_coverage=True, render=True, **kwargs
    ):
        """overrides the base class function"""
        ...
        if not skip_coverage:
            ...
            if self.temporal_coverage or self.spatial_coverage:
                if self.temporal_coverage:
                    ...
                    temp_action = create_action.format(
                        logical_file_class_name, self.logical_file.id, element_name
                    )
                    temp_cov_form.action = temp_action
                ...
                if self.spatial_coverage:
                    ...
                    spatial_action = create_action.format(
                        logical_file_class_name, self.logical_file.id, element_name
                    )
                    spatial_cov_form.action = spatial_action
            else:
                action = create_action.format(
                    logical_file_class_name, self.logical_file.id, element_name
                )
...
class GenericLogicalFile(AbstractLogicalFile):
    ...
    metadata = models.OneToOneField(
        GenericFileMetaData, on_delete=models.CASCADE, related_name="logical_file"
    )
    ...
    def set_file_type(cls, resource, user, file_id=None, folder_path="", extra_data={}):
        ...
        with FileTypeContext(
            ...
            log.info(
                "Generic aggregation was created for file:{}.".format(
