...
class OriginalCoverageGeofeature(AbstractMetaDataElement):
        ...
        for _, _, cov in graph.triples((subject, cls.get_class_term(), None)):
            ...
            for key_value in value.split(";"):
                ...
                if k == "units":
                    ...
                    value_dict[k] = v
            OriginalCoverageGeofeature.create(
                **value_dict, content_object=content_object
            )
    ...
    def get_html_form(cls, resource, element=None, allow_edit=True, file_type=False):
        ...
        orig_coverage_form = OriginalCoverageGeofeatureForm(
...
class GeographicFeatureMetaDataMixin(models.Model):
    ...
    def get_supported_element_names(cls):
        ...
        elements = super(
            GeographicFeatureMetaDataMixin, cls
    ...
    def get_required_missing_elements(self):  # show missing required meta
        missing_required_elements = super(
            GeographicFeatureMetaDataMixin, self
        ).get_required_missing_elements()
        if not (
            self.coverages.all().filter(type="box").first()
...
class GeoFeatureFileMetaData(GeographicFeatureMetaDataMixin, AbstractFileMetaData):
    ...
    def get_html_forms(self, datatset_name_form=True, **kwargs):
        ...
        update_action = (
            "/hsapi/_internal/GeoFeatureLogicalFile/{0}/{1}/{2}/update-file-metadata/"
        )
        create_action = (
            "/hsapi/_internal/GeoFeatureLogicalFile/{0}/{1}/add-file-metadata/"
        )
    ...
    def get_geometry_information_form(self):
        return GeometryInformation.get_html_form(
            resource=None,
            element=self.geometryinformation,
            file_type=True,
            allow_edit=False,
        )

    def get_original_coverage_form(self):
        return OriginalCoverageGeofeature.get_html_form(
            resource=None,
...
class GeoFeatureLogicalFile(AbstractLogicalFile):
    metadata = models.OneToOneField(
        GeoFeatureFileMetaData, on_delete=models.CASCADE, related_name="logical_file"
    )
    ...
    def get_aggregation_display_name():
        return (
            "Geographic Feature Content: The multiple files that are part of a geographic "
            "shapefile"
        )
        ...
        with FileTypeContext(
            ...
            with transaction.atomic():
                try:
                    ...
                    log.info(
                        "GeoFeature aggregation - files were added to the aggregation."
                    )
                    add_metadata(resource, meta_dict, xml_file, logical_file)
                    log.info(
                        "GeoFeature aggregation and resource level metadata updated."
                    )
    ...
    def create_aggregation_xml_documents(self, create_map_xml=True):
        super(GeoFeatureLogicalFile, self).create_aggregation_xml_documents(
            create_map_xml
        )
...
def extract_metadata_and_files(resource, res_file, file_type=True):
    ...
    """
    shape_files, shp_res_files = get_all_related_shp_files(
        resource, res_file, file_type=file_type
    )
    temp_dir = os.path.dirname(shape_files[0])
    if not _check_if_shape_files(shape_files):
        if res_file.extension.lower() == ".shp":
            err_msg = (
                "There was a problem parsing the component files associated with "
            ...
            err_msg = (
                "One or more dependent shape files are missing in the selected zip file "
...
def add_metadata(resource, metadata_dict, xml_file, logical_file=None):
    ...
    if "coverage" in list(metadata_dict.keys()):
        ...
        target_obj.metadata.create_element(
            "coverage", type=coverage_dict["type"], value=coverage_dict["value"]
        )
    ...
    target_obj.metadata.create_element(
        "originalcoveragegeofeature", **originalcoverage_dict
    )
    ...
    target_obj.metadata.create_element(
        "geometryinformation", **geometryinformation_dict
    )
    if xml_file:
            ...
            if "description" in shp_xml_metadata:
                ...
                if title_element.value.lower() == "untitled resource":
                    resource.metadata.update_element(
                        "title", title_element.id, value=title
                    )
    ...
    if selected_resource_file.extension.lower() == ".shp":
        for f in resource.files.all():
            if f.file_folder == selected_resource_file.file_folder:
                if f.extension.lower() == ".xml" and not f.file_name.lower().endswith(
                    ".shp.xml"
                ):
                    continue
                if (
        ...
        for f in shape_res_files:
            temp_file = utils.get_file_from_irods(
                resource=resource, file_path=f.storage_path
            )
            if not temp_dir:
        ...
        temp_file = utils.get_file_from_irods(
            resource=resource, file_path=selected_resource_file.storage_path
        )
        temp_dir = os.path.dirname(temp_file)
...
def _check_if_shape_files(files, temp_files=True):
    ...
    if len(files) >= 3:
        ...
        if temp_files:
            ...
            file_extensions = set(
                [os.path.splitext(os.path.basename(f).lower())[1] for f in files]
            )
        else:
            # files are in db
        ...
        if temp_files:
            ...
            file_names = set(
                [
                    ...
                    for f in files
                    if not f.lower().endswith(".shp.xml")
                ]
            )
        else:
            # files are in db
            file_names = set(
                [
                    os.path.splitext(os.path.basename(f.file_name))[0]
                    for f in files
                    if not f.file_name.lower().endswith(".shp.xml")
                ]
            )
        if len(file_names) > 1:
...
def extract_metadata(shp_file_full_path):
    ...
    try:
        ...
        original_coverage_dict["originalcoverage"] = {
        ...
        for field_name in field_name_list:
            ...
            field_info_dict_item["fieldinformation"] = parsed_md_dict[
                "field_meta_dict"
            ]["field_attr_dict"][field_name]
        ...
        # geometry
        geometryinformation = {
            "featureCount": parsed_md_dict["feature_count"],
