...
class ModelInstanceFileMetaData(GenericFileMetaDataMixin):
        ...
        with executed_by_div:
            ...
            if self.executed_by:
                ...
                if self.logical_file.resource.short_id != mp_aggr.resource.short_id:
                    hs_res_url = os.path.join(
                        current_site_url(), "resource", mp_aggr.resource.short_id
                    )
                    display_string = (
                        "HydroShare resource that contains the Model Program:"
                    )
                    ...
                    with dom_tags.p(aggr_path):
                        dom_tags.span(
                            mp_aggr.aggregation_name, style="font-weight: bold;"
                        )
                else:
                    display_string = "{} ({})".format(
                        mp_aggr.aggregation_name, mp_aggr.dataset_name
                    )
                    dom_tags.p(display_string)
            else:
            ...
            with metadata_json_div:
                ...
                for k, v in self.metadata_json.items():
                    if type(v) not in (int, float, bool):
                        ...
                        k_title = k
                        if metadata_schema:
                            root_properties_schema_node = metadata_schema[
                    ...
                    with dom_tags.div(cls="row"):
                        ...
                        def display_dict_type_value(value):
                            for child_k, child_v in value.items():
                                ...
                                if metadata_schema:
                                    child_properties_schema_node = (
                                        root_properties_schema_node[k]
                                    )
                                    if "type" in child_properties_schema_node:
                                        if (
                                            child_properties_schema_node["type"]
                                            == "array"
                                        ):
                                            child_properties_schema_node = (
                                                child_properties_schema_node["items"]
                                            )
                                    child_properties_schema_node = (
                                        child_properties_schema_node[
                                            schema_properties_key
                                    ...
                                    if child_k in child_properties_schema_node:
                                        child_k_title = child_properties_schema_node[
                                            child_k
                                        ]["title"]

                                display_json_meta_field(
                                    field_name=child_k_title, field_value=child_v
                                )
    ...
    def get_html_forms(self, dataset_name_form=True, temporal_coverage=True, **kwargs):
        ...
        base_div, context = super(ModelInstanceFileMetaData, self).get_html_forms(
            render=False
        )
        ...
        def get_schema_based_form():
            ...
            with schema_div:
                with dom_tags.form(
                    id="id-schema-based-form",
                    action=json_form_action,
                    method="post",
                    ...
                    with dom_tags.fieldset():
                        ...
                        if invalid_metadata:
                            dom_tags.div(
                                "Existing metadata is invalid as per the schema",
                        ...
                        dom_tags.input(
                            value=json_schema, id="id-json-schema", type="hidden"
                        )
                        dom_tags.input(
                            value=json_data,
                            id="id-metadata-json",
        ...
        def get_executed_by_form():
                    ...
                    if self.executed_by and self.executed_by.metadata_schema_json:
                        ...
                        try:
                            jsonschema.Draft4Validator(
                                self.executed_by.metadata_schema_json
                            ).validate(self.metadata_json)
            ...
            with executed_by_div:
                dom_tags.label(
                    "Select a Model Program", fr="id_executed_by", cls="control-label"
                )
                with dom_tags.select(
                    cls="form-control", id="id_executed_by", name="executed_by"
                ):
                    ...
                    for mp_aggr in this_resource.modelprogramlogicalfile_set.all():
                        ...
                        if self.executed_by:
                            if self.executed_by.id == mp_aggr.id:
                                dom_tags.option(
                                    option, selected="selected", value=mp_aggr.id
                                )
                            else:
                    ...
                    if self.executed_by:
                        ...
                        # migrated mi resource)
                        if (
                            self.logical_file.resource.short_id
                            != mp_aggr.resource.short_id
                        ):
                            option = "{} ({})".format(
                                mp_aggr.aggregation_name, mp_aggr.dataset_name
                            )
                ...
                if self.executed_by:
                    ...
                    if self.logical_file.resource.short_id != mp_aggr.resource.short_id:
                        ...
                        external_mp_aggr_msg = (
                            "Selected model program exists in a different resource. "
                ...
                if invalid_schema:
                    dom_tags.div(
                        "Metadata schema in the associated model program fails to validate existing metadata. "
                        "Updating the schema from model program will lead to loss of all schema based "
                        "metadata in this model instance.",
                        cls="alert alert-danger",
                        id="div-invalid-schema-message",
                    )
                if self.executed_by and self.executed_by.metadata_schema_json:
                    if DeepDiff(
                        self.logical_file.metadata_schema_json,
                        self.executed_by.metadata_schema_json,
                    ):
                        schema_update_url = (
                            "/hsapi/_internal/{}/update-modelinstance-meta-schema/"
                        )
                        schema_update_url = schema_update_url.format(
                        ...
                        dom_tags.button(
                            "Update Metadata Schema from Model Program",
                            type="button",
                            id="btn-mi-schema-update",
                            data_schema_update_url=schema_update_url,
                            cls=btn_cls,
                        )
                if self.logical_file.metadata_schema_json:
                    dom_tags.button(
                        "Show/Hide Model Instance Metadata JSON Schema",
                        type="button",
                        cls="btn btn-success btn-block",
                        data_toggle="collapse",
                        data_target="#meta-schema",
                    )
                    mi_schema_div = dom_tags.div(
                        cls="content-block collapse",
                        id="meta-schema",
                        style="margin-top:10px; padding-bottom: 20px;",
                    )
                    with mi_schema_div:
                        json_schema = json.dumps(
                            self.logical_file.metadata_schema_json, indent=4
                        )
                        dom_tags.textarea(
                            json_schema,
                            readonly=True,
                ...
                if (
                    self.executed_by
                    and not self.executed_by.metadata_schema_json
                    and not self.logical_file.metadata_schema_json
                ):
                    missing_schema_msg = (
                        "Selected model program is missing metadata schema. With the current "
                    ...
                    with dom_tags.fieldset(cls="fieldset-border"):
                        with dom_tags.div(cls="form-group"):
                            with dom_tags.div(cls="control-group"):
                                    ...
                                    with dom_tags.div(cls="control-group"):
                                        with dom_tags.div(cls="controls"):
                                            with dom_tags.label(
                                                ...
                                                if (
                                                    self.logical_file.metadata.has_model_output
                                                ):
                                                    dom_tags.input(
                                                        type="radio",
                                                        id="id_mi_includes_output_yes",
                                ...
                                with dom_tags.div(
                                    id="mi_executed_by", cls="control-group"
                                ):
                                    with dom_tags.div(cls="controls"):
                                        dom_tags.legend(
    ...
    def get_rdf_graph(self):
        ...
        graph.add(
            (subject, HSTERMS.includesModelOutput, Literal(self.has_model_output))
        )
        ...
        if self.executed_by:
            if self.executed_by:
                ...
                hs_res_url = os.path.join(
                    current_site_url(), "resource", resource.file_path
                )
                aggr_url = (
                    os.path.join(hs_res_url, self.executed_by.map_short_file_path)
                    + "#aggregation"
                )
        ...
        if self.logical_file.metadata_schema_json:
            graph.add(
                (
                    subject,
                    HSTERMS.modelProgramSchema,
                    URIRef(self.logical_file.schema_file_url),
                )
            )
        ...
        if self.metadata_json:
            graph.add(
                (
                    subject,
                    HSTERMS.modelProgramSchemaValues,
                    URIRef(self.logical_file.schema_values_file_url),
                )
            )
    ...
    def ingest_metadata(self, graph):
        ...
        has_model_output = graph.value(
            subject=subject, predicate=HSTERMS.includesModelOutput
        )
        if has_model_output:
            self.has_model_output = str(has_model_output).lower() == "true"
            self.save()

        executed_by = graph.value(
            subject=subject, predicate=HSTERMS.executedByModelProgram
        )
        if executed_by:
            aggr_map_path = executed_by.split("/resource/", 1)[1].split("#")[0]
            mp_aggr = get_logical_file_by_map_file_path(
                self.logical_file.resource, ModelProgramLogicalFile, aggr_map_path
            )
        ...
        schema_values_file = graph.value(
            subject=subject, predicate=HSTERMS.modelProgramSchemaValues
        )
...
class ModelInstanceLogicalFile(NestedLogicalFileMixin, AbstractModelLogicalFile):
    ...
    metadata = models.OneToOneField(
        ModelInstanceFileMetaData, on_delete=models.CASCADE, related_name="logical_file"
    )
    ...
    def create(cls, resource):
        ...
        mi_metadata = ModelInstanceFileMetaData.objects.create(
    ...
    def create_aggregation_xml_documents(self, create_map_xml=True):
        super(ModelInstanceLogicalFile, self).create_aggregation_xml_documents(
            create_map_xml=create_map_xml
        )
        for child_aggr in self.get_children():
    ...
    def can_contain_aggregation(self, aggregation):
        ...
        if (
            aggregation.is_model_instance
            or aggregation.is_model_program
            or aggregation.is_fileset
        ):
            return False
        return True
    ...
    def add_resource_files_in_folder(self, resource, folder):
        ...
        res_files = ResourceFile.list_folder(
            resource=resource, folder=folder, sub_folders=True
        )
    ...
    def get_copy(self, copied_resource):
        ...
        copy_of_logical_file = super(ModelInstanceLogicalFile, self).get_copy(
            copied_resource
        )
        ...
        if self.metadata.executed_by:
            ...
            if src_executed_by in src_mp_logical_files:
                ...
                for tgt_mp_logical_file in tgt_mp_logical_files:
                    if (
                        src_executed_by.aggregation_name
                        == tgt_mp_logical_file.aggregation_name
                    ):
    ...
    def logical_delete(self, user, delete_res_files=True):
        ...
        super(ModelInstanceLogicalFile, self).logical_delete(
            user, delete_res_files=delete_res_files
        )
