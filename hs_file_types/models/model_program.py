...
class ModelProgramResourceFileType(models.Model):
    ...
    def create(cls, **kwargs):
        ...
        if res_file not in logical_file.files.all():
            raise ValidationError(
                f"Resource file {res_file} is not part of the aggregation"
            )
        ...
        if mp_metadata.mp_file_types.filter(res_file=res_file).exists():
            raise ValidationError(
                f"Resource file {res_file} is already set to model program file type"
            )
    ...
    def type_from_string(cls, type_string):
        ...
        """
        type_map = {
            "release notes": cls.RELEASE_NOTES,
            "documentation": cls.DOCUMENTATION,
            "software": cls.SOFTWARE,
            "computational engine": cls.ENGINE,
        }
    ...
    def get_xml_name(self):
        xml_name_map = {
            self.RELEASE_NOTES: HSTERMS.modelReleaseNotes,
...
class ModelProgramFileMetaData(GenericFileMetaDataMixin):
    ...
    def get_rdf_graph(self):
        ...
        for mp_file_type in self.mp_file_types.all():
            ...
            graph.add(
                (
                    subject,
                    mp_file_type_xml_name,
                    URIRef(site_url + mp_file_type.res_file.url),
                )
            )
        ...
        if self.release_date:
            graph.add(
                (
                    subject,
                    HSTERMS.modelReleaseDate,
                    Literal(self.release_date.isoformat()),
                )
            )
        if self.website:
        ...
        if self.code_repository:
            graph.add(
                (subject, HSTERMS.modelCodeRepository, URIRef(self.code_repository))
            )
        if self.programming_languages:
            for model_program_languages in self.programming_languages:
                graph.add(
                    (
                        subject,
                        HSTERMS.modelProgramLanguage,
                        Literal(model_program_languages),
                    )
                )
    ...
    def ingest_metadata(self, graph):
        ...
        xml_name_map = {
            "release notes": HSTERMS.modelReleaseNotes,
                ...
                try:
                    ...
                    if not ModelProgramResourceFileType.objects.filter(
                        ...
                        ModelProgramResourceFileType.create(
                            file_type=mp_file_type, res_file=file, mp_metadata=self
                        )
    ...
    def get_html(self, include_extra_metadata=True, **kwargs):
        ...
        if self.code_repository:
            ...
            with code_repo_div:
                ...
                dom_tags.a(
                    self.code_repository, href=self.code_repository, target="_blank"
                )
            ...
            with mp_files_div:
                ...
                for mp_file in self.mp_file_types.all():
                    with dom_tags.div(cls="row"):
                        ...
                        with dom_tags.div(cls="col-md-6"):
                            mp_file_type_name = (
                                ModelProgramResourceFileType.type_name_from_type(
                                    mp_file.file_type
                                )
                            )
    ...
    def get_html_forms(self, dataset_name_form=True, temporal_coverage=True, **kwargs):
        ...
        base_div, context = super(ModelProgramFileMetaData, self).get_html_forms(
            render=False, skip_coverage=True
        )
        ...
        def get_html_form_mp_file_types():
            ...
            with res_file_options_div:
                dom_tags.input(
                    type="text", name="mp_file_types", value="", style="display: None;"
                )
            for res_file in aggregation.files.all():
                with dom_tags.div(cls="row file-row"):
                    ...
                    with dom_tags.div(cls="col-md-6"):
                        mp_file_type_obj = self.mp_file_types.filter(
                            res_file=res_file
                        ).first()
                                ...
                                if mp_file_type_obj is not None:
                                    if mp_file_type_obj.file_type == mp_file_type[0]:
                                        dom_tags.option(
                                            mp_file_type_name,
                                            selected="selected",
                                            value=mp_file_type_name,
                                        )
                                    else:
                                        dom_tags.option(
                                            mp_file_type_name, value=mp_file_type_name
                                        )
                                else:
                                    dom_tags.option(
                                        mp_file_type_name, value=mp_file_type_name
                    ...
                    with dom_tags.fieldset(cls="fieldset-border"):
                        ...
                        with dom_tags.div(cls="form-group"):
                            with dom_tags.div(cls="control-group"):
                                dom_tags.label(
                                    "Version", cls="control-label", fr="file_version"
                                )
                                ...
                                dom_tags.label(
                                    "Website", fr="file_website", cls="control-label"
                                )
                                with dom_tags.div(cls="controls"):
                                    if self.website:
                                ...
                                dom_tags.label(
                                    ...
                                    fr="file_programming_languages",
                                    cls="control-label",
                                )
                                with dom_tags.div(cls="controls"):
                                    programming_languages = (
                                        self.programming_languages_as_string
                                    )
                                    dom_tags.input(
                                        value=programming_languages,
                                        cls="form-control input-sm textinput textInput",
                                        id="file_programming_languages",
                                        maxlength="250",
                                ...
                                with dom_tags.div(cls="controls"):
                                    ...
                                    with dom_tags.fieldset(cls="fieldset-border"):
                                        with dom_tags.legend(
                                            "Model Instance Metadata Schema",
                                            cls="legend-border",
                                            style="font-size: 14px; font-weight:bold;",
                                        ):
                                            dom_tags.span(
                                                cls="glyphicon glyphicon-info-sign text-muted",
                                                data_toggle="tooltip",
                                                data_placement="auto",
                                        ...
                                        with dom_tags.div(cls="form-group"):
                                            with dom_tags.select(
                                                cls="form-control",
                                                name="mi_json_schema_template",
                                                style="margin-top:10px;",
                                            ):
                                                dom_tags.option(
                                                    "Select a schema", value=""
                                                )
                                                template_path = (
                                                    settings.MODEL_PROGRAM_META_SCHEMA_TEMPLATE_PATH
                                                )
                                                template_path = os.path.join(
                                                    template_path, "*.json"
                                                )
                                                for schema_template in glob.glob(
                                                    template_path
                                                ):
                                                    template_file_name = (
                                                        os.path.basename(
                                            ...
                                            with dom_tags.div(cls="row file-row"):
                                                with dom_tags.div(cls="col-md-12"):
                                                    ...
                                                    dom_tags.input(
                                                        type="file",
                                                        accept=".json",
                                                        name="mi_json_schema_file",
                                                        id="mi-json-schema-file",
                                                    )
                                        ...
                                        if json_schema:
                                            ...
                                            dom_tags.div(
                                                ...
                                                id="div-missing-schema-message",
                                            )

                            with dom_tags.div(
                                id="mp_content_files", cls="control-group"
                            ):
                                with dom_tags.div(cls="controls"):
                                    dom_tags.legend("Content Files")
...
class ModelProgramLogicalFile(AbstractModelLogicalFile):
    ...
    metadata = models.OneToOneField(
        ModelProgramFileMetaData, on_delete=models.CASCADE, related_name="logical_file"
    )
    ...
    def create(cls, resource):
        ...
        mp_metadata = ModelProgramFileMetaData.objects.create(
    ...
    def validate_meta_schema(meta_schema):
        ...
        if json_schema:
            ...
            if not schema_version:
                ...
                if "$schema" not in json_schema:
                    ...
                    err_message = err_message.format(
                        "Key '$schema' is missing a value for schema version"
                    )
                ...
                if "/draft-04/" not in schema_version:
                    is_schema_valid = False
                    err_message = (
                        "Not a valid JSON schema. Schema version is invalid. Supported valid version(s): "
                        "draft-04"
                    )
                    validation_errors.append(err_message)
            ...
            if "properties" not in json_schema:
                ...
                validation_errors.append(
                    "Not a valid metadata schema. Attribute 'properties' is missing"
                )
            ...
            if is_schema_valid:
                try:
                    ...
                    is_schema_valid = False
                    schema_err_msg = (
                        f"{ex.message}. Schema invalid field path:{str(list(ex.path))}"
                    )
        ...
        if is_schema_valid:
            ...
            if "additionalProperties" not in json_schema:
                is_schema_valid = False
                validation_errors.append(
                    "Not a valid metadata schema. Attribute 'additionalProperties' is missing"
                )
                    ...
                    if isinstance(v, dict):
                        if k not in ("properties", "items"):
                            ...
                            if v["type"] == "array":
                                ...
                                if "format" not in v:
                                    ...
                                    msg = (
                                        f"Not a valid metadata schema. Attribute 'format' should be set "
                                        f"to table for {k}"
                                    )
                                    validation_errors.append(msg)
                        ...
                        if "type" in v and v["type"] == "object":
                            ...
                            if "additionalProperties" not in v:
                                msg = (
                                    "Not a valid metadata schema. Attribute 'additionalProperties' is "
                                    f"missing for {k}"
                                )
                                validation_errors.append(msg)
                            elif v["additionalProperties"]:
                                msg = (
                                    "Not a valid metadata schema. Attribute 'additionalProperties' must "
                        ...
                        if "type" in v and v["type"] == "object":
                            ...
                            for k_child, v_child in v.items():
                                if isinstance(v_child, dict):
                                    if (
                                        "type" in v_child
                                        and v_child["type"] == "object"
                                    ):
                                        msg = (
                                            "Not a valid metadata schema. Nested object types are not allowed. "
        ...
        for mp_file_type in self.metadata.mp_file_types.all():
            ...
            for res_file in tgt_logical_file.files.all():
                if res_file.short_path == mp_file_type.res_file.short_path:
                    ...
                    break
            if mp_res_file:
                ModelProgramResourceFileType.objects.create(
                    file_type=mp_file_type.file_type,
                    res_file=mp_res_file,
    ...
    def get_copy(self, copied_resource):
        ...
        copy_of_logical_file = super(ModelProgramLogicalFile, self).get_copy(
            copied_resource
        )
        copy_of_logical_file.metadata.version = self.metadata.version
        copy_of_logical_file.metadata.programming_languages = (
            self.metadata.programming_languages
        )
        copy_of_logical_file.metadata.operating_systems = (
            self.metadata.operating_systems
        )
    ...
    def create_aggregation_xml_documents(self, create_map_xml=True):
        super(ModelProgramLogicalFile, self).create_aggregation_xml_documents(
