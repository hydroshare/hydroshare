    ...
    def clean_executed_by(self):
        ...
        if executed_by is not None and executed_by > 0:
            ...
            for mp_aggr in mp_aggregations:
                ...
                self.add_error(
                    "executed_by",
                    "Selected model program aggregation must be from the same resource",
                )
    ...
    def update_metadata(self, metadata):
        ...
        if executed_by:
            ...
            if not logical_file.metadata_schema_json or not metadata.metadata_json:
                logical_file.metadata_schema_json = (
                    self.user_selected_mp_aggr.metadata_schema_json
                )
                logical_file.save()
        else:
...
class ModelProgramMetadataValidationForm(forms.Form):
        ...
        if mp_file_types_string:
            ...
            for mp_file_type_string in mp_file_types:
                ...
                if len(mp_file_type_lst) != 2:
                    err_msg = (
                        "Input data format ({}) for model program file type "
                        "is invalid".format(mp_file_type_string)
                    )
                    self.add_error("mp_file_types", err_msg)
                if mp_file_type_lst[1].lower() != "none":
                    mp_file_type = ModelProgramResourceFileType.type_from_string(
                        mp_file_type_lst[1].lower()
                    )
                    if mp_file_type is None:
                        err_msg = "{} not a valid model program file type".format(
                            mp_file_type_lst[1]
                        )
                        self.add_error("mp_file_types", err_msg)
        ...
        if json_schema_file:
            ...
            if schema_data:
                return self._validate_json_schema(
                    schema_string=schema_data, field_name="mi_json_schema_file"
                )
        return json_schema
    ...
    def update_metadata(self, metadata):
        ...
        if self.cleaned_data["mi_json_schema_template"]:
            ...
            logical_file.metadata_schema_json = self.cleaned_data[
                "mi_json_schema_template"
            ]
            ...
            for res_file in logical_file.files.all():
                if res_file.short_path in mp_file_types_dict:
                    ...
                    if mp_file_type is not None:
                        ModelProgramResourceFileType.create(
                            file_type=mp_file_type,
                            res_file=res_file,
    ...
    def _validate_json_schema(self, schema_string, field_name):
        ...
        """
        json_schema, validation_errors = ModelProgramLogicalFile.validate_meta_schema(
            schema_string
        )
...
class OriginalCoverageFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        crispy_form_fields = get_crispy_form_fields(
            form_field_names, file_type=file_type
        )
        layout = Layout(*crispy_form_fields)

        super(OriginalCoverageFormHelper, self).__init__(
            allow_edit,
            res_short_id,
            element_id,
            element_name,
...
class OriginalCoverageForm(forms.Form):
    ...
    projection = forms.CharField(
        max_length=100, required=False, label="Coordinate Reference System"
    )
    ...
    projection_string_type = forms.CharField(
        max_length=20,
        label="Coordinate String Type",
        required=False,
        widget=forms.TextInput(),
    )
    projection_string_text = forms.CharField(
        label="Coordinate String", required=False, widget=forms.Textarea()
    )
    datum = forms.CharField(
        max_length=300, label="Datum", required=False, widget=forms.TextInput()
    )

    def __init__(
        self, allow_edit=True, res_short_id=None, element_id=None, *args, **kwargs
    ):
        file_type = kwargs.pop("file_type", False)
        super(OriginalCoverageForm, self).__init__(*args, **kwargs)
        self.helper = OriginalCoverageFormHelper(
            allow_edit,
            res_short_id,
            element_id,
...
class OriginalCoverageRasterFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(OriginalCoverageRasterFormHelper, self).__init__(
            allow_edit,
            res_short_id,
            element_id,
            element_name,
...
class OriginalCoverageSpatialForm(forms.Form):
    ...
    northlimit = forms.DecimalField(
        label="North Extent", widget=forms.TextInput(attrs={"readonly": "readonly"})
    )
    eastlimit = forms.DecimalField(
        label="East Extent", widget=forms.TextInput(attrs={"readonly": "readonly"})
    )
    southlimit = forms.DecimalField(
        label="South Extent", widget=forms.TextInput(attrs={"readonly": "readonly"})
    )
    westlimit = forms.DecimalField(
        label="West Extent", widget=forms.TextInput(attrs={"readonly": "readonly"})
    )
    units = forms.CharField(
        max_length=50,
        label="Coordinate Reference System Unit",
        widget=forms.TextInput(attrs={"readonly": "readonly"}),
    )
    projection_string = forms.CharField(
        required=False,
    ...
    def __init__(
        ...
        self.helper = OriginalCoverageRasterFormHelper(
            allow_edit,
            res_short_id,
            element_id,
...
class CellInfoFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(CellInfoFormHelper, self).__init__(
            allow_edit,
            res_short_id,
            element_id,
            element_name,
...
class CellInfoForm(ModelForm):
    ...
    def __init__(
        ...
        self.helper = CellInfoFormHelper(
            allow_edit, res_short_id, element_id, element_name="CellInformation"
        )
    ...
    class Meta:
        ...
        exclude = ["content_object"]
        widgets = {
            "rows": forms.TextInput(attrs={"readonly": "readonly"}),
...
class BandInfoFormHelper(BandBaseFormHelper):
    ...
    def __init__(
        self, res_short_id=None, element_id=None, element_name=None, *args, **kwargs
    ):
        ...
        super(BandInfoFormHelper, self).__init__(
            res_short_id, element_id, element_name, layout, *args, **kwargs
        )
...
class BandInfoForm(ModelForm):
    ...
    def __init__(
        self, allow_edit=False, res_short_id=None, element_id=None, *args, **kwargs
    ):
        super(BandInfoForm, self).__init__(*args, **kwargs)

        self.helper = BandInfoFormHelper(
            res_short_id, element_id, element_name="Band Information"
        )
        self.delete_modal_form = None
        self.number = 0
        self.allow_edit = allow_edit
        if res_short_id:
            self.action = (
                "/hsapi/_internal/%s/bandinformation/add-metadata/" % res_short_id
            )
        else:
            self.action = ""
...
class OriginalCoverageGeofeatureFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(OriginalCoverageGeofeatureFormHelper, self).__init__(
            allow_edit,
            res_short_id,
            element_id,
            element_name,
...
class OriginalCoverageGeofeatureForm(forms.Form):
    ...
    projection_string = forms.CharField(
        required=False, label="Coordinate String", widget=forms.Textarea()
    )
    projection_name = forms.CharField(
        max_length=256, required=False, label="Coordinate Reference System"
    )
    ...
    def __init__(
        ...
        self.helper = OriginalCoverageGeofeatureFormHelper(
            allow_edit,
            res_short_id,
            element_id,
            element_name="OriginalCoverage",
...
class GeometryInformationFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(GeometryInformationFormHelper, self).__init__(
            allow_edit,
            res_short_id,
            element_id,
            element_name,
            layout,
            element_name_label="Geometry Information",
            *args,
            **kwargs
        )
...
class GeometryInformationForm(forms.Form):
    ...
    featureCount = forms.IntegerField(
        label="Feature Count", required=True, widget=forms.TextInput()
    )
    ...
    def __init__(
        ...
        self.helper = GeometryInformationFormHelper(
            allow_edit,
            res_short_id,
            element_id,
...
class SiteFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        element_id=None,
        element_name=None,
        show_site_code_selection=False,
        *args,
        **kwargs
    ):
        ...
        super(SiteFormHelper, self).__init__(
...
class SiteForm(ModelForm):
    ...
    def __init__(
        ...
        _set_available_elements_form_field(
            form=self, elements=available_sites, element_name="site"
        )
        code_selection_label = "Select any existing sites to use for this series"
        _set_element_code_selection_form_field(
            form=self,
            form_field_name="site_code_choices",
            form_field_label=code_selection_label,
            element_id=element_id,
            elements=available_sites,
    ...
    def set_dropdown_widgets(self, site_type=None, elevation_datum=None):
        cv_site_type_choices = _get_cv_dropdown_widget_items(
            self.cv_site_types, site_type
        )
        self.fields["site_type"].widget = forms.Select(choices=cv_site_type_choices)

        cv_e_datum_choices = _get_cv_dropdown_widget_items(
            self.cv_elevation_datums, elevation_datum
        )
    ...
    class Meta:
        ...
        exclude = ["content_object"]
        widgets = {
            "elevation_m": forms.TextInput(),
            "latitude": forms.TextInput(),
            "longitude": forms.TextInput(),
        }
        labels = {"latitude": "WGS84 Latitude*", "longitude": "WGS84 Longitude*"}
...
class SiteValidationForm(forms.Form):
    ...
    def clean_latitude(self):
        ...
        if lat < -90 or lat > 90:
            raise forms.ValidationError(
                "Value for latitude must be in the range of -90 to 90"
            )
        return lat
    ...
    def clean_longitude(self):
        ...
        if lon < -180 or lon > 180:
            raise forms.ValidationError(
                "Value for longitude must be in the range of -180 to 180"
            )
        return lon
    ...
    def clean(self):
        ...
        if elevation_m is not None:
            if len(elevation_datum.strip()) == 0:
                self._errors["elevation_datum"] = [
                    "A value for elevation datum is missing"
                ]
...
class VariableTimeseriesFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        element_id=None,
        element_name=None,
        show_variable_code_selection=False,
        *args,
        **kwargs
    ):
        file_type = kwargs.pop("file_type", False)
        ...
        super(VariableTimeseriesFormHelper, self).__init__(
            allow_edit, res_short_id, element_id, element_name, layout, *args, **kwargs
        )
...
class VariableTimeseriesForm(ModelForm):
    ...
    def __init__(
        ...
        show_variable_code_selection = (
            len(available_variables) > 0 and show_variable_code_selection
        )
        self.helper = VariableTimeseriesFormHelper(
            allow_edit,
            res_short_id,
            element_id,
            element_name="Variable",
        ...
        self.fields["selected_series_id"].initial = selected_series_id
        _set_available_elements_form_field(
            form=self, elements=available_variables, element_name="variable"
        )
        code_selection_label = "Select any existing variables to use for this series"
        _set_element_code_selection_form_field(
            form=self,
            form_field_name="variable_code_choices",
            form_field_label=code_selection_label,
            element_id=element_id,
            elements=available_variables,
            element_code_att_name="variable_code",
            element_name_att_name="variable_name",
        )

    def set_dropdown_widgets(
        self, variable_type=None, variable_name=None, speciation=None
    ):
        cv_var_type_choices = _get_cv_dropdown_widget_items(
            self.cv_variable_types, variable_type
        )
        self.fields["variable_type"].widget = forms.Select(choices=cv_var_type_choices)

        cv_var_name_choices = _get_cv_dropdown_widget_items(
            self.cv_variable_names, variable_name
        )
        self.fields["variable_name"].widget = forms.Select(choices=cv_var_name_choices)

        cv_speciation_choices = _get_cv_dropdown_widget_items(
            self.cv_speciations, speciation
        )
...
class MethodFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        element_id=None,
        element_name=None,
        show_method_code_selection=False,
        *args,
        **kwargs
    ):
        # the order in which the model fields are listed for the FieldSet is the order these
        ...
        super(MethodFormHelper, self).__init__(
            allow_edit, res_short_id, element_id, element_name, layout, *args, **kwargs
        )
...
class MethodForm(ModelForm):
    ...
    def __init__(
        ...
        show_method_code_selection = (
            len(available_methods) > 0 and show_method_code_selection
        )
        ...
        _set_element_code_selection_form_field(
            form=self,
            form_field_name="method_code_choices",
            form_field_label=code_selection_label,
            element_id=element_id,
            elements=available_methods,
            element_code_att_name="method_code",
            element_name_att_name="method_name",
        )

    def set_dropdown_widgets(self, current_method_type=None):
        cv_method_type_choices = _get_cv_dropdown_widget_items(
            self.cv_method_types, current_method_type
        )
...
class ProcessingLevelFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        element_id=None,
        element_name=None,
        show_processing_level_code_selection=False,
        *args,
        **kwargs
    ):
        # the order in which the model fields are listed for the FieldSet is the order these
        ...
        super(ProcessingLevelFormHelper, self).__init__(
            allow_edit, res_short_id, element_id, element_name, layout, *args, **kwargs
        )
...
class ProcessingLevelForm(ModelForm):
    ...
    def __init__(
        ...
        show_processing_level_code_selection = kwargs.pop(
            "show_processing_level_code_selection", False
        )
        file_type = kwargs.pop("file_type", False)
        ...
        self.fields["selected_series_id"].initial = selected_series_id
        _set_available_elements_form_field(
            self, available_processinglevels, "processinglevel"
        )
        code_selection_label = (
            "Select any existing processing level to use for this series"
        )
        _set_element_code_selection_form_field(
            form=self,
...
class TimeSeriesResultFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(TimeSeriesResultFormHelper, self).__init__(
            allow_edit, res_short_id, element_id, element_name, layout, *args, **kwargs
        )
...
class TimeSeriesResultForm(ModelForm):
    ...
    def set_dropdown_widgets(
        self,
        current_sample_medium=None,
        current_units_type=None,
        current_agg_statistics=None,
        current_status=None,
    ):
        cv_sample_medium_choices = _get_cv_dropdown_widget_items(
            self.cv_sample_mediums, current_sample_medium
        )
        self.fields["sample_medium"].widget = forms.Select(
            choices=cv_sample_medium_choices
        )
        cv_units_type_choices = _get_cv_dropdown_widget_items(
            self.cv_units_types, current_units_type
        )
        self.fields["units_type"].widget = forms.Select(choices=cv_units_type_choices)
        cv_status_choices = _get_cv_dropdown_widget_items(
            self.cv_statuses, current_status
        )
        self.fields["status"].widget = forms.Select(choices=cv_status_choices)
        cv_agg_statistics_choices = _get_cv_dropdown_widget_items(
            self.cv_aggregation_statistics, current_agg_statistics
        )
...
class TimeSeriesResultValidationForm(forms.Form):
    ...
    def clean(self):
        ...
        if (
            aggregation_statistics is None
            or aggregation_statistics == NO_SELECTION_DROPDOWN_OPTION
        ):
            self._errors["aggregation_statistics"] = [
                "A value for aggregation statistic " "is missing"
            ]
...
class UTCOffSetFormHelper(BaseFormHelper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(UTCOffSetFormHelper, self).__init__(
            allow_edit, res_short_id, element_id, element_name, layout, *args, **kwargs
        )
...
class UTCOffSetForm(ModelForm):
    ...
    def __init__(
        ...
        self.helper = UTCOffSetFormHelper(
            allow_edit,
            res_short_id,
            element_id,
...
def _get_cv_dropdown_widget_items(dropdown_items, selected_item_name):
    ...
    dropdown_items = [
        item for item in dropdown_items if item.name != selected_item_name
    ]
...
def _set_available_elements_form_field(form, elements, element_name):
    ...
    for element in elements:
        element_data = model_to_dict(
            element, exclude=["object_id", "series_ids", "content_type"]
        )
...
def _set_element_code_selection_form_field(
    form,
    form_field_name,
    form_field_label,
    element_id,
    elements,
    element_code_att_name,
    element_name_att_name,
):
    """Timeseries aggregation related form helper function."""
    ...
    if len(elements) > 0:
        ...
        form.fields[form_field_name].widget = forms.Select(choices=element_code_choices)
...
def _set_form_helper_layout(
    common_layout, element_name, is_show_element_code_selection, field_css
):
    """Timeseries aggregation related form helper function."""
    ...
    if is_show_element_code_selection:
        element_choice_help = (
            "Select '----' for a new {} or any other option to use an "
