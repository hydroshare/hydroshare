...
from .forms import (
    ModelInstanceMetadataValidationForm,
    ModelProgramMetadataValidationForm,
)
    ...
    def real_decorator(view_func):
        def wrapper(request, *args, **kwargs):
            ...
            if file_type == "NestedLogicalFile":
                ...
                logical_file, json_response = _get_logical_file(
                    hs_file_type, file_type_id
                )
...
def get_res_file(pk, file_path):
    ...
    if not file_rel_path:
        return JsonResponse(
            "file_path cannot be empty", status=status.HTTP_400_BAD_REQUEST
        )

    # security checks deny illicit requests
    if file_rel_path.find("/../") >= 0 or file_rel_path.endswith("/.."):
        return JsonResponse(
            "file_path must not contain /../", status=status.HTTP_400_BAD_REQUEST
        )
    ...
    try:
        folder, file_name = ResourceFile.resource_path_is_acceptable(
            resource, file_storage_path, test_exists=True
        )
...
def set_file_type_public(request, pk, file_path, hs_file_type):
    ...
    if hs_file_type == "FileSet":
        ...
        json_response = set_file_type(
            request=request, resource_id=pk, hs_file_type=hs_file_type
        )
    else:
    ...
    return Response(data=response_dict["message"], status=json_response.status_code)
...
def get_fileset_id(resource_id, file_path):
    ...
    if not filesets:
        return Response(
            "Folder {} does not exist.".format(file_path),
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def remove_aggregation_public(request, resource_id, hs_file_type, file_path, **kwargs):
    ...
    if hs_file_type == "FileSetLogicalFile":
        ...
        return remove_aggregation(
            request,
            resource_id=resource_id,
            hs_file_type=hs_file_type,
            file_type_id=fileset_id,
        )
    else:
...
@api_view(["POST"])
def move_aggregation_public(
    request, resource_id, hs_file_type, file_path, tgt_path="", **kwargs
):
    """moves all files associated with an aggregation and all the associated metadata."""
    res_file = get_res_file(resource_id, file_path)
    if isinstance(res_file, Response):
        return res_file
    return move_aggregation(
        request, resource_id, hs_file_type, res_file.logical_file.id, tgt_path, **kwargs
    )


@swagger_auto_schema(
    method="get", responses={200: serializers.ModelProgramMetaTemplateSchemaSerializer}
)
@api_view(["GET"])
...
@swagger_auto_schema(
    method="get",
    responses={
        200: "Metadata template schema as JSON",
...
def get_model_program_template_metadata_schema(request, schema_filename, **kwargs):
    ...
    return JsonResponse(
        data=f"{schema_filename} is not a valid metadata template schema filename",
        safe=False,
        status=status.HTTP_400_BAD_REQUEST,
    )
...
@swagger_auto_schema(
    method="put",
    responses={
        204: "Template metadata schema was assigned to model program aggregation",
        400: "Invalid/bad request message",
        403: "You don't have permission to edit this resource",
    },
)
@api_view(["PUT"])
def use_template_metadata_schema_for_model_program(
    request, resource_id, aggregation_path, schema_filename, **kwargs
):
    """Assigns the specified template metadata schema as the metadata schema for a model program aggregation."""

    try:
        resource, aggr = _validate_model_aggregation_api_request(
            request=request,
            resource_id=resource_id,
        ...
        return JsonResponse(
            data=str(ex), safe=False, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    msg = (
        f"Template schema:{schema_filename} was set as the metadata schema for model program "
...
@swagger_auto_schema(
    method="put",
    responses={
        204: "Metadata schema was updated for model instance aggregation",
        400: "Invalid/bad request message",
        403: "You don't have permission to edit this resource",
    },
)
@api_view(["PUT"])
def update_metadata_schema_for_model_instance(
    request, resource_id, aggregation_path, **kwargs
):
    ...
    if not aggr.metadata.executed_by:
        err_msg = (
            f"Specified model instance aggregation:{aggregation_path} is not executed by a "
    ...
    resource_modified(resource, request.user, overwrite_bag=False)
    msg = (
        f"Metadata schema was updated for model instance aggregation:{aggregation_path} from the "
...
@swagger_auto_schema(
    method="get",
    operation_description="Gets metadata for a model program aggregation in json",
    responses={
        200: serializers.ModelProgramMetaSerializer,
        400: "Invalid/bad request message",
...
@swagger_auto_schema(
    method="put",
    operation_description="Updates metadata for a model program aggregation",
...
def model_program_metadata_in_json(request, resource_id, aggregation_path, **kwargs):
    ...
    if request.method == "GET":
        ...
        return JsonResponse(
            data=meta_serializer.serialize(mp_aggr=aggr), status=status.HTTP_200_OK
        )

    meta_serializer = serializers.ModelProgramMetaSerializer(
        data=request.data, context={"mp_aggr": aggr}, partial=True
    )
...
@swagger_auto_schema(
    method="get",
    operation_description="Gets metadata for a model instance aggregation in json",
    responses={
        200: serializers.ModelInstanceMetaSerializer,
        400: "Invalid/bad request message",
        403: "You don't have permission to view this resource",
    },
)
@swagger_auto_schema(
    method="put",
    request_body=serializers.ModelInstanceMetaSerializer,
...
def model_instance_metadata_in_json(request, resource_id, aggregation_path, **kwargs):
    ...
    if request.method == "GET":
        ...
        return JsonResponse(
            data=meta_serializer.serialize(mi_aggr=aggr), status=status.HTTP_200_OK
        )

    meta_serializer = serializers.ModelInstanceMetaSerializer(
        data=request.data, context={"mi_aggr": aggr, "resource": resource}, partial=True
    )
    if not meta_serializer.is_valid():
        raise RF_ValidationError(detail=meta_serializer.errors)
...
def move_aggregation(
    ...
    **kwargs,
    ...
    if tgt_path:
        ...
        src_fileset_aggr = res.get_fileset_aggregation_in_path(
    ...
    for file in res_files:
        ...
        if istorage.exists(tgt_full_path):
            ...
            override_tgt_res_files.append(
                ResourceFile.get(res, file=file_name, folder=tgt_path)
            )

    if override_tgt_paths:
        if not file_override:
            override_file_names = ", ".join(
                [os.path.basename(tgt_path) for tgt_path in override_tgt_paths]
            )
    ...
    if run_async:
        task = move_aggregation_task.apply_async(
            (resource_id, file_type_id, hs_file_type, tgt_path)
        )
        task_id = task.task_id
        task_dict = get_or_create_task_notification(
            task_id,
            name="aggregation move",
            payload=resource_id,
...
def update_metadata_element(
    request, hs_file_type, file_type_id, element_name, element_id, **kwargs
):
...
def add_metadata_element(request, hs_file_type, file_type_id, element_name, **kwargs):
    ...
    if is_add_success:
        ...
        form_action = form_action.format(
            logical_file.type_name(), logical_file.id, element_name, element.id
        )
...
@authorise_for_aggregation_edit
@login_required
def delete_coverage_element(request, hs_file_type, file_type_id, element_id, **kwargs):
...
def update_key_value_metadata(request, hs_file_type, file_type_id, **kwargs):
    ...
    def validate_key():
        if key in list(logical_file.metadata.extra_metadata.keys()):
            ...
            return False, JsonResponse(
                ajax_response_data, status=status.HTTP_400_BAD_REQUEST
            )
        return True, None
    ...
    resource = logical_file.resource
    resource_modified(resource, request.user, overwrite_bag=False)
    extra_metadata_div = super(
        logical_file.metadata.__class__, logical_file.metadata
    ).get_extra_metadata_html_form()
    context = Context({})
...
def add_keyword_metadata(request, hs_file_type, file_type_id, **kwargs):
    ...
    if (
        hs_file_type == "RefTimeseriesLogicalFile"
        and logical_file.metadata.has_keywords_in_json
    ):
        ...
        ajax_response_data = {
            ...
            "logical_file_type": logical_file.type_name(),
            "element_name": "keyword",
            "message": "Adding of keyword is not allowed",
        }
        return JsonResponse(ajax_response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    if not any(kw.lower() in keywords for kw in existing_keywords):
        ...
        resource_keywords = [
            subject.value.lower() for subject in resource.metadata.subjects.all()
        ]
        for kw in keywords:
        ...
        resource_modified(resource, request.user, overwrite_bag=False)
        resource_keywords = [
            subject.value for subject in resource.metadata.subjects.all()
        ]
        ajax_response_data = {
            "status": "success",
            "logical_file_type": logical_file.type_name(),
...
def delete_keyword_metadata(request, hs_file_type, file_type_id, **kwargs):
    ...
    if (
        ...
        ajax_response_data = {
            ...
            "logical_file_type": logical_file.type_name(),
            "element_name": "keyword",
            "message": "Keyword delete is not allowed",
        }
        return JsonResponse(ajax_response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    if keyword.lower() in existing_keywords:
        logical_file.metadata.keywords = [
            kw for kw in logical_file.metadata.keywords if kw.lower() != keyword.lower()
        ]
        if hs_file_type != "TimeSeriesLogicalFile":
            metadata = logical_file.metadata
            metadata.is_dirty = True
...
def update_refts_abstract(request, file_type_id, **kwargs):
    ...
    if abstract.strip():
        ...
        ajax_response_data = {
            ...
            "logical_file_type": logical_file.type_name(),
            "element_name": "abstract",
            "message": "Data is missing for abstract",
        }
        return JsonResponse(ajax_response_data, status=status.HTTP_400_BAD_REQUEST)
...
def update_model_program_metadata(request, file_type_id, **kwargs):
    ...
    refresh_metadata = (
        len(mp_validation_form.cleaned_data["mi_json_schema_file"]) > 0
    ...
    ajax_response_data = {
        "status": "success",
        "logical_file_type": logical_file.type_name(),
        "element_name": "multiple-elements",
...
def update_model_instance_metadata_json(request, file_type_id, **kwargs):
    ...
    try:
        ...
        schema_err_msg = "{}. Schema invalid field path:{}".format(
            ex.message, str(list(ex.path))
        )
        msg = "JSON metadata is not valid as per the associated metadata schema. Error: {}".format(
            schema_err_msg
        )
        error_response = {"status": "error", "message": msg}
        return JsonResponse(error_response, status=status.HTTP_400_BAD_REQUEST)
...
def update_model_instance_meta_schema(request, file_type_id, **kwargs):
    ...
    ajax_response_data = {
        "status": "success",
        "logical_file_type": logical_file.type_name(),
        "element_name": "metadata_schema_json",
        "message": "Update was successful",
    }
    return JsonResponse(ajax_response_data, status=status.HTTP_200_OK)
...
def update_model_instance_metadata(request, file_type_id, **kwargs):
    ...
    mi_validation_form = ModelInstanceMetadataValidationForm(
        request.POST, user=request.user, resource=logical_file.resource
    )
...
def get_metadata(request, hs_file_type, file_type_id, metadata_mode):
    ...
    authorize(
        request,
        logical_file.resource.short_id,
...
def get_timeseries_metadata(request, file_type_id, series_id, resource_mode):
    ...
    logical_file, json_response = _get_logical_file(
        "TimeSeriesLogicalFile", file_type_id
    )
    if json_response is not None:
        return json_response
...
def _get_logical_file(hs_file_type, file_type_id):
    try:
        content_type = ContentType.objects.get(
            app_label="hs_file_types", model=hs_file_type.lower()
        )
...
def _validate_model_aggregation_api_request(
    request, resource_id, aggregation_path, model_type
):
