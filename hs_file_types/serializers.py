...
class SpatialCoverageSerializer((serializers.Serializer)):
    ...
    units = serializers.CharField(
        max_length=100, help_text="Units of measurement for bounding box co-ordinates"
    )
    name = serializers.CharField(
        required=False, max_length=100, help_text="Name of the place"
    )
    ...
    west = serializers.FloatField(
        required=False,
        help_text="West co-ordinate (required if the coverage type is box)",
    )
    south = serializers.FloatField(
        required=False,
        help_text="South co-ordinate (required if the coverage type is box)",
    )
        ...
        if data:
            try:
                Coverage.validate_coverage_type_value_attributes(
                    coverage_type=data["type"], value_dict=data, use_limit_postfix=False
                )
...
class BaseAggregationMetaSerializer(serializers.Serializer):
    ...
    additional_metadata = serializers.HStoreField(
...
class ModelProgramMetaTemplateSchemaSerializer(serializers.Serializer):
    meta_schema_filename = serializers.CharField(
        max_length=100,
        required=True,
...
class ModelProgramResourceFileTypeSerializer(serializers.Serializer):
    file_type = serializers.ChoiceField(
        choices=[choice[1] for choice in ModelProgramResourceFileType.CHOICES],
        required=False,
        help_text="Type of model program file",
    )
    file_path = serializers.CharField(
        required=False,
        help_text="Relative path of the file in model program " "aggregation",
    )
...
class ModelProgramMetaSerializer(BaseAggregationMetaSerializer):
    ...
    program_schema_json = serializers.JSONField(
        required=False,
        allow_null=True,
    ...
    code_repository = serializers.URLField(
        required=False,
        allow_blank=True,
        max_length=255,
        help_text="A URL to the source code repository (e.g. git, mercurial, svn)",
    )
    program_file_types = ModelProgramResourceFileTypeSerializer(
        many=True, required=False, default=None, allow_null=True
    )
        ...
        if value:
            try:
                ...
                raise serializers.ValidationError(
                    f"Metadata schema is not valid JSON. Error:{str(ex)}"
                )
            # validate schema json here against the hs schema validator
            (
                meta_schema_dict,
                validation_errors,
            ) = ModelProgramLogicalFile.validate_meta_schema(meta_schema_json)
            if validation_errors:
                err_msg = ", ".join(validation_errors)
                raise serializers.ValidationError(
                    f"Metadata schema is not valid. Error:{err_msg}"
                )
    ...
    def validate_program_file_types(self, value):
        ...
        for ft_item_dict in value:
            ...
            if ft_item_tuple in seen:
                raise serializers.ValidationError(
                    f"Duplicate program_file_types:{ft_item_dict}"
                )
        ...
        for ft_item in value:
            if ft_item["file_path"] not in valid_file_paths:
                raise serializers.ValidationError(
                    f"Invalid file path:{ft_item['file_path']}"
                )
    ...
    def update(self, mp_aggr, validated_data):
        ...
        mp_aggr.metadata.keywords = validated_data.get(
            "keywords", mp_aggr.metadata.keywords
        )
        mp_aggr.metadata.extra_metadata = validated_data.get(
            "additional_metadata", mp_aggr.metadata.extra_metadata
        )
        mp_aggr.metadata.version = validated_data.get(
            "version", mp_aggr.metadata.version
        )
        mp_aggr.metadata.programming_languages = validated_data.get(
            "programming_languages", mp_aggr.metadata.programming_languages
        )
        mp_aggr.metadata.operating_systems = validated_data.get(
            "operating_systems", mp_aggr.metadata.operating_systems
        )
        mp_aggr.metadata.release_date = validated_data.get(
            "release_date", mp_aggr.metadata.release_date
        )
        mp_aggr.metadata.website = validated_data.get(
            "website", mp_aggr.metadata.website
        )
        mp_aggr.metadata.code_repository = validated_data.get(
            "code_repository", mp_aggr.metadata.code_repository
        )
        mp_aggr.metadata_schema_json = validated_data.get(
            "program_schema_json", mp_aggr.metadata_schema_json
        )
        ...
        if program_file_types is not None:
            res_file_path_map = {
                res_file.short_path: res_file for res_file in mp_aggr.files.all()
            }
    ...
    def serialize(mp_aggr):
        ...
        if mp_aggr.metadata.release_date:
            data["release_date"] = mp_aggr.metadata.release_date.strftime(
                _OUT_DATE_FORMAT
            )
        else:
        ...
        for mp_file in mp_aggr.metadata.mp_file_types.all():
            mp_file_type_name = ModelProgramResourceFileType.type_name_from_type(
                mp_file.file_type
            )
            mp_file_types.append(
                {
                    "file_type": mp_file_type_name,
                    "file_path": mp_file.res_file.short_path,
                }
            )
...
class ModelInstanceMetaSerializer(BaseAggregationMetaSerializer):
    temporal_coverage = TemporalCoverageSerializer(
        required=False, help_text="Temporal coverage"
    )
    spatial_coverage = SpatialCoverageSerializer(
        required=False, help_text="Spatial coverage"
    )
    has_model_output = serializers.BooleanField(
        required=False, help_text="If the aggregation contains model output files"
    )
    ...
    metadata_json = serializers.JSONField(
        required=False, help_text="Schema-based metadata"
    )
        ...
        if value:
            try:
                ...
                if not aggr.is_model_program:
                    raise serializers.ValidationError(
                        f"Specified aggregation:{value} is not a model "
                        f"program aggregation"
                    )
            except ObjectDoesNotExist:
                raise serializers.ValidationError(
                    f"No model program aggregation was found for the path:{value}"
                )
            return aggr
        return value
    ...
    def validate_metadata_json(self, value):
        ...
        if value:
            try:
                ...
                raise serializers.ValidationError(
                    f"Metadata is not valid JSON. Error:{str(ex)}"
                )
            ...
            meta_json = value
            try:
                jsonschema.Draft4Validator(mi_aggr.metadata_schema_json).validate(
                    meta_json
                )
    ...
    def update(self, mi_aggr, validated_data):
        ...
        mi_aggr.metadata.keywords = validated_data.get(
            "keywords", mi_aggr.metadata.keywords
        )
        mi_aggr.metadata.extra_metadata = validated_data.get(
            "additional_metadata", mi_aggr.metadata.extra_metadata
        )
        mi_aggr.metadata.has_model_output = validated_data.get(
            "has_model_output", mi_aggr.metadata.has_model_output
        )
        ...
        mi_aggr.metadata.metadata_json = validated_data.get(
            "metadata_json", mi_aggr.metadata.metadata_json
        )
        ...
        if spatial_coverage is not None:
            if spatial_coverage:
                ...
                if mi_aggr.metadata.spatial_coverage:
                    mi_aggr.metadata.update_element(
                        "coverage",
                        mi_aggr.metadata.spatial_coverage.id,
                        type=cove_type,
                        value=spatial_coverage,
                    )
                else:
                    mi_aggr.metadata.create_element(
                        "coverage", type=cove_type, value=spatial_coverage
                    )
        ...
        if temporal_coverage is not None:
            if temporal_coverage:
                temporal_coverage["start"] = temporal_coverage["start"].strftime(
                    _OUT_DATE_FORMAT
                )
                temporal_coverage["end"] = temporal_coverage["end"].strftime(
                    _OUT_DATE_FORMAT
                )
                if mi_aggr.metadata.temporal_coverage:
                    mi_aggr.metadata.update_element(
                        "coverage",
                        mi_aggr.metadata.temporal_coverage.id,
                        type="period",
