...
from hs_file_types.models.raster import (
    OriginalCoverageRaster,
    CellInformation,
    BandInformation,
)
from .utils import (
    assert_raster_file_type_metadata,
    CompositeResourceTestMixin,
    get_path_with_no_file_extension,
)


class RasterFileTypeTest(
    ...
    def setUp(self):
        ...
        self.logan_tif_1_file = "hs_file_types/tests/{}".format(
            self.logan_tif_1_file_name
        )
        self.logan_tif_2_file = "hs_file_types/tests/{}".format(
            self.logan_tif_2_file_name
        )
        self.logan_vrt_file = "hs_file_types/tests/{}".format(self.logan_vrt_file_name)
        self.logan_vrt_file2 = "hs_file_types/tests/{}".format(
            self.logan_vrt_file_name2
        )
        ...
        self.raster_with_coverage_crossing_dateline_name = (
            "raster_with_coverage_crossing_dateline.tif"
        )
        self.invalid_raster_zip_file_name = "bad_small_vrt.zip"
        self.raster_file = "hs_file_types/tests/{}".format(self.raster_file_name)
        self.raster_zip_file = "hs_file_types/tests/{}".format(
            self.raster_zip_file_name
        )
        self.invalid_raster_file = "hs_file_types/tests/{}".format(
            self.invalid_raster_file_name
        )
        self.raster_with_coverage_crossing_dateline_file = (
            "hs_file_types/tests/data/{}".format(
                self.raster_with_coverage_crossing_dateline_name
            )
        )
    ...
    def test_create_aggregation_from_tif_file_3(self):
        ...
        self.add_file_to_resource(
            file_to_add=self.invalid_raster_file, upload_folder=new_folder
        )
        self.assertEqual(self.composite_resource.files.all().count(), 2)
    ...
    def test_create_aggregation_from_tif_file_5(self):
        ...
        self.create_composite_resource()
        self.add_file_to_resource(
            file_to_add=self.raster_with_coverage_crossing_dateline_file
        )
        self.assertEqual(self.composite_resource.files.all().count(), 1)
        res_file = self.composite_resource.files.first()
    ...
    def test_create_aggregation_from_zip_file_2(self):
        ...
        ResourceFile.create_folder(self.composite_resource, new_folder)
        self.add_file_to_resource(
            file_to_add=self.raster_zip_file, upload_folder=new_folder
        )
    ...
    def test_create_aggregation_from_multiple_tif_with_vrt(self):
        ...
        GeoRasterLogicalFile.set_file_type(
            self.composite_resource, self.user, res_file_tif.id
        )
    ...
    def test_aggregation_validation(self):
        ...
        self.assertTrue(
            "is listed by more than one vrt file" in validation_error.exception.message
        )
    ...
    def test_create_aggregation_from_multiple_tif_without_vrt(self):
        ...
        for res_file in self.composite_resource.files.all():
            if (
                res_file_tif.file_name == res_file.file_name
                or res_file.extension.lower() == ".vrt"
            ):
                self.assertEqual(res_file.has_logical_file, True)
            else:
                self.assertEqual(res_file.has_logical_file, False)
    ...
    def test_create_aggregation_with_extra_tif_with_vrt(self):
        ...
        self.assertEqual(GeoRasterLogicalFile.objects.count(), 1)
        GeoRasterLogicalFile.set_file_type(
            self.composite_resource, self.user, lone_tif_file.id
        )
    ...
    def test_set_file_type_to_geo_raster_invalid_file_3(self):
        ...
        self._test_invalid_file()
        self.assertTrue(
            self.composite_resource.files.first().file_name.endswith(".zip")
        )
        self.composite_resource.delete()
    ...
    def test_metadata_CRUD(self):
        ...
        with self.assertRaises(ValidationError):
            logical_file.metadata.delete_element(
                "bandinformation", logical_file.metadata.bandInformations.first().id
            )
        ...
        logical_file.metadata.update_element(
            "originalcoverage", logical_file.metadata.originalCoverage.id, value=value_2
        )
    ...
    def test_remove_aggregation(self):
        ...
        GeoRasterLogicalFile.set_file_type(
            self.composite_resource, self.user, res_tif_file.id
        )
        ...
        self.assertFalse(
            self.composite_resource.files.first().file_name.endswith(".vrt")
        )
        self.assertEqual(
            self.composite_resource.files.first().file_name, res_tif_file.file_name
        )
        self.assertFalse(self.composite_resource.dangling_aggregations_exist())
        self.composite_resource.delete()
    ...
    def test_aggregation_folder_rename(self):
        ...
        self.add_file_to_resource(
            file_to_add=self.raster_file, upload_folder=folder_for_raster
        )
        res_file = self.composite_resource.files.first()
    ...
    def test_aggregation_folder_move(self):
        ...
        # test aggregation xml file paths
        self.assertNotEqual(
            logical_file.metadata_short_file_path, metadata_short_file_path
        )
    ...
    def _test_file_metadata_on_file_delete(self, ext):
        ...
        hydroshare.delete_resource_file(
            self.composite_resource.short_id, res_file_tif.id, self.user
        )
    ...
    def _test_aggregation_from_zip_file(self, aggr_folder_path):
        ...
        self.assertEqual(
            (self.composite_resource.files.count() - logical_file.files.count()), 1
        )
        ...
        self.assertEqual(
            logical_file.size, sum([f.size for f in logical_file.files.all()])
        )
    ...
    def test_main_file(self):
        ...
        self.assertEqual(logical_file.extra_data["vrt_created"], "True")
        self.assertEqual(
            ".vrt", GeoRasterLogicalFile.objects.first().get_main_file_type()
        )
        self.assertEqual(
            "small_logan.vrt",
