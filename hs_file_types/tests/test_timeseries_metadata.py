...
class TimeSeriesFileTypeTest(
    ...
    def setUp(self):
        ...
        self.odm2_csv_file = "hs_file_types/tests/data/{}".format(
            self.odm2_csv_file_name
        )
    ...
    def test_create_aggregation_from_sqlite_file_2(self):
        ...
        self.add_file_to_resource(
            file_to_add=self.sqlite_file, upload_folder=new_folder
        )
        self.assertEqual(self.composite_resource.files.all().count(), 1)
        res_file = self.composite_resource.files.first()
    ...
    def test_create_aggregation_from_sqlite_file_3(self):
        ...
        self.add_file_to_resource(
            file_to_add=self.sqlite_invalid_file, upload_folder=new_folder
        )
    ...
    def test_create_aggregation_from_CSV_file_2(self):
        ...
        ResourceFile.create_folder(self.composite_resource, new_folder)
        self.add_file_to_resource(
            file_to_add=self.odm2_csv_file, upload_folder=new_folder
        )
    ...
    def test_aggregation_sqlite_metadata_update(self):
        ...
        logical_file.metadata.update_element(
            "Variabletimeseries", variable.id, **variable_data
        )
        variable = logical_file.metadata.variables.filter(variable_code="USU36").first()
        ...
        logical_file.metadata.update_element(
            "ProcessingLevel", pro_level.id, **pro_level_data
        )
        pro_level = logical_file.metadata.processing_levels.filter(
            processing_level_code=1
        ).first()
        ...
        logical_file.metadata.update_element(
            "timeseriesresult", ts_result.id, **ts_data
        )
        ts_result = logical_file.metadata.time_series_results.all().first()
        self.assertNotEqual(ts_result, None)
        self.assertEqual(ts_result.units_type, "Temperature")
    ...
    def test_aggregation_folder_rename(self):
        ...
        expected_meta_file_path = "{}/{}{}".format(
            new_folder, base_sqlite_file_name, METADATA_FILE_ENDSWITH
        )
        self.assertEqual(logical_file.metadata_short_file_path, expected_meta_file_path)

        expected_map_file_path = "{}/{}{}".format(
            new_folder, base_sqlite_file_name, RESMAP_FILE_ENDSWITH
        )
        ...
        # test aggregation xml file paths
        expected_meta_file_path = "{}_1/{}{}".format(
            new_folder, base_sqlite_file_name, METADATA_FILE_ENDSWITH
        )
        self.assertEqual(logical_file.metadata_short_file_path, expected_meta_file_path)

        expected_map_file_path = "{}_1/{}{}".format(
            new_folder, base_sqlite_file_name, RESMAP_FILE_ENDSWITH
        )
        self.assertEqual(logical_file.map_short_file_path, expected_map_file_path)
    ...
    def test_aggregation_parent_folder_rename(self):
        ...
        # test aggregation xml file paths
        expected_meta_file_path = "{}/{}{}".format(
            aggregation_folder_name, base_sqlite_file_name, METADATA_FILE_ENDSWITH
        )
        self.assertEqual(logical_file.metadata_short_file_path, expected_meta_file_path)

        expected_map_file_path = "{}/{}{}".format(
            aggregation_folder_name, base_sqlite_file_name, RESMAP_FILE_ENDSWITH
        )
        self.assertEqual(logical_file.map_short_file_path, expected_map_file_path)
        ...
        tgt_path = "data/contents/{0}/{1}".format(
            parent_folder, aggregation_folder_name
        )
        ...
        res_file = self.composite_resource.files.first()
        logical_file = res_file.logical_file
        expected_aggr_name = "{0}/{1}/{2}".format(
            parent_folder_rename, aggregation_folder_name, sqlite_file_name
        )
        self.assertEqual(logical_file.aggregation_name, expected_aggr_name)

        # test aggregation xml file paths after folder rename
        expected_meta_file_path = "{0}/{1}/{2}{3}".format(
            parent_folder_rename,
        ...
        self.assertEqual(logical_file.metadata_short_file_path, expected_meta_file_path)
        expected_map_file_path = "{0}/{1}/{2}{3}".format(
            parent_folder_rename,
            aggregation_folder_name,
            base_sqlite_file_name,
    ...
    def test_aggregation_folder_move(self):
        ...
        logical_file = res_file.logical_file
        expected_aggr_name = "{0}/{1}/{2}".format(
            parent_folder, aggregation_folder_name, sqlite_file_name
        )
        self.assertEqual(logical_file.aggregation_name, expected_aggr_name)
    ...
    def _test_CSV_aggregation(self, expected_aggr_folder, value_count_by_series="20"):
        ...
        self.assertEqual(
            sqlite_res_file.logical_file_type_name, "TimeSeriesLogicalFile"
        )
        self.assertEqual(TimeSeriesLogicalFile.objects.count(), 1)
        ...
        self.assertEqual(
            set(logical_file.metadata.value_counts.keys()), csv_data_column_names
        )

        # there should be 20 data values for each series
        self.assertEqual(
            logical_file.metadata.value_counts["Temp_DegC_Mendon"],
            value_count_by_series,
        )
        self.assertEqual(
            logical_file.metadata.value_counts["Temp_DegC_Paradise"],
            value_count_by_series,
        )
        ...
        self.assertEqual(
            self.composite_resource.metadata.coverages.filter(type="period").count(), 1
        )
    ...
    def test_main_file(self):
        ...
        self.assertEqual(
            ".sqlite", TimeSeriesLogicalFile.objects.first().get_main_file_type()
        )
        self.assertEqual(
