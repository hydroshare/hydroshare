...
class FileSetFileTypeTest(
    ...
    def test_create_aggregation_2(self):
        ...
        self.assertEqual(
            len(fileset_aggregation.metadata.get_required_missing_elements()), 0
        )
    ...
    def test_create_aggregation_3(self):
        ...
        self.add_files_to_resource(
            files_to_add=[self.raster_file], upload_folder=raster_child_folder
        )
        ...
        self.assertEqual(FileSetLogicalFile.objects.count(), 0)
        FileSetLogicalFile.set_file_type(
            self.composite_resource, self.user, folder_path=parent_folder
        )
        self.assertEqual(FileSetLogicalFile.objects.count(), 1)
        # check that there are no missing required metadata for the fileset aggregation
        fs_aggr = FileSetLogicalFile.objects.first()
    ...
    def test_create_aggregation_6(self):
        ...
        # add the the txt file to the resource at the above folder
        self.add_file_to_resource(
            file_to_add=self.generic_file, upload_folder=non_empty_child_folder
        )
    ...
    def test_fileset_aggregation_not_allowed_inside_model_program_aggregation(self):
        ...
        self.add_file_to_resource(
            file_to_add=self.states_prj_file, upload_folder=child_folder
        )
        # there should be two resource file at this point
        self.assertEqual(self.composite_resource.files.all().count(), 2)
        # set the parent folder to model program aggregation
        ModelProgramLogicalFile.set_file_type(
            resource=self.composite_resource, user=self.user, folder_path=parent_folder
        )
        ...
        with self.assertRaises(ValidationError):
            FileSetLogicalFile.set_file_type(
                self.composite_resource, self.user, folder_path=child_folder
            )
        self.assertEqual(FileSetLogicalFile.objects.count(), 0)
        self.assertFalse(self.composite_resource.dangling_aggregations_exist())
        self.composite_resource.delete()
    ...
    def test_move_file_into_aggregation_1(self):
        ...
        json_res_file = ResourceFile.get(
            resource=self.composite_resource, file=self.json_file_name
        )
    ...
    def test_move_file_into_aggregation_2(self):
        ...
        GenericLogicalFile.set_file_type(
            self.composite_resource, self.user, file_id=json_res_file.id
        )
    ...
    def test_delete_aggregation_in_fileset_1(self):
        ...
        # aggregation
        self.add_files_to_resource(
            files_to_add=[self.raster_file], upload_folder=raster_folder_path
        )
    ...
    def test_delete_aggregation_in_fileset_2(self):
        ...
        child_fs_folder_path = "data/contents/{}/child_fileset_folder".format(
            parent_fs_folder
        )
    ...
    def test_create_folder_in_fileset(self):
        ...
        self.add_file_to_resource(
            file_to_add=self.json_file, upload_folder=agg_sub_folder
        )
        # there should be two resource files
        self.assertEqual(self.composite_resource.files.all().count(), 2)
        json_res_file = ResourceFile.get(
        ...
        # check that there are no missing required metadata for the fileset aggregation
        self.assertEqual(
            len(fs_aggregation.metadata.get_required_missing_elements()), 0
        )
        self.assertFalse(self.composite_resource.dangling_aggregations_exist())
        self.composite_resource.delete()
    ...
    def test_delete_folder_containing_fileset(self):
        ...
        # add the the txt file to the resource at the above folder
        self.add_file_to_resource(
            file_to_add=self.generic_file, upload_folder=fileset_folder
        )
        # there should be one resource file
        self.assertEqual(self.composite_resource.files.all().count(), 1)
        # create a fileset aggregation using the fileset_folder
        FileSetLogicalFile.set_file_type(
            self.composite_resource, self.user, folder_path=fileset_folder
        )
        self.assertEqual(FileSetLogicalFile.objects.count(), 1)
        # check that there are no missing required metadata for the fileset aggregation
        fs_aggr = FileSetLogicalFile.objects.first()
    ...
    def test_delete_fileset_folder(self):
        ...
        full_folder_path = os.path.join(
            self.composite_resource.file_path, fileset_folder
        )
    ...
    def test_auto_netcdf_aggregation_creation(self):
        ...
        # aggregation
        self.add_files_to_resource(
            files_to_add=[self.netcdf_file], upload_folder=fs_aggr_path
        )
    ...
    def test_auto_raster_aggregation_creation(self):
        ...
        # aggregation
        self.add_files_to_resource(
            files_to_add=[self.raster_file], upload_folder=fs_aggr_path
        )
        # there should be three resource file - one generated by raster aggregation
        self.assertEqual(self.composite_resource.files.all().count(), 3)
        self.assertEqual(GeoRasterLogicalFile.objects.count(), 1)
        ...
        raster_res_file = ResourceFile.get(
            resource=self.composite_resource,
            file=self.raster_file_name,
            folder=fs_aggr_path,
        )
    ...
    def test_auto_geofeature_aggregation_creation(self):
        ...
        # aggregation
        geo_feature_files = [
            self.states_shp_file,
            self.states_shx_file,
            self.states_dbf_file,
            self.states_prj_file,
        ]
        self.add_files_to_resource(
            files_to_add=geo_feature_files, upload_folder=fs_aggr_path
        )
        ...
        # aggregation
        shp_res_file = ResourceFile.get(
            resource=self.composite_resource,
            file=self.states_shp_file_name,
            folder=fs_aggr_path,
        )
    ...
    def test_auto_timeseries_aggregation_creation(self):
        ...
        # aggregation
        self.add_files_to_resource(
            files_to_add=[self.sqlite_file], upload_folder=fs_aggr_path
        )
        # there should be two resource files
        self.assertEqual(self.composite_resource.files.all().count(), 2)
        ...
        sqlite_res_file = ResourceFile.get(
            resource=self.composite_resource,
            file=self.sqlite_file_name,
            folder=fs_aggr_path,
        )
    ...
    def test_auto_reftimeseries_aggregation_creation(self):
        ...
        # aggregation
        self.add_files_to_resource(
            files_to_add=[self.json_file], upload_folder=fs_aggr_path
        )
        # there should be two resource files
        self.assertEqual(self.composite_resource.files.all().count(), 2)
    ...
    def test_rename_aggregation_2(self):
        ...
        # There should be 2 fileset aggregations
        self.assertEqual(FileSetLogicalFile.objects.count(), 2)
        fs_aggr_child = FileSetLogicalFile.objects.filter(
            folder=child_fs_folder
        ).first()
        ...
        self.assertEqual(
            FileSetLogicalFile.objects.filter(folder=new_child_fs_folder).count(), 1
        )
        self.assertEqual(
            FileSetLogicalFile.objects.filter(folder=parent_fs_folder).count(), 1
        )
        self.assertFalse(self.composite_resource.dangling_aggregations_exist())
        self.composite_resource.delete()
    ...
    def test_rename_aggregation_3(self):
        ...
        # There should be 2 fileset aggregations
        self.assertEqual(FileSetLogicalFile.objects.count(), 2)
        fs_aggr_parent = FileSetLogicalFile.objects.filter(
            folder=parent_fs_folder
        ).first()
        self.assertEqual(
            FileSetLogicalFile.objects.filter(folder=child_fs_folder).count(), 1
        )
        ...
        self.assertEqual(
            FileSetLogicalFile.objects.filter(folder=new_parent_fs_folder).count(), 1
        )
        self.assertEqual(
            FileSetLogicalFile.objects.filter(folder=new_child_fs_folder).count(), 1
        )
    ...
    def test_rename_aggregation_4(self):
        ...
        # add the the json file to the resource at the above sub folder
        self.add_file_to_resource(
            file_to_add=self.json_file, upload_folder=child_fs_folder
        )
        # there should be two resource files
        self.assertEqual(self.composite_resource.files.all().count(), 2)
    ...
    def test_move_fileset_into_another_fileset(self):
        ...
        # add the the txt file to the resource at the above folder
        self.add_file_to_resource(
            file_to_add=self.generic_file, upload_folder=fs_1_folder
        )
        # there should be one resource file
        self.assertEqual(self.composite_resource.files.all().count(), 1)
        # set folder to fileset logical file type (aggregation)
        FileSetLogicalFile.set_file_type(
            self.composite_resource, self.user, folder_path=fs_1_folder
        )
        # There should be one fileset aggregation associated with one resource file
        self.assertEqual(FileSetLogicalFile.objects.count(), 1)
        ...
        FileSetLogicalFile.set_file_type(
            self.composite_resource, self.user, folder_path=fs_2_folder
        )

        # There should be 2 fileset aggregations now
        self.assertEqual(FileSetLogicalFile.objects.count(), 2)
        self.assertEqual(
            FileSetLogicalFile.objects.filter(folder=fs_1_folder).count(), 1
        )
        self.assertEqual(
            FileSetLogicalFile.objects.filter(folder=fs_2_folder).count(), 1
        )
        ...
        self.assertEqual(
            FileSetLogicalFile.objects.filter(folder=new_fs2_folder).count(), 1
        )
        self.assertFalse(self.composite_resource.dangling_aggregations_exist())
        self.composite_resource.delete()
    ...
    def test_remove_child_fs_aggregation(self):
        ...
        txt_res_file = ResourceFile.get(
            resource=self.composite_resource,
            file=self.generic_file_name,
            folder=parent_fs_folder,
        )
    ...
    def test_remove_grand_child_aggregation(self):
        ...
        parent_fs_aggr = self.composite_resource.get_aggregation_by_name(
            parent_fs_folder
        )
        ...
        self.add_files_to_resource(
            files_to_add=[self.raster_file], upload_folder=child_fs_folder
        )
    ...
    def test_auto_update_temporal_coverage_from_children_1(self):
        ...
        for temp_date in ("start", "end"):
            self.assertEqual(
                fs_aggr.metadata.temporal_coverage.value[temp_date],
    ...
    def test_auto_update_spatial_coverage_from_children_1(self):
        ...
        for limit in ("northlimit", "eastlimit", "southlimit", "westlimit"):
            self.assertEqual(
                fs_aggr.metadata.spatial_coverage.value[limit],
    ...
    def test_auto_update_spatial_coverage_from_children_2(self):
        ...
        self.assertNotEqual(fs_aggr.metadata.spatial_coverage, None)
        self.assertEqual(
            fs_aggr.metadata.spatial_coverage.value["northlimit"], 42.05002695977342
        )
        self.assertEqual(
            fs_aggr.metadata.spatial_coverage.value["eastlimit"], -111.57773718106199
        )
        self.assertEqual(
            fs_aggr.metadata.spatial_coverage.value["southlimit"], 41.98722286030317
        )
        self.assertEqual(
            fs_aggr.metadata.spatial_coverage.value["westlimit"], -111.6975629308406
        )
    ...
    def test_update_spatial_coverage_from_children(self):
        ...
        self.assertEqual(
            fs_aggr.metadata.spatial_coverage.value["eastlimit"], -111.5059403684569
        )
        self.assertEqual(
            fs_aggr.metadata.spatial_coverage.value["southlimit"], 41.86390807452128
        )
        self.assertEqual(
            fs_aggr.metadata.spatial_coverage.value["westlimit"], -111.6975629308406
        )
    ...
    def test_delete_aggregation_coverage(self):
        ...
        self.assertNotEqual(fs_aggr.metadata.spatial_coverage, None)
        fs_aggr.metadata.delete_element(
            "coverage", fs_aggr.metadata.spatial_coverage.id
        )
        self.assertEqual(fs_aggr.metadata.spatial_coverage, None)
        self.assertTrue(fs_aggr.metadata.is_dirty)
        ...
        fs_aggr.metadata.save()
        fs_aggr.metadata.delete_element(
            "coverage", fs_aggr.metadata.temporal_coverage.id
        )
        self.assertEqual(fs_aggr.metadata.temporal_coverage, None)
        self.assertTrue(fs_aggr.metadata.is_dirty)
        self.assertFalse(self.composite_resource.dangling_aggregations_exist())
