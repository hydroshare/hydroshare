...
from hs_tracking.models import (
    Variable,
    Session,
    Visitor,
    SESSION_TIMEOUT,
...
class ViewTests(TestCase):
    ...
    def test_get(self):
        ...
        app_name = "test"
        request_url = (
            "https://apps.hydroshare.org/apps/hydroshare-gis/"
    ...
    def test_get_bad_redirect(self):
        ...
        res_type = "GenericResource"
        app_name = "test"
        request_url = "https://www.youtube.com/" "?res_id=%s&res_type=%s" % (
            res_id,
            res_type,
        )
...
class TrackingTests(TestCase):
    ...
    def test_record_bad_value(self):
        self.assertRaises(
            TypeError, self.session.record, "bad", ["oh no i cannot handle arrays"]
        )
    ...
    def test_for_request_expired(self):
        ...
        with patch("hs_tracking.models.datetime") as dt_mock:
            dt_mock.now.return_value = datetime.now() + timedelta(
