...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--days", type=int, dest="days", help="number of days to list"
        )
        parser.add_argument(
            "--resource", type=str, dest="resource", help="resource to list"
        )
        parser.add_argument(
            "--username", type=str, dest="username", help="username to list"
        )
    ...
    def handle(self, *args, **options):
        ...
        if user is not None and resource is not None:
            ...
            query = Q(
                resource_id=resource, timestamp__gte=(datetime.now() - timedelta(days))
            )
        else:
        ...
        for v in Variable.objects.filter(query):
            ...
            print(
                "{} name={} resource_id={} landing={} rest={} internal={} value={}".format(
                    time,
                    v.name,
                    v.last_resource_id,
                    v.landing,
