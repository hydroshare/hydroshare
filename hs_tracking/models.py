...
class SessionManager(models.Manager):
    def for_request(self, request, user=None):
        ...
        if signed_id:
            ...
            try:
                session = (
                    Session.objects.filter(variable__timestamp__gte=cut_off)
                    .filter(id=tracking_id["id"])
                    .first()
                )
...
class Session(models.Model):
    ...
    visitor = models.ForeignKey(
        Visitor, on_delete=models.CASCADE, related_name="session"
    )
...
class Variable(models.Model):
    ...
    TYPE_CHOICES = [
        (i, label) for (i, label) in enumerate(label for (label, coercer) in TYPES)
    ]

    from hs_core.models import BaseResource

    session = models.ForeignKey(
        Session, on_delete=models.CASCADE, related_name="variable"
    )
    ...
    resource = models.ForeignKey(
        BaseResource, null=True, related_name="variable", on_delete=models.SET_NULL
    )
    ...
    def encode(cls, value):
        if value is None:
            ...
            raise ValueError(
                "Unknown type (%s) for tracking variable: %r",
                type(value).__name__,
                value,
            )
    ...
    def encode_type(cls, value):
        if value is None:
            ...
            raise TypeError(
                "Unable to record variable of unrecognized type %s",
                type(value).__name__,
            )
    ...
    def recent_users(cls, resource, n_users=5, days=60):
        ...
        return (
            User.objects.filter(
                visitor__session__variable__resource=resource,
                visitor__session__variable__name="visit",
                visitor__session__variable__timestamp__gte=(
