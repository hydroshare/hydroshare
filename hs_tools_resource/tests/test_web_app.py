...
from hs_tools_resource.receivers import (
...
class TestWebAppFeature(TestCaseCommonUtilities, TransactionTestCase):
    ...
    def test_web_app_res_specific_metadata(self):
        ...
        resource.create_metadata_element(
            self.resWebApp.short_id, "RequestUrlBase", value="https://www.google.com"
        )
        ...
        resource.create_metadata_element(
            self.resWebApp.short_id, "ToolVersion", value="1.0"
        )
        ...
        with self.assertRaises(Exception):
            resource.create_metadata_element(
                self.resWebApp.short_id, "ToolVersion", value="2.0"
            )
        self.assertEqual(ToolVersion.objects.all().count(), 1)
        ...
        self.assertEqual(
            SupportedResTypes.objects.first().supported_res_types.all()[0].description,
            "CollectionResource",
        )
        ...
        self.assertEqual(
            ToolIcon.objects.first().value,
            "https://www.hydroshare.org/static/static/img/logo-sm.png",
        )

        # delete ToolIcon obj
        resource.delete_metadata_element(
            self.resWebApp.short_id, "ToolIcon", element_id=ToolIcon.objects.first().id
        )
        self.assertEqual(ToolIcon.objects.all().count(), 0)
        ...
        resource.create_metadata_element(
            self.resWebApp.short_id, "AppHomePageUrl", value="https://my_web_app.com"
        )
        ...
        self.assertEqual(
            AppHomePageUrl.objects.first().value, "http://my_web_app_3.com"
        )
    ...
    def test_metadata_element_pre_create_and_update(self):
        ...
        data = metadata_element_pre_create_handler(
            sender=ToolResource, element_name="RequestUrlBase", request=request
        )
        self.assertTrue(data["is_valid"])
        data = metadata_element_pre_update_handler(
            sender=ToolResource, element_name="RequestUrlBase", request=request
        )
        self.assertTrue(data["is_valid"])
        ...
        data = metadata_element_pre_create_handler(
            sender=ToolResource, element_name="ToolVersion", request=request
        )
        self.assertTrue(data["is_valid"])
        data = metadata_element_pre_update_handler(
            sender=ToolResource, element_name="ToolVersion", request=request
        ...
        data = metadata_element_pre_create_handler(
            sender=ToolResource, element_name="SupportedResTypes", request=request
        )
        self.assertTrue(data["is_valid"])
        data = metadata_element_pre_update_handler(
            sender=ToolResource, element_name="SupportedResTypes", request=request
        ...
        data = metadata_element_pre_create_handler(
            sender=ToolResource, element_name="ToolIcon", request=request
        )
        self.assertTrue(data["is_valid"])
    ...
    def test_bulk_metadata_update(self):
        ...
        for res_type in supported_res_type.supported_res_types.all():
            self.assertIn(
                res_type.description, ["CollectionResource", "CompositeResource"]
            )
        ...
        metadata.append(
            {"supportedsharingstatus": {"sharing_status": ["Public", "Discoverable"]}}
        )
        # do the bulk metadata update
        self.resWebApp.metadata.update(metadata, self.user)
    ...
    def test_web_app_extended_metadata_appkey_association(self):
        ...
        self.assertIsNotNone(
            relevant_tools,
            msg="relevant_tools should not be None with appkey " "matching",
        )
        ...
        self.assertIsNone(
            relevant_tools,
            msg="relevant_tools should be None with no appkey " "matching",
        )
    ...
    def test_web_app_extended_metadata_custom_key(self):
        ...
        metadata = []
        metadata.append(
            {"requesturlbase": {"value": "https://www.google.com?s=${search_string}"}}
        )
        metadata.append(
            {"supportedrestypes": {"supported_res_types": ["CompositeResource"]}}
        )
        ...
        self.assertIsNone(
            relevant_tools,
            msg="relevant_tools should be None with no web app " "matching",
        )
        ...
        self.assertIsNotNone(
            relevant_tools,
            msg="relevant_tools should not be None with a default " "web app key",
        )
        tl = relevant_tools["tool_list"][0]
        self.assertEqual(tl["url"], "{'value': 'https://www.google.com?s=it works'}")
    ...
    def test_utils(self):
        ...
        new_url_string = parse_app_url_template(
            url_template_string, [self.resComposite.get_hs_term_dict(), term_dict_user]
        )
    ...
    def test_agg_types(self):
        ...
        resource_file_add_process(
            resource=self.resComposite, files=(open_file,), user=self.user
        )
        ...
        relevant_tools = resource_level_tool_urls(self.resComposite, request)
        self.assertIsNone(
            relevant_tools, msg="relevant_tools should have no approved resources"
        )
    ...
    def test_copy(self):
        ...
        new_web_app = create_empty_resource(
            self.resWebApp.short_id, self.user, action="copy"
        )
