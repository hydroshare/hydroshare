...
class TestWebAppValidationFeature(TestCaseCommonUtilities, TransactionTestCase):
    ...
    def test_file_level_keys_add_good(self):
        ...
        response = add_metadata_element(
            request, shortkey=self.resWebApp.short_id, element_name="requesturlbasefile"
        )
    ...
    def test_file_level_keys_add_custom(self):
        custom_url = "https://www.google.com?" "file_id=${custom_KEY}"
    ...
    def test_aggregation_level_keys_add_custom(self):
        custom_url = "https://www.google.com?" "file_id=${HS_CUSTOM}"
    ...
    def test_resource_level_keys_add_good(self):
        ...
        request.META["HTTP_REFERER"] = "http_referer"
        response = add_metadata_element(
            request, shortkey=self.resWebApp.short_id, element_name="requesturlbase"
        )
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
    ...
    def test_file_extensions_update(self):
        ...
        self.assertEqual(1, SupportedFileExtensions.objects.all().count())
        self.assertEqual(
            updated_extensions, SupportedFileExtensions.objects.first().value
        )
