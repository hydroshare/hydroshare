...
print(("[{0}] Find {1} Tools Resources\n".format(str(datetime.now()), tool_res_count)))
counter = 0
...
for tool_res_obj in tool_res_list:
    try:
        ...
        print(
            (
                "[{0}] Checking Tool {1}: ({2}/{3})\n".format(
                    str(datetime.now()), tool_res_obj.short_id, counter, tool_res_count
                )
            )
        )
        ...
        if tool_res_obj.metadata.supported_res_types is not None:
            if tool_res_obj.metadata.supported_res_types.count() != 1:
                print(
                    (
                        "obj count: {0}".format(
                            tool_res_obj.metadata.supported_res_types.count()
                        )
                    )
                )
        ...
        if tool_res_obj.metadata.supported_sharing_status is not None:
            if tool_res_obj.metadata.supported_sharing_status.count() != 1:
                print(
                    (
                        "obj count: {0}".format(
                            tool_res_obj.metadata.supported_sharing_status.count()
                        )
                    )
                )
        ...
        print(
            (
                "[{0}] Failed to check Tools {1}: {2}\n".format(
                    str(datetime.now()), tool_res_obj.short_id, str(ex)
                )
            )
        )
...
print(
    (
        "TOTAL RES: {0}; CHECK SUCCESS: {1}; CHECK ERROR: {2}\n".format(
            tool_res_count, success_check_counter, error_check_counter
        )
    )
)
...
print(
    (
        "Error supported_sharing_status: {0}\n".format(
            len(error_supported_sharing_status)
        )
    )
)
