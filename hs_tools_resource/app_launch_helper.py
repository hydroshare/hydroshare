...
from hs_tools_resource.utils import (
    parse_app_url_template,
    get_SupportedResTypes_choices,
)
...
def resource_level_tool_urls(resource_obj, request_obj):
    ...
    if (
        resource_obj.resource_type in supported_res_types
        and tool_app_key in resource_obj.extra_metadata
    ):
        # check matching appkey with web app tool resources
        appkey_dict = {tool_app_key: resource_obj.extra_metadata[tool_app_key]}
        for tool_res_obj in ToolResource.objects.filter(
            extra_metadata__contains=appkey_dict
        ):
            # tool_res_obj has the same appkey-value pair so needs to associate with the resource
            if _check_user_can_view_app(
                request_obj, tool_res_obj
            ) and _check_app_supports_resource_sharing_status(
                resource_obj, tool_res_obj
            ):
                tl = _get_app_tool_info(
                    request_obj, resource_obj, tool_res_obj, open_with=True
                )
                if tl:
                    tool_list.append(tl)
    ...
    for tool_res_obj in ToolResource.objects.exclude(short_id__in=tool_res_id_list):
        ...
        if tool_metadata.supported_resource_types.supported_res_types.filter(
            description__iexact=resource_obj.resource_type
        ).exists():
            if _check_user_can_view_app(
                request_obj, tool_res_obj
            ) and _check_app_supports_resource_sharing_status(
...
def _get_app_tool_info(request_obj, resource_obj, tool_res_obj, open_with=False):
    ...
    """
    is_open_with_app = (
        True if open_with else _check_open_with_app(tool_res_obj, request_obj)
    )
    ...
    tool_metadata = tool_res_obj.metadata
    tool_url_resource = tool_metadata.url_base.value if tool_metadata.url_base else None
    tool_url_aggregation = (
        tool_metadata.url_base_aggregation.value
        if tool_metadata.url_base_aggregation
        else None
    )
    tool_url_file = (
        tool_metadata.url_base_file.value if tool_metadata.url_base_file else None
    )
    tool_icon_url = (
        tool_metadata.app_icon.data_url if tool_metadata.app_icon else "raise-img-error"
    )
    ...
    if any(
        [
            tool_url_resource_new is not None,
            tool_url_agg_new is not None,
            tool_url_file_new is not None,
        ]
    ):
        tl = {
            "title": str(tool_res_obj.metadata.title.value),
...
def _check_open_with_app(tool_res_obj, request_obj):
    return _check_webapp_in_user_open_with_list(
        tool_res_obj, request_obj
    ) or _check_webapp_is_approved(tool_res_obj)
...
def _check_app_supports_resource_sharing_status(resource_obj, tool_res_obj):
    ...
    supported_sharing_status_obj = tool_res_obj.metadata.supported_sharing_status
    if supported_sharing_status_obj is not None:
        supported_sharing_status_str = (
            supported_sharing_status_obj.get_sharing_status_str()
        )
        if len(supported_sharing_status_str) > 0:
            res_sharing_status = resource_obj.raccess.sharing_status
            if (
                supported_sharing_status_str.lower().find(res_sharing_status.lower())
                != -1
            ):
