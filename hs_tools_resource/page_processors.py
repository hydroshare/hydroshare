...
from .forms import (
    ...
    RoadmapForm,
    VersionForm,
    SupportedResTypesForm,
    SupportedAggTypesForm,
    SupportedSharingStatusForm,
    ToolIconForm,
    UrlBaseForm,
    SupportedFileExtensionsForm,
    UrlBaseAggregationForm,
    UrlBaseFileForm,
)
...
def landing_page(request, page):
    ...
    if not edit_resource:
        ...
        if content_model.metadata.supported_resource_types:
            extended_metadata_exists = True
            supported_res_types_str = (
                content_model.metadata.supported_resource_types.get_supported_res_types_str()
            )
        ...
        if content_model.metadata.supported_aggregation_types:
            extended_metadata_exists = True
            supported_agg_types_str = (
                content_model.metadata.supported_aggregation_types.get_supported_agg_types_str()
            )
        ...
        if content_model.metadata.supported_sharing_status is not None:
            extended_metadata_exists = True
            sharing_status_str = (
                content_model.metadata.supported_sharing_status.get_sharing_status_str()
            )
        ...
        url_base_aggregation_form = UrlBaseAggregationForm(
            instance=url_base_aggregation,
            res_short_id=content_model.short_id,
            element_id=url_base_aggregation.id if url_base_aggregation else None,
        )

        url_base_file = content_model.metadata.url_base_file
        url_base_file_form = UrlBaseFileForm(
            instance=url_base_file,
            res_short_id=content_model.short_id,
            element_id=url_base_file.id if url_base_file else None,
        )

        supported_file_extensions = content_model.metadata.supported_file_extensions
        supported_file_extensions_form = SupportedFileExtensionsForm(
            instance=supported_file_extensions,
            res_short_id=content_model.short_id,
        ...
        homepage_url_form = AppHomePageUrlForm(
            instance=homepage_url,
            res_short_id=content_model.short_id,
            element_id=homepage_url.id if homepage_url else None,
        )

        testing_protocol_url = content_model.metadata.testing_protocol_url
        testing_protocol_url_form = TestingProtocolUrlForm(
            instance=testing_protocol_url,
            res_short_id=content_model.short_id,
            element_id=testing_protocol_url.id if testing_protocol_url else None,
        )

        help_page_url = content_model.metadata.help_page_url
        help_page_url_form = HelpPageUrlForm(
            instance=help_page_url,
            res_short_id=content_model.short_id,
            element_id=help_page_url.id if help_page_url else None,
        )

        source_code_url = content_model.metadata.source_code_url
        source_code_url_form = SourceCodeUrlForm(
            instance=source_code_url,
            res_short_id=content_model.short_id,
            element_id=source_code_url.id if source_code_url else None,
        )

        issues_page_url = content_model.metadata.issues_page_url
        issues_page_url_form = IssuesPageUrlForm(
            instance=issues_page_url,
            res_short_id=content_model.short_id,
            element_id=issues_page_url.id if issues_page_url else None,
        )

        mailing_list_url = content_model.metadata.mailing_list_url
        mailing_list_url_form = MailingListUrlForm(
            instance=mailing_list_url,
            res_short_id=content_model.short_id,
            element_id=mailing_list_url.id if mailing_list_url else None,
        )
        ...
        roadmap_form = RoadmapForm(
            instance=roadmap,
            res_short_id=content_model.short_id,
            element_id=roadmap.id if roadmap else None,
        )
        ...
        supported_agg_types_form = SupportedAggTypesForm(
            instance=supported_agg_types_obj,
            res_short_id=content_model.short_id,
            element_id=supported_agg_types_obj.id if supported_agg_types_obj else None,
        )

        sharing_status_obj = content_model.metadata.supported_sharing_status
        sharing_status_obj_form = SupportedSharingStatusForm(
            instance=sharing_status_obj,
            res_short_id=content_model.short_id,
            element_id=sharing_status_obj.id if sharing_status_obj else None,
        )
        ...
        ext_md_layout = Layout(
            ...
            HTML(
                '<div class="form-group col-lg-6 col-xs-12" id="SupportedAggTypes"> '
                "{% load crispy_forms_tags %} "
            ...
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='supported_file_extensions'> "
                "{% load crispy_forms_tags %} "
            ...
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='url_bases'> "
                "{% load crispy_forms_tags %} "
                "{% crispy url_base_form %} "
                "</div>"
            ),
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='url_bases_aggregation'> "
                "{% load crispy_forms_tags %} "
                "{% crispy url_base_aggregation_form %} "
                "</div>"
            ),
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='url_bases_file'> "
                "{% load crispy_forms_tags %} "
                "{% crispy url_base_file_form %} "
                "</div>"
            ),
            HTML(
                '<div class="form-group col-lg-6 col-xs-12" id="version"> '
                "{% load crispy_forms_tags %} "
                "{% crispy version_form %} "
                "</div> "
            ),
            HTML(
                '<div class="form-group col-lg-6 col-xs-12" id="tool_icon"> '
                "{% load crispy_forms_tags %} "
                "{% crispy tool_icon_form %} "
                "</div> "
            ),
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='testing_protocol_url'> "
                "{% load crispy_forms_tags %} "
                "{% crispy testing_protocol_url_form %} "
                "</div>"
            ),
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='help_page_url'> "
                "{% load crispy_forms_tags %} "
                "{% crispy help_page_url_form %} "
                "</div>"
            ),
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='source_code_url'> "
                "{% load crispy_forms_tags %} "
                "{% crispy source_code_url_form %} "
                "</div>"
            ),
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='issues_page_url'> "
                "{% load crispy_forms_tags %} "
                "{% crispy issues_page_url_form %} "
                "</div>"
            ),
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='mailing_list_url'> "
                "{% load crispy_forms_tags %} "
                "{% crispy mailing_list_url_form %} "
                "</div>"
            ),
            HTML(
                "<div class='form-group col-lg-6 col-xs-12' id='roadmap'> "
                "{% load crispy_forms_tags %} "
                "{% crispy roadmap_form %} "
                "</div>"
            ),
