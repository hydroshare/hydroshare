...
urlpatterns = [
    ...
    url(
        r"^$",
        schema_view_yasg.with_ui("swagger", cache_timeout=None),
        name="schema-swagger-ui",
    ),
    url(
        r"^redoc/$",
        schema_view_yasg.with_ui("redoc", cache_timeout=None),
        name="schema-redoc",
    ),
    # resource API
    url(
        r"^resource/types/$",
        core_views.resource_rest_api.ResourceTypes.as_view(),
        name="list_resource_types",
    ),
    url(
        r"^resource/content_types/$",
        core_views.resource_rest_api.ContentTypes.as_view(),
        name="list_contenttypes",
    ),
    url(
        r"^resource/$",
        core_views.resource_rest_api.ResourceListCreate.as_view(),
        name="list_create_resource",
    ),
    # Public endpoint for resource flags
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/flag/$",
        core_views.set_resource_flag_public,
        name="public_set_resource_flag",
    ),
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/$",
    ...
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/version/$",
        core_views.create_new_version_resource_public,
        name="new_version_resource_public",
    ),
    # public copy resource endpoint
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/copy/$",
        core_views.copy_resource_public,
        name="copy_resource_public",
    ),
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/share/(?P<privilege>[a-z]+)/group/(?P<group_id>[\w.@+-]+)/$",
        ShareResourceGroup.as_view(),
        name="share_resource_group_public",
    ),
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/share/(?P<privilege>[a-z]+)/user/(?P<user_id>[\w.@+-]+)/$",
        ShareResourceUser.as_view(),
        name="share_resource_user_public",
    ),
    # DEPRECATED: use form above instead
    url(
        r"^resource/accessRules/(?P<pk>[0-9a-f-]+)/$",
        core_views.resource_rest_api.AccessRulesUpdate.as_view(),
        name="DEPRECATED_update_access_rules",
    ),
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/sysmeta/$",
        core_views.resource_rest_api.SystemMetadataRetrieve.as_view(),
        name="get_system_metadata",
    ),
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/scimeta/$",
    ...
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/scimeta/elements/$",
    ...
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/file_list/$",
        core_views.resource_rest_api.ResourceFileListCreate.as_view(),
    ...
    url(
        r"^user/$",
        core_views.user_rest_api.UserInfo.as_view(),
        name="get_logged_in_user_info",
    ),
    url(
        r"^userInfo/$",
        core_views.user_rest_api.UserInfo.as_view(),
    ...
    url(
        r"^userDetails/(?P<user_identifier>.+)/$",
        core_views.hsapi_get_user,
        name="get_user_details",
    ),
    url(
        r"^dictionary/universities/$",
        dict_views.ListUniversities.as_view(),
        name="get_dictionary",
    ),
    # Resource Access
    url(
        r"^resource/(?P<pk>[0-9a-f-]+)/access/$",
        core_views.resource_access_api.ResourceAccessUpdateDelete.as_view(),
        name="get_update_delete_resource_access",
    ),
    url(
        r"^resource/search$",
        DiscoverSearchView.as_view({"get": "list"}),
        name="discover-hsapi",
    ),
    # Gets a list of metadata template schema file names for model program aggregation
    url(
        r"^modelprogram/template/meta/schemas/$",
        file_type_views.list_model_program_template_metadata_schemas,
    ...
    url(
        r"^modelprogram/template/meta/schema/(?P<schema_filename>.*)$",
    ...
    url(
        r"^resource/(?P<resource_id>[0-9a-f]+)/modelprogram/template/meta/schema/(?P<aggregation_path>.*)/"
        r"(?P<schema_filename>.*)$",
    ...
    # aggregation
    url(
        r"^resource/(?P<resource_id>[0-9a-f]+)/modelinstance/meta/schema/(?P<aggregation_path>.*)$",
    ...
    url(
        r"^resource/(?P<resource_id>[0-9a-f]+)/modelprogram/meta/(?P<aggregation_path>.*)$",
    ...
    url(
        r"^resource/(?P<resource_id>[0-9a-f]+)/modelinstance/meta/(?P<aggregation_path>.*)$",
