...
class TestServiceAccountMinIO(HSRESTTestCase):
    ...
    def test_cannot_delete_other_users_access_key(self):
        other_user = users.create_account(
        ...
        try:
            other_token, _ = AuthToken.objects.create(user=other_user)
            other_access_key = f"{other_user.username}:{other_token.token_key}"

            delete_url = reverse('minio_service_accounts_delete', kwargs={'access_key': other_access_key})
            response = self.client.delete(delete_url)

            self.assertEqual(response.status_code, 404)
            self.assertTrue(AuthToken.objects.filter(user=other_user, token_key=other_token.token_key).exists())
        finally:
            other_user.delete()
