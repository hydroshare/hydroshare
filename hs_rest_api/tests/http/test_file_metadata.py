...
class TestResourceFileMetadataEndpoint(HSRESTTestCase):
    ...
    def test_metadata_update_retrieve(self):
        ...
        response = self.client.get(
            reverse(
                "get_update_resource_file_metadata",
                kwargs={"pk": "abc123", "pathname": "bad_path"},
            )
        )
        self.assertEqual(
        ...
        response = self.client.get(
            reverse("get_update_delete_resource", kwargs={"pk": res_id})
        )
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)

        response = self.client.get(
            reverse("list_create_resource_file", kwargs={"pk": res_id})
        )
        response_json = json.loads(response.content.decode())
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        file_metadata_url = reverse(
            "get_update_resource_file_metadata",
            kwargs={"pk": res_id, "pathname": txt_file_name},
        )
        ...
        response = self.client.put(
            file_metadata_url, {"title": "New Metadata Title"}, format="json"
        )
        response = self.client.get(file_metadata_url)
        ...
        # Update keywords
        response = self.client.put(
            file_metadata_url, {"keywords": ["keyword1", "keyword2"]}, format="json"
        )
        response = self.client.get(file_metadata_url)
        json_response = json.loads(response.content.decode())
        ...
        response = self.client.put(
            file_metadata_url,
            {"extra_metadata": {"this": "is", "a": "test"}},
            format="json",
        )
        response = self.client.get(file_metadata_url)
        json_response = json.loads(response.content.decode())
        self.assertEqual(
            json_response.get("extra_metadata"), {"a": "test", "this": "is"}
        )
        ...
        json_response = json.loads(response.content.decode())
        self.assertEqual(
            json_response.get("spatial_coverage"),
            {
                "units": "Decimal degrees",
                "east": -84.0465,
                "north": 49.6791,
        ...
        response = self.client.put(
            file_metadata_url,
            {"temporal_coverage": {"start": "2018-02-04", "end": "2018-02-06"}},
            format="json",
        )
        response = self.client.get(file_metadata_url)
        json_response = json.loads(response.content.decode())
        self.assertEqual(
            json_response.get("temporal_coverage"),
            {"start": "2018-02-04", "end": "2018-02-06"},
        )
