...
| `resource.distributionContact.present` | Implemented via Contact-tagged entry in the `creator` array. MetaDIG jq `select(.roleName? == "Contact")` finds the entry. Note: the check evaluates `.creator[]`, not `contactPoint` — a separate `contactPoint` is emitted for Google Dataset Search but is not what this check evaluates. **Schema.org/MetaDIG clash**: `roleName` is not a valid `Person` property in strict schema.org (a `Role` wrapper is correct), but MetaDIG's jq requires `roleName` and `identifier` on the same object — a `Role` wrapper would break `resource.distributionContactIdentifier.present`. The schema.org validator warning for `roleName` on `Person` is accepted as a necessary MetaDIG compatibility trade-off. | `creator` array with Contact-role entry derived from first creator by `order` | Implemented |
