...
def test_resource_timeseries_csv_extraction():
    resource_id = str(uuid.uuid4())  # Generate a random hex resource ID
    write_s3_json(f"admin/{resource_id}/.hsmetadata/ODM2_Multi_Site_One_Variable_Test.csv.user_metadata.json", {
    ...
    with open("tests/test_files/timeseries/ODM2_Multi_Site_One_Variable_Test.csv", "rb") as f:
        s3_client.upload_fileobj(
            f, "admin", f"{resource_id}/data/contents/ODM2_Multi_Site_One_Variable_Test.csv")
    ...
    result_netcdf_metadata = read_s3_json(
        f"admin/{resource_id}/.hsjsonld/ODM2_Multi_Site_One_Variable_Test.csv.json")
    assert len(result_netcdf_metadata["associatedMedia"]) == 1
    assert result_netcdf_metadata["associatedMedia"][
        0]["name"] == "ODM2_Multi_Site_One_Variable_Test.csv"
    assert len(result_netcdf_metadata["isPartOf"]) == 1
...
def test_resource_timeseries_sqlite_extraction():
    ...
    with open("tests/test_files/timeseries/ODM2_Multi_Site_One_Variable.sqlite", "rb") as f:
        s3_client.upload_fileobj(
            f, "admin", f"{resource_id}/data/contents/ODM2_Multi_Site_One_Variable.sqlite")
    ...
    result_netcdf_metadata = read_s3_json(
        f"admin/{resource_id}/.hsjsonld/ODM2_Multi_Site_One_Variable.sqlite.json")
    assert len(result_netcdf_metadata["associatedMedia"]) == 1
    assert result_netcdf_metadata["associatedMedia"][
        0]["name"] == "ODM2_Multi_Site_One_Variable.sqlite"
    assert len(result_netcdf_metadata["isPartOf"]) == 1
...
def test_singlefile():
    resource_id = str(uuid.uuid4())  # Generate a random hex resource ID
    print(resource_id)
    write_s3_json(f"admin/{resource_id}/.hsmetadata/singlefile_aggregation/testfile.txt.user_metadata.json", {
                  "user_metadata": "this is singlefile user metadata"})
    sleep(1)
    with open("tests/test_files/folder_aggregation/testfile.txt", "rb") as f:
        s3_client.upload_fileobj(
            f, "admin", f"{resource_id}/data/contents/singlefile_aggregation/testfile.txt")
    ...
    result_metadata = read_s3_json(
        f"admin/{resource_id}/.hsjsonld/singlefile_aggregation/testfile.txt.json")
...
def test_raster():
    resource_id = str(uuid.uuid4())  # Generate a random hex resource ID
    print(resource_id)
    write_s3_json(f"admin/{resource_id}/.hsmetadata/raster_aggregation/logan1.tif.user_metadata.json", {
                  "user_metadata": "this is raster user metadata"})
    sleep(1)
    with open("tests/test_files/rasters/single/logan1.tif", "rb") as f:
        s3_client.upload_fileobj(
            f, "admin", f"{resource_id}/data/contents/raster_aggregation/logan1.tif")
    ...
    result_metadata = read_s3_json(
        f"admin/{resource_id}/.hsjsonld/raster_aggregation/logan1.tif.json")
    assert len(result_metadata["associatedMedia"]) == 1
    assert result_metadata["associatedMedia"][
        0]["name"] == "logan1.tif"
    assert len(result_metadata["isPartOf"]) == 1
...
def test_raster_vrt():
    resource_id = str(uuid.uuid4())  # Generate a random hex resource ID
    print(resource_id)
    write_s3_json(f"admin/{resource_id}/.hsmetadata/raster_aggregation/logan.vrt.user_metadata.json", {
                  "user_metadata": "this is raster user metadata"})
    sleep(1)
    with open("tests/test_files/rasters/logan.vrt", "rb") as f:
        s3_client.upload_fileobj(
            f, "admin", f"{resource_id}/data/contents/raster_aggregation/logan.vrt")
    with open("tests/test_files/rasters/logan1.tif", "rb") as f:
        s3_client.upload_fileobj(
            f, "admin", f"{resource_id}/data/contents/raster_aggregation/logan1.tif")
    with open("tests/test_files/rasters/logan2.tif", "rb") as f:
        s3_client.upload_fileobj(
            f, "admin", f"{resource_id}/data/contents/raster_aggregation/logan2.tif")
    ...
    result_metadata = read_s3_json(
        f"admin/{resource_id}/.hsjsonld/raster_aggregation/logan.vrt.json")
