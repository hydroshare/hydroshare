...
def test_singlefile_usermetadata():
    ...
    print(resource_id)
    with open("tests/test_files/folder_aggregation/testfile.txt", "rb") as f:
        s3_client.upload_fileobj(f, "admin", f"{resource_id}/data/contents/singlefile_aggregation/testfile.txt")

    sleep(1)
    result_resource_metadata = read_s3_json(f"admin/{resource_id}/.hsjsonld/dataset_metadata.json")
    ...
    result_resource_metadata = read_s3_json(f"admin/{resource_id}/.hsjsonld/dataset_metadata.json")
    assert len(result_resource_metadata["hasPart"]) == 1
    result_metadata = read_s3_json(f"admin/{resource_id}/.hsjsonld/singlefile_aggregation/testfile.txt.json")
