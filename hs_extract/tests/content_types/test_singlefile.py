...
def test_metadataobject_from_user_metadata(use_folder):
    ...
    with open("tests/test_files/folder_aggregation/testfile.txt", "rb") as f:
        s3_client.upload_fileobj(f, "resource", f"{resource_id}/data/contents/{folder_prefix}{file_name}")

    sleep(1)
    md = determine_metadata_object_from_user_metadata(user_metadata_path, True, TEST_ZONE)
...
def test_singlefile_is_content_type(use_folder, user_meta_file):
    ...
    content_path = f"resource/{resource_id}/data/contents/{folder_prefix}testfile.txt"
    user_metadata_path = f"resource/{resource_id}/.hsmetadata/{folder_prefix}testfile.txt.user_metadata.json"

    with open("tests/test_files/folder_aggregation/testfile.txt", "rb") as f:
        s3_client.upload_fileobj(
            f, "resource", f"{resource_id}/data/contents/{folder_prefix}testfile.txt")

    if user_meta_file:
        write_s3_json(user_metadata_path, {"user_metadata": "this is singlefile user metadata"})
    sleep(1)

    assert SingleFileMetadataObject.is_content_type(content_path, TEST_ZONE) is user_meta_file
...
def test_singlefile(use_folder):
    ...
    folder_prefix = "singlefile_aggregation/" if use_folder else ""
    print(resource_id)
    write_s3_json(f"resource/{resource_id}/.hsmetadata/{folder_prefix}testfile.txt.user_metadata.json", {
    ...
    result_metadata = read_s3_json(
        f"resource/{resource_id}/.hsjsonld/{folder_prefix}testfile.txt.json")
...
def test_singlefile_usermetadata(use_folder):
    ...
    print(resource_id)
    with open("tests/test_files/folder_aggregation/testfile.txt", "rb") as f:
        s3_client.upload_fileobj(f, "resource", f"{resource_id}/data/contents/{folder_prefix}testfile.txt")
    ...
    assert len(result_has_parts) == 1
    assert result_has_parts[0]["url"].endswith(f"{folder_prefix}testfile.txt.json")
    singlefile_metadata = read_s3_json(f"resource/{resource_id}/.hsjsonld/{folder_prefix}testfile.txt.json")
