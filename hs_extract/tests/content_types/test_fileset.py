...
def test_fileset_user_metadata():
    ...
    print(resource_id)
    with open("tests/test_files/folder_aggregation/testfile.txt", "rb") as f:
        s3_client.upload_fileobj(f, "admin", f"{resource_id}/data/contents/folder_aggregation/testfile.txt")
    ...
    result_netcdf_metadata = read_s3_json(f"admin/{resource_id}/.hsjsonld/folder_aggregation/dataset_metadata.json")
    assert result_netcdf_metadata["user_metadata"] == "this is fileset user metadata"
