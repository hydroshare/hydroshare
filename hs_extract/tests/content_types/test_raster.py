...
def test_raster_usermetadata():
    ...
    # Wait for metadata to be consistent
    sleep(1)
    result_metadata = read_s3_json(f"admin/{resource_id}/.hsjsonld/raster_aggregation/logan1.tif.json")
...
def test_raster_vrt_usermetadata():
    ...
    print(resource_id)
    with open("tests/test_files/rasters/logan.vrt", "rb") as f:
        s3_client.upload_fileobj(f, "admin", f"{resource_id}/data/contents/raster_aggregation/logan.vrt")
    with open("tests/test_files/rasters/logan1.tif", "rb") as f:
        s3_client.upload_fileobj(f, "admin", f"{resource_id}/data/contents/raster_aggregation/logan1.tif")
    with open("tests/test_files/rasters/logan2.tif", "rb") as f:
        s3_client.upload_fileobj(f, "admin", f"{resource_id}/data/contents/raster_aggregation/logan2.tif")

    # Wait for metadata to be consistent
    sleep(2)
    result_metadata = read_s3_json(f"admin/{resource_id}/.hsjsonld/raster_aggregation/logan.vrt.json")
    assert "user_metadata" not in result_metadata
