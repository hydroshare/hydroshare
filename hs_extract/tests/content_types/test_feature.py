...
def test_metadataobject(use_folder, shp_xml):
    ...
    res_bucket_path = "resource/resourceid"
    ...
    md = FeatureMetadataObject(
        f"{res_bucket_path}/data/contents/{folder_prefix}{file_name}", True, TEST_ZONE
    )
    file_name = "watersheds.shp"
...
def test_feature_is_content_type(use_folder, shp_xml):
    ...
    assert FeatureMetadataObject.is_content_type(content_path, TEST_ZONE) is True
...
def test_feature(use_folder):
    ...
    folder_prefix = "feature_aggregation/" if use_folder else ""
    print(resource_id)
    write_s3_json(f"resource/{resource_id}/.hsmetadata/{folder_prefix}watersheds.shp.user_metadata.json", {
                  "user_metadata": "this is feature user metadata"})
    sleep(1)
    with open("tests/test_files/watersheds/watersheds.shp", "rb") as f:
        s3_client.upload_fileobj(
            f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.shp")

    with open("tests/test_files/watersheds/watersheds.cpg", "rb") as f:
        s3_client.upload_fileobj(
            f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.cpg")

    with open("tests/test_files/watersheds/watersheds.dbf", "rb") as f:
        s3_client.upload_fileobj(
            f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.dbf")

    with open("tests/test_files/watersheds/watersheds.prj", "rb") as f:
        s3_client.upload_fileobj(
            f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.prj")

    with open("tests/test_files/watersheds/watersheds.sbn", "rb") as f:
        s3_client.upload_fileobj(
            f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.sbn")

    with open("tests/test_files/watersheds/watersheds.sbx", "rb") as f:
        s3_client.upload_fileobj(
            f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.sbx")

    with open("tests/test_files/watersheds/watersheds.shx", "rb") as f:
        s3_client.upload_fileobj(
            f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.shx")

    with open("tests/test_files/watersheds/watersheds.shp.xml", "rb") as f:
        s3_client.upload_fileobj(
            f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.shp.xml")
    ...
    assert_manifest_reference(result_resource_metadata, resource_id, "resource", expected_media_obj_count=8)
...
def test_feature_user_metadata(use_folder):
    ...
    print(resource_id)
    with open("tests/test_files/watersheds/watersheds.shp", "rb") as f:
        s3_client.upload_fileobj(f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.shp")
    with open("tests/test_files/watersheds/watersheds.shx", "rb") as f:
        s3_client.upload_fileobj(f, "resource", f"{resource_id}/data/contents/{folder_prefix}watersheds.shx")

    sleep(2)
    result_metadata = read_s3_json(f"resource/{resource_id}/.hsjsonld/{folder_prefix}watersheds.shp.json")
