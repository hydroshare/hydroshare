...
def test_metadataobject(use_folder):
    ...
    md = BaseMetadataObject(f"resource/resourceid/data/contents/{folder_prefix}file.txt", True, TEST_ZONE)
    assert md.file_object_path == f"resource/resourceid/data/contents/{folder_prefix}file.txt"
...
def test_resource_extraction(use_folder):
    ...
    write_s3_json(f"resource/{resource_id}/data/contents/{folder_prefix}file.txt",
    ...
    write_s3_json(f"resource/{resource_id}/data/contents/{folder_prefix}file-1.txt",
                  {"file_metadata": "this is file-1 metadata"})
    sleep(1)
    # read in the resulting resource metadata file
    result_resource_metadata = read_s3_json(
        f"resource/{resource_id}/.hsjsonld/dataset_metadata.json")
    assert_manifest_reference(result_resource_metadata, resource_id, "resource", expected_media_obj_count=2)
    assert_has_part_reference(result_resource_metadata, resource_id, "resource", expected_has_part_count=0)
...
def test_resource_extraction_without_data_files_writes_empty_sidecars():
    ...
    write_s3_json(
        f"resource/{resource_id}/.hsmetadata/system_metadata.json",
        {"system_metadata": "this is system metadata"},
    )
    write_s3_json(
    ...
    assert result_resource_metadata["user_metadata"] == "this is user metadata"
    file_manifest_path = f"resource/{resource_id}/.hsjsonld/file_manifest.json"
    has_parts_path = f"resource/{resource_id}/.hsjsonld/has_parts.json"
