...
def test_resource_netcdf_user_metadata():
    ...
    # Wait for metadata to be consistent
    sleep(1)
    result_netcdf_metadata = read_s3_json(f"admin/{resource_id}/.hsjsonld/netcdf_valid.nc.json")
    assert "user_metadata" not in result_netcdf_metadata

    write_s3_json(f"admin/{resource_id}/.hsmetadata/user_metadata.json", {"user_metadata": "this is user metadata"})
    sleep(1)
