...
##########################################################################################################3
    ...
    def test_geofeature_pre_create_resource(self):
        ...
        self.assertEqual (len(files), 7, "extracted files number error")

        for dict in metadata:
            self.assertEqual(len(dict.keys()),1)
            key=dict.keys()[0]
            if key == "OriginalFileInfo":
                self.assertEqual(dict["OriginalFileInfo"]["baseFilename"],"states")
                self.assertEqual(dict["OriginalFileInfo"]["fileCount"],7)
                self.assertEqual(dict["OriginalFileInfo"]["fileType"],"ZSHP")
            elif key == "field_info_array":
                self.assertEqual(len(dict["field_info_array"]),5)
            elif key == "geometryinformation":
                self.assertEqual(dict["geometryinformation"]["featureCount"],51)
                self.assertEqual(dict["geometryinformation"]["geometryType"],"MULTIPOLYGON")
            elif key == "originalcoverage":
                self.assertEqual(dict["originalcoverage"]['datum'],'North_American_Datum_1983')
                self.assertEqual(dict["originalcoverage"]['eastlimit'],-66.96927125875777)
                self.assertEqual(dict["originalcoverage"]['northlimit'],71.40623539396698)
                self.assertEqual(dict["originalcoverage"]['southlimit'],18.92178634508703)
    ...
    def test_geofeature_pre_add_files_to_resource(self):
        ...
        self.assertEqual(len(self.resGeoFeature.metadata.fieldinformation.all()), 7)
