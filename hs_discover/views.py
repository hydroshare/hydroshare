    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if filters.get("geofilter"):
                sqs = sqs.filter(
                    north__range=[-90, 90]
                )  # return resources with geographic data
            ...
            if filters.get("date"):
                try:
                    ...
                    sqs = sqs.filter(
                        start_date__gt=datetime.datetime.strptime(
                            "1900-01-01", "%Y-%m-%d"
                        )
                    ).filter(
                        end_date__lte=datetime.datetime.strptime(
                            datetime.date.today().isoformat(), "%Y-%m-%d"
                        )
                    )
                    ...
                    return JsonResponse(
                        {
                            "message": "Filter date parsing error expecting String %Y-%m-%d : {}".format(
                                str(date_ex)
                            ),
                    ...
                    return JsonResponse(
                        {
                            "message": "Filter date parsing error expecting two date string values : {}".format(
                                str(gen_date_ex)
                            ),
                            "received": request.query_params,
                        },
            ...
            return JsonResponse(
                {
                    "message": "Filter JSON parsing error - {}".format(str(parse_ex)),
                    "received": request.query_params,
                },
                status=400,
            )
            ...
            return JsonResponse(
                {
                    "message": "{}".format(
                        "{}: query error. Contact a server administrator.".format(
                            type(gen_ex)
                        )
                    )
                },
        ...
        if request.GET.get("filterbuilder"):
            authors = sqs.facet("author", limit=self.filterlimit).facet_counts()[
                "fields"
            ]["author"]
            owners = sqs.facet("owner", limit=self.filterlimit).facet_counts()[
                "fields"
            ]["owner"]
            subjects = sqs.facet("subject", limit=self.filterlimit).facet_counts()[
                "fields"
            ]["subject"]
            contributors = sqs.facet(
                "contributor", limit=self.filterlimit
            ).facet_counts()["fields"]["contributor"]
            types = sqs.facet("content_type", limit=self.filterlimit).facet_counts()[
                "fields"
            ]["content_type"]
            availability = sqs.facet(
                "availability", limit=self.filterlimit
            ).facet_counts()["fields"]["availability"]
        ...
        for result in p.page(pnum):
            contributor = (
                "None"  # contributor is actually a list and can have multiple values
            )
            ...
            author = (
                result.author if result.author is not None else "None"
            )  # SOLR scalar
            author_link = (
                result.author_url if result.author_url is not None else "None"
            )  # SOLR scalar
            ...
            resources.append(
                {
                    ...
                    "availabilityurl": "/static/static/img/{}.png".format(
