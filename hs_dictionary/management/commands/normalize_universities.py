...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(dirname(__file__) + "/normalize_universities.csv", "r") as csvfile:
            ...
            profiles_with_organization = UserProfile.objects.exclude(
            ...
            for profile in profiles_with_organization:
                ...
                match = [
                    tl for tl in transliterations if tl["Old"].lower() == organization
                ][0]
                ...
                if (
                    secondary_match
                    and not University.objects.filter(name=secondary_match).first()
                ):
                    if dry_run:
                        print(
                            "[DRY RUN] Would create dictionary term: %s"
                            % secondary_match
                        )
                    else:
